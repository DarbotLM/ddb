...
struct SupervisedSchedulerPool {
  ...
  explicit SupervisedSchedulerPool(
      ...
      : metricsFeature(std::make_shared<darbotdb::metrics::MetricsFeature>(
  ...
  std::shared_ptr<darbotdb::metrics::MetricsFeature> metricsFeature;
...
class ThreadPoolPerfTest : public testing::Test {
  void SetUp() override {
    darbotdb::Logger::CLUSTER.setLogLevel(LogLevel::ERR);
    darbotdb::Logger::THREADS.setLogLevel(LogLevel::ERR);
  }
};
