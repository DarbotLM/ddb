...
struct ThreadPoolScheduler : darbotdb::actor::IScheduler {
 private:
  darbotdb::ThreadGuard threads;
  std::queue<darbotdb::actor::LazyWorker> jobs;
 ...
 public:
  ...
  void queue(darbotdb::actor::LazyWorker&& job) override {
    {
      std::unique_lock lock(queue_mutex);
