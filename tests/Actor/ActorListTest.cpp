...
using darbotdb::actor::ActorID;
using darbotdb::actor::DistributedActorPID;

using ActorList = darbotdb::actor::ActorList<DistributedActorPID>;
using ActorBase = darbotdb::actor::ActorBase<DistributedActorPID>;
...
struct ActorBaseMock : ActorBase {
  ...
  auto process(DistributedActorPID sender,
               darbotdb::actor::MessagePayloadBase& msg) -> void override {}
  auto process(DistributedActorPID sender,
               darbotdb::velocypack::SharedSlice msg) -> void override {}
  auto typeName() -> std::string_view override { return type; }
  auto serialize() -> darbotdb::velocypack::SharedSlice override {
    return darbotdb::velocypack::SharedSlice();
  }
  auto finish(darbotdb::actor::ExitReason reason) -> void override {
    finished = true;
  }
...
TEST(ActorListTest, applies_function_to_each_actor) {
  ...
  list.apply([](std::shared_ptr<ActorBase>& actor) -> void {
    actor->finish(darbotdb::actor::ExitReason::kFinished);
  });
