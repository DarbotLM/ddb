...
using namespace darbotdb::actor;
using namespace darbotdb::actor::test;
...
struct EmptyExternalDispatcher : IExternalDispatcher {
  void dispatch(DistributedActorPID sender, DistributedActorPID receiver,
                darbotdb::velocypack::SharedSlice msg) override {}
};
...
TYPED_TEST(RuntimeTest, formats_runtime_and_actor_state) {
  ...
  ASSERT_EQ(
      fmt::format(
          "{}", darbotdb::inspection::json(
                    *runtime, darbotdb::inspection::JsonPrintFormat::kMinimal)),
      expected);
...
auto inspect(Inspector& f, SomeMessages& x) {
  return f.variant(x).unqualified().alternatives(
      darbotdb::inspection::type<SomeMessage>("someMessage"));
}
TYPED_TEST(
    RuntimeTest,
