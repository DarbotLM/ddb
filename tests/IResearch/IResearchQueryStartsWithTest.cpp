...
class QueryStartsWith : public QueryTest {
 protected:
  ...
  void queryTests() {
    ...
    {
      auto queryResult =
          darbotdb::tests::executeQuery(_vocbase, "RETURN starts_with()");
    ...
    {
      auto queryResult =
          darbotdb::tests::executeQuery(_vocbase, "RETURN starts_with('abc')");
      ASSERT_FALSE(queryResult.result.ok());
    }
    ...
    {
      auto queryResult =
          darbotdb::tests::executeQuery(_vocbase, "RETURN starts_with('a', 1)");
      ASSERT_TRUE(queryResult.result.ok());
      auto result = queryResult.data->slice();
      EXPECT_TRUE(result.isArray());
      darbotdb::velocypack::ArrayIterator resultIt(result);
    ...
    // d.prefix = abc*, d.seq DESC
    {
      ...
      for (auto const& doc : _insertedDocs) {
        ...
        if (prefixSlice.isNone() ||
            !darbotdb::iresearch::getStringRef(prefixSlice)
                 .starts_with("abc")) {
          continue;
        }
