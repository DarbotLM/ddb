...
class IResearchLinkTest
 ...
 protected:
  ...
  IResearchLinkTest() : server(false) {
    ...
    auto& ars = server.getFeature<darbotdb::iresearch::IResearchFeature>();
    ars.collectOptions(opts);
    auto* commitThreads = opts->get<darbotdb::options::UInt32Parameter>(
        "--arangosearch.commit-threads");
    opts->processingResult().touch("arangosearch.commit-threads");
    EXPECT_NE(nullptr, commitThreads);
    *commitThreads->ptr = 1;
    auto* consolidationThreads = opts->get<darbotdb::options::UInt32Parameter>(
    ...
    *consolidationThreads->ptr = 1;
    ...
    EXPECT_EQ((std::pair<size_t, size_t>{1, 1}),
              ars.limits(darbotdb::iresearch::ThreadGroup::_0));
    EXPECT_EQ((std::pair<size_t, size_t>{1, 1}),
              ars.limits(darbotdb::iresearch::ThreadGroup::_1));
...
static_assert("1_3simd" == getFormat(darbotdb::iresearch::LinkVersion::MIN));
static_assert("1_5simd" == getFormat(darbotdb::iresearch::LinkVersion::MAX));
...
TEST_F(IResearchLinkTest, test_defaults) {
  // no view specified
  {
    ...
    try {
      StorageEngineMock::buildLinkMock(darbotdb::IndexId{1}, *logicalCollection,
                                       json->slice());
      EXPECT_TRUE(false);
  ...
  // Agency yet)
  {
    ...
    auto logicalCollection = vocbase.createCollection(collectionJson->slice());
    ASSERT_NE(nullptr, logicalCollection);
    auto json = darbotdb::velocypack::Parser::fromJson(R"({ "view": "42" })");
    auto link = StorageEngineMock::buildLinkMock(
        darbotdb::IndexId{1}, *logicalCollection, json->slice());
  ...
  // valid link creation
  {
    ...
    auto* impl = dynamic_cast<darbotdb::iresearch::IResearchLink*>(link.get());
  ...
  // ensure jSON is still valid after unload()
  {
    ...
    EXPECT_EQ(0, link->memory());
    EXPECT_TRUE(link->sparse());
    EXPECT_EQ(darbotdb::Index::IndexType::TRI_IDX_TYPE_IRESEARCH_LINK,
              link->type());
    EXPECT_EQ(darbotdb::iresearch::StaticStrings::ViewArangoSearchType,
              link->typeName());
    EXPECT_FALSE(link->unique());
...
TEST_F(IResearchLinkTest, test_init) {
  // collection registered with view (collection initially not in view)
  {
    ...
    {
      std::unordered_set<darbotdb::DataSourceId> expected;
      std::set<darbotdb::DataSourceId> actual;

      EXPECT_TRUE((logicalView->visitCollections(
          [&actual](darbotdb::DataSourceId cid, LogicalView::Indexes*) {
            actual.emplace(cid);
            return true;
          })));
    ...
    auto link = StorageEngineMock::buildLinkMock(
        darbotdb::IndexId{1}, *logicalCollection, linkJson->slice());
    EXPECT_NE(nullptr, link.get());
...
TEST_F(IResearchLinkTest, test_self_token) {
  // test empty token
  {
    darbotdb::iresearch::AsyncLinkHandle empty(nullptr);
    auto lock = empty.lock();
    EXPECT_EQ(nullptr, lock.get());
  }

  darbotdb::iresearch::IResearchLink::AsyncLinkPtr self;

  {
    auto collectionJson = darbotdb::velocypack::Parser::fromJson(
        R"({ "name": "testCollection" })");
    auto linkJson =
        darbotdb::velocypack::Parser::fromJson(R"({ "view": "testView" })");
    auto viewJson = darbotdb::velocypack::Parser::fromJson(
    ...
    auto index = StorageEngineMock::buildLinkMock(
        darbotdb::IndexId{42}, *logicalCollection, linkJson->slice());
...
TEST_F(IResearchLinkTest, test_drop) {
  ...
  // touch view
  {
    ...
    EXPECT_EQ(dynamic_cast<darbotdb::iresearch::IResearchLink*>(link0.get())
                  ->getViewId(),
              logicalView->guid());
...
TEST_F(IResearchLinkTest, test_unload) {
  // index unload does not touch the view (subsequent destroy)
  {
    ...
    EXPECT_EQ(dynamic_cast<darbotdb::iresearch::IResearchLink*>(link.get())
                  ->getViewId(),
              logicalView->guid());
    link->unload();
    EXPECT_EQ(dynamic_cast<darbotdb::iresearch::IResearchLink*>(link.get())
...
TEST_F(IResearchLinkTest, test_write_index_creation_version_0) {
  ...
  auto doc0 = darbotdb::velocypack::Parser::fromJson(R"({ "abc": "def" })");
  TRI_vocbase_t vocbase(testDBInfo(server.server()));
  std::string dataPath =
  ...
  auto collectionJson =
      darbotdb::velocypack::Parser::fromJson(R"({ "name": "testCollection" })");
  auto viewJson = darbotdb::velocypack::Parser::fromJson(
      "{ \
    \"id\": 42, \
    \"name\": \"testView\", \
  ...
  view->open();
  ASSERT_TRUE(server.server().hasFeature<darbotdb::FlushFeature>());
  ...
  EXPECT_EQ(0, reader.Reopen().live_docs_count());
  {
    ...
    trx.addHint(darbotdb::transaction::Hints::Hint::INDEX_CREATION);
    EXPECT_TRUE((trx.begin().ok()));
    auto* l = dynamic_cast<darbotdb::iresearch::IResearchLinkMock*>(link.get());
    ASSERT_EQ(static_cast<uint32_t>(darbotdb::iresearch::LinkVersion::MIN),
              l->meta()._version);
    ASSERT_TRUE(l != nullptr);
    EXPECT_TRUE(
        (l->insert(trx, darbotdb::LocalDocumentId(1), doc0->slice()).ok()));
    l->commit(true);
    EXPECT_EQ(
...
TEST_F(IResearchLinkTest, test_write) {
  static std::vector<std::string> const kEmpty;
  auto doc0 = darbotdb::velocypack::Parser::fromJson(R"({ "abc": "def" })");
  auto doc1 = darbotdb::velocypack::Parser::fromJson(R"({ "ghi": "jkl" })");
  TRI_vocbase_t vocbase(testDBInfo(server.server()));
  std::string dataPath =
      ((((std::filesystem::path() /= testFilesystemPath) /=
         std::string("databases")) /=
...

  {
    ...
    ASSERT_TRUE(l != nullptr);
    EXPECT_TRUE(
        (l->insert(trx, darbotdb::LocalDocumentId(2), doc1->slice()).ok()));
    EXPECT_TRUE((trx.commit().ok()));
    EXPECT_TRUE((l->commit().ok()));
  }
...

  {
    ...
    auto* l = dynamic_cast<darbotdb::iresearch::IResearchLinkMock*>(link.get());
    ASSERT_TRUE(l != nullptr);
    EXPECT_TRUE((l->remove(trx, darbotdb::LocalDocumentId(2)).ok()));
    EXPECT_TRUE((trx.commit().ok()));
    EXPECT_TRUE((l->commit().ok()));
  }
...
TEST_F(IResearchLinkTest, test_write_with_custom_compression_nondefault_sole) {
  static std::vector<std::string> const kEmpty;
  auto doc0 = darbotdb::velocypack::Parser::fromJson(
...
TEST_F(IResearchLinkTest,
  ...
  auto doc0 = darbotdb::velocypack::Parser::fromJson(
      R"({ "abc": "def", "abc2":"aaa", "sort":"ps"  })");
  auto doc1 = darbotdb::velocypack::Parser::fromJson(
      R"({ "ghi": "jkl", "sort":"pp" })");
  TRI_vocbase_t vocbase(testDBInfo(server.server()));
  std::string dataPath =
      ((((std::filesystem::path() /= testFilesystemPath) /=
...
TEST_F(
  ...
  auto linkCallbackRemover =
      darbotdb::iresearch::IResearchLinkMock::setCallbackForScope([]() {
...
TEST_F(IResearchLinkTest, test_maintenance_disabled_at_creation) {
  ...
  std::shared_ptr<darbotdb::Index> link;
  auto logicalCollection = vocbase.createCollection(collectionJson->slice());
  ASSERT_NE(nullptr, logicalCollection);
...
#ifdef darbotdb_ENABLE_FAILURE_TESTS
    // ensure consolidation is rescheduled after exception
    {
      auto clearFailurePoints =
          darbotdb::scopeGuard(TRI_ClearFailurePointsDebugging);
...
using LinkStats = darbotdb::iresearch::IResearchDataStore::Stats;
using darbotdb::iresearch::MetricStats;
...
class IResearchLinkMetricsTest : public IResearchLinkTest {
 protected:
  ...
  std::array<std::shared_ptr<darbotdb::velocypack::Builder>, 3> _docs;
  std::shared_ptr<darbotdb::LogicalCollection> _logicalCollection;
  uint64_t _cleanupIntervalStep = 0;
  uint64_t _commitIntervalMs = 0;
  uint64_t _consolidationIntervalMs = 0;
  std::shared_ptr<darbotdb::LogicalView> _view;
  std::filesystem::path _dirPath = testFilesystemPath;
  std::shared_ptr<darbotdb::Index> _link;

  IResearchLinkMetricsTest() {
    _docs[0] = darbotdb::velocypack::Parser::fromJson(R"({ "abc": "def" })");
    _docs[1] = darbotdb::velocypack::Parser::fromJson(R"({ "ghia": "jkla" })");
    _docs[2] = darbotdb::velocypack::Parser::fromJson(R"({ "1234": "56789" })");
  ...
  bool checkMetricExist(std::string_view name, std::string_view label) const {
    darbotdb::metrics::MetricKeyView key{name, label};
    auto& f = _vocbase.server().getFeature<darbotdb::metrics::MetricsFeature>();
  ...
  void setLink(bool withSegmentsMin1 = false) {
    auto temp = fmt::format(
      ...
      "type": "arangosearch" {}
        ...
        withSegmentsMin1
            ...
            : "");
    auto viewJson = darbotdb::velocypack::Parser::fromJson(temp);
    ...
    EXPECT_TRUE(server.server().hasFeature<darbotdb::FlushFeature>());
    ...
    EXPECT_TRUE(checkMetricExist("darbotdb_search_num_failed_commits", label));
    EXPECT_TRUE(checkMetricExist("darbotdb_search_num_failed_cleanups", label));
    EXPECT_TRUE(
        checkMetricExist("darbotdb_search_num_failed_consolidations", label));
    EXPECT_TRUE(checkMetricExist("darbotdb_search_commit_time", label));
    EXPECT_TRUE(checkMetricExist("darbotdb_search_cleanup_time", label));
    EXPECT_TRUE(checkMetricExist("darbotdb_search_consolidation_time", label));
  }
  ...
  darbotdb::iresearch::IResearchLinkMock* getLink() {
    auto* l =
        dynamic_cast<darbotdb::iresearch::IResearchLinkMock*>(_link.get());
    EXPECT_NE(l, nullptr);
    return l;
  }
  ...
  void getPrometheusStr(std::string& result) {
    auto& f = _vocbase.server().getFeature<darbotdb::metrics::MetricsFeature>();
    auto [lock, batch] = f.getBatch("darbotdb_search_link_stats");
  ...
  double insert(uint64_t begin, uint64_t end, size_t docId,
    ...
    EXPECT_TRUE(trx.begin().ok());
    for (; begin != end; ++begin) {
      EXPECT_TRUE(l->insert(trx, darbotdb::LocalDocumentId(begin),
                            _docs[docId]->slice())
                      .ok());
    }
    EXPECT_TRUE(trx.commit().ok());
  ...
  double remove(uint64_t begin, uint64_t end) {
    ...
    EXPECT_TRUE(trx.begin().ok());
    for (; begin != end; ++begin) {
      EXPECT_TRUE(l->remove(trx, darbotdb::LocalDocumentId(begin)).ok());
    }
...
TEST_F(IResearchLinkMetricsTest, RemoveMetrics) {
  ...
  auto label = getLinkMetricLabel();
  resetLink();
  EXPECT_FALSE(checkMetricExist("darbotdb_search_num_failed_commits", label));
  EXPECT_FALSE(checkMetricExist("darbotdb_search_num_failed_cleanups", label));
  EXPECT_FALSE(
      checkMetricExist("darbotdb_search_num_failed_consolidations", label));
  EXPECT_FALSE(checkMetricExist("darbotdb_search_commit_time", label));
  EXPECT_FALSE(checkMetricExist("darbotdb_search_cleanup_time", label));
  EXPECT_FALSE(checkMetricExist("darbotdb_search_consolidation_time", label));
}
...
R"(# HELP darbotdb_search_num_docs Number of documents
# TYPE darbotdb_search_num_docs gauge
darbotdb_search_num_docs{{db="testVocbase",view="h3039/42",collection="{0}",index_id="{1}",shard=""}}3
# HELP darbotdb_search_num_live_docs Number of live documents
# TYPE darbotdb_search_num_live_docs gauge
darbotdb_search_num_live_docs{{db="testVocbase",view="h3039/42",collection="{0}",index_id="{1}",shard=""}}3
# HELP darbotdb_search_num_primary_docs Number of primary documents
# TYPE darbotdb_search_num_primary_docs gauge
darbotdb_search_num_primary_docs{{db="testVocbase",view="h3039/42",collection="{0}",index_id="{1}",shard=""}}3
# HELP darbotdb_search_num_segments Number of segments
# TYPE darbotdb_search_num_segments gauge
darbotdb_search_num_segments{{db="testVocbase",view="h3039/42",collection="{0}",index_id="{1}",shard=""}}3
# HELP darbotdb_search_num_files Number of files
# TYPE darbotdb_search_num_files gauge
darbotdb_search_num_files{{db="testVocbase",view="h3039/42",collection="{0}",index_id="{1}",shard=""}}16
# HELP darbotdb_search_index_size Size of the index in bytes
# TYPE darbotdb_search_index_size gauge
darbotdb_search_index_size{{db="testVocbase",view="h3039/42",collection="{0}",index_id="{1}",shard=""}}2054
)"
...
# HELP darbotdb_search_num_segments Number of segments
# TYPE darbotdb_search_num_segments gauge
darbotdb_search_num_segments{{db="testVocbase",view="h3039/42",collection="{0}",index_id="{1}",shard=""}}2
# HELP darbotdb_search_num_files Number of files
# TYPE darbotdb_search_num_files gauge
darbotdb_search_num_files{{db="testVocbase",view="h3039/42",collection="{0}",index_id="{1}",shard=""}}11
# HELP darbotdb_search_index_size Size of the index in bytes
# TYPE darbotdb_search_index_size gauge
darbotdb_search_index_size{{db="testVocbase",view="h3039/42",collection="{0}",index_id="{1}",shard=""}}1513
)"
    , cid, _link->id().id());  // clang-format on
    std::string actual;
    getPrometheusStr(actual);
...
# HELP darbotdb_search_num_live_docs Number of live documents
# TYPE darbotdb_search_num_live_docs gauge
darbotdb_search_num_live_docs{{db="testVocbase",view="h3039/42",collection="{0}",index_id="{1}",shard=""}}2
# HELP darbotdb_search_num_primary_docs Number of primary documents
# TYPE darbotdb_search_num_primary_docs gauge
darbotdb_search_num_primary_docs{{db="testVocbase",view="h3039/42",collection="{0}",index_id="{1}",shard=""}}3
# HELP darbotdb_search_num_segments Number of segments
...
# HELP darbotdb_search_num_files Number of files
# TYPE darbotdb_search_num_files gauge
darbotdb_search_num_files{{db="testVocbase",view="h3039/42",collection="{0}",index_id="{1}",shard=""}}12
# HELP darbotdb_search_index_size Size of the index in bytes
# TYPE darbotdb_search_index_size gauge
darbotdb_search_index_size{{db="testVocbase",view="h3039/42",collection="{0}",index_id="{1}",shard=""}}1561
)"
    , cid, _link->id().id());  // clang-format on
    std::string actual;
    getPrometheusStr(actual);
...
R"(# HELP darbotdb_search_num_docs Number of documents
# TYPE darbotdb_search_num_docs gauge
darbotdb_search_num_docs{{db="testVocbase",view="h3039/42",collection="{0}",index_id="{1}",shard=""}}1
# HELP darbotdb_search_num_live_docs Number of live documents
# TYPE darbotdb_search_num_live_docs gauge
darbotdb_search_num_live_docs{{db="testVocbase",view="h3039/42",collection="{0}",index_id="{1}",shard=""}}1
# HELP darbotdb_search_num_primary_docs Number of primary documents
# TYPE darbotdb_search_num_primary_docs gauge
darbotdb_search_num_primary_docs{{db="testVocbase",view="h3039/42",collection="{0}",index_id="{1}",shard=""}}1
# HELP darbotdb_search_num_segments Number of segments
# TYPE darbotdb_search_num_segments gauge
darbotdb_search_num_segments{{db="testVocbase",view="h3039/42",collection="{0}",index_id="{1}",shard=""}}1
# HELP darbotdb_search_num_files Number of files
# TYPE darbotdb_search_num_files gauge
darbotdb_search_num_files{{db="testVocbase",view="h3039/42",collection="{0}",index_id="{1}",shard=""}}6
# HELP darbotdb_search_index_size Size of the index in bytes
# TYPE darbotdb_search_index_size gauge
darbotdb_search_index_size{{db="testVocbase",view="h3039/42",collection="{0}",index_id="{1}",shard=""}}681
)"
    , cid, _link->id().id());  // clang-format on
    std::string actual;
    getPrometheusStr(actual);
...
class IResearchLinkInRecoveryDBServerOnUpgradeTest
 ...
 protected:
  ...
  IResearchLinkInRecoveryDBServerOnUpgradeTest()
    ...
    *consolidationThreads->ptr = 1;
    ars.validateOptions(opts);
    server.getFeature<darbotdb::ClusterFeature>().disable();
    server.startFeatures();
    EXPECT_EQ((std::pair<size_t, size_t>{1, 1}),
              ars.limits(darbotdb::iresearch::ThreadGroup::_0));
    EXPECT_EQ((std::pair<size_t, size_t>{1, 1}),
  ...
  ~IResearchLinkInRecoveryDBServerOnUpgradeTest() override {
    StorageEngineMock::recoveryStateResult = darbotdb::RecoveryState::DONE;
    TRI_RemoveDirectory(testFilesystemPath.c_str());
  }
};
...
TEST_F(IResearchLinkInRecoveryDBServerOnUpgradeTest,
       test_init_in_recovery_no_name_includeAll) {
  {
    ...
    EXPECT_THROW(
        StorageEngineMock::buildLinkMock(darbotdb::IndexId{1},
