...
TEST_F(IResearchPrimaryKeyReuse, test_multiple_transactions_sequential) {
  ...
  darbotdb::LogicalView* view;
  std::shared_ptr<darbotdb::LogicalCollection> collection;
  ...
  // create view
  {
    ...
    EXPECT_EQ(TRI_ERROR_NO_ERROR, darbotdb::tests::executeQuery(
                                      vocbase,
    ...
    // reinsert with _rev and isRestore == true
    {
      ...
      for (auto& entry : insertedDocs) {
        ...
        EXPECT_EQ(
            res.slice()
                .get("new")
                .get(darbotdb::StaticStrings::RevString)
                .copyString(),
            entry.slice().get(darbotdb::StaticStrings::RevString).copyString());
      }
...
TEST_F(IResearchPrimaryKeyReuse, test_multiple_transactions_interleaved) {
  TRI_vocbase_t vocbase(testDBInfo(server.server()));
  std::vector<darbotdb::velocypack::Builder> insertedDocs;
  std::vector<darbotdb::velocypack::Builder> extraDocs;
  darbotdb::LogicalView* view;
  std::shared_ptr<darbotdb::LogicalCollection> collection;
