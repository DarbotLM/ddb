...
using darbotdb::geo::json::parseCoordinates;
...
TEST_F(IResearchFilterGeoFunctionsTest, GeoIntersects) {
  {
    ...
    irs::Or expected;
    auto& filter = expected.add<darbotdb::iresearch::GeoFilter>();
    *filter.mutable_field() = mangleString("name", "mygeojson");
    auto* opts = filter.mutable_options();
    opts->type = darbotdb::iresearch::GeoFilterType::INTERSECTS;
    opts->prefix = "";
...

  {
    ...
    *filter.mutable_field() = mangleString("name", "mygeojson");
    ...
    ctx.vars.emplace(
        "lat", darbotdb::aql::AqlValue(darbotdb::aql::AqlValueHintInt{2}));
    ctx.vars.emplace(
        "lng", darbotdb::aql::AqlValue(darbotdb::aql::AqlValueHintInt{1}));
...
TEST_F(IResearchFilterGeoFunctionsTest, GeoContains) {
  {
    ...
    *filter.mutable_field() = mangleString("name", "mygeojson");
    auto* opts = filter.mutable_options();
    opts->type = darbotdb::iresearch::GeoFilterType::IS_CONTAINED;
    opts->prefix = "";
    ASSERT_TRUE(parseCoordinates<true>(json->slice(), opts->shape, true).ok());
...

  {
    ...
    *filter.mutable_field() = mangleString("name", "mygeojson");
    auto* opts = filter.mutable_options();
    opts->type = darbotdb::iresearch::GeoFilterType::CONTAINS;
    opts->prefix = "";
    ASSERT_TRUE(parseCoordinates<true>(json->slice(), opts->shape, true).ok());
...
TEST_F(IResearchFilterGeoFunctionsTest, GeoDistance) {
  {
    irs::Or expected;
    auto& filter = expected.add<darbotdb::iresearch::GeoDistanceFilter>();
    *filter.mutable_field() = mangleString("name", "mygeojson");
    auto* opts = filter.mutable_options();
    ...
    ExpressionContextMock ctx;
    ctx.vars.emplace(
        "lat", darbotdb::aql::AqlValue(darbotdb::aql::AqlValueHintInt{0}));
    ctx.vars.emplace(
        "lng", darbotdb::aql::AqlValue(darbotdb::aql::AqlValueHintInt{0}));
    ctx.vars.emplace(
        "dist", darbotdb::aql::AqlValue(darbotdb::aql::AqlValueHintInt{5000}));
...

  {
    ...
    auto& filter = expected.add<irs::And>()
                       .add<irs::Not>()
                       .filter<darbotdb::iresearch::GeoDistanceFilter>();
    *filter.mutable_field() = mangleString("name", "mygeojson");
    auto* opts = filter.mutable_options();
    opts->origin = S2LatLng::FromDegrees(0., 0.).Normalized().ToPoint();
    opts->range.min = 5000;
...

  {
    ...
    {
      auto& filter = andGroup.add<darbotdb::iresearch::GeoDistanceFilter>();
      *filter.mutable_field() = mangleString("name", "mygeojson");
      auto* opts = filter.mutable_options();
      opts->origin = S2LatLng::FromDegrees(0., 0.).Normalized().ToPoint();
      opts->range.min = 5000;
