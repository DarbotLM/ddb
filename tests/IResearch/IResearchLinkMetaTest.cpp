 ...
 public:
  ...
  static bool normalize(std::string_view args, std::string& out) {
    ...
    if (slice.isString()) {
      ...
      darbotdb::iresearch::addStringRef(
          builder, "args",
          darbotdb::iresearch::getStringRef(slice.get("args")));
    } else {
      return false;
    }
...
class IResearchLinkMetaTest
    : public ::testing::Test,
      public darbotdb::tests::LogSuppressor<darbotdb::Logger::AGENCYCOMM,
...
TEST_F(IResearchLinkMetaTest, test_defaults) {
  darbotdb::iresearch::IResearchLinkMeta meta;
  ...
  EXPECT_TRUE(darbotdb::iresearch::ValueStorage::NONE == meta._storeValues);
...
TEST_F(IResearchLinkMetaTest, test_readDefaults) {
  ...
  // without active vobcase
  {
    ...
    EXPECT_FALSE(darbotdb::iresearch::hasHotFields(meta));
#endif
    EXPECT_EQ(darbotdb::iresearch::ValueStorage::NONE, meta._storeValues);
...
TEST_F(IResearchLinkMetaTest, test_readCustomizedValues) {
  ...
  // with active vocbase
  {
    ...
    for (auto& field : meta._fields) {
      ...
      for (auto& fieldOverride : field.second._fields) {
        ...
        if ("default" == fieldOverride.first) {
          ...
          EXPECT_EQ(darbotdb::iresearch::ValueStorage::NONE,
                    actual._storeValues);
          ...
          EXPECT_TRUE((darbotdb::iresearch::Features(
          ...
          EXPECT_EQ(darbotdb::iresearch::ValueStorage::VALUE,
                    actual._storeValues);
          EXPECT_EQ(1U, actual._analyzers.size());
          EXPECT_TRUE(*(actual._analyzers.begin()));
          ...
          EXPECT_TRUE(actual._trackListPositions);
          EXPECT_EQ(darbotdb::iresearch::ValueStorage::ID, actual._storeValues);
    ...
    EXPECT_EQ(darbotdb::iresearch::ValueStorage::VALUE, meta._storeValues);
...
TEST_F(IResearchLinkMetaTest, test_writeDefaults) {
  // without active vobcase (not fullAnalyzerDefinition)
  {
    darbotdb::iresearch::IResearchLinkMeta meta;
    darbotdb::velocypack::Builder builder;
    darbotdb::velocypack::Slice tmpSlice;
...
TEST_F(IResearchLinkMetaTest, test_writeCustomizedValues) {
  ...
  darbotdb::iresearch::IResearchAnalyzerFeature::EmplaceResult emplaceResult;
  darbotdb::iresearch::IResearchLinkMeta meta;

  analyzers.emplace(
      emplaceResult, darbotdb::StaticStrings::SystemDatabase + "::empty",
      "empty", VPackParser::fromJson("{ \"args\": \"en\" }")->slice(),
      darbotdb::transaction::OperationOriginTestCase{},
      {{}, irs::IndexFeatures::FREQ});

  auto identity =
      analyzers.get("identity", darbotdb::QueryAnalyzerRevisions::QUERY_LATEST,
                    darbotdb::transaction::OperationOriginTestCase{});
  ...
  meta._storeValues = darbotdb::iresearch::ValueStorage::VALUE;
  ...
  meta._sort.emplace_back(
      {darbotdb::basics::AttributeName(std::string_view("_key"), false)}, true);
  meta._sort.emplace_back(
      {darbotdb::basics::AttributeName(std::string_view("_id"), false)}, false);
  ...
  overrideAll._storeValues = darbotdb::iresearch::ValueStorage::NONE;
  overrideAll._analyzers.clear();
  overrideAll._analyzers.emplace_back(
      darbotdb::iresearch::IResearchLinkMeta::Analyzer(
          analyzers.get(darbotdb::StaticStrings::SystemDatabase + "::empty",
                        darbotdb::QueryAnalyzerRevisions::QUERY_LATEST,
                        darbotdb::transaction::OperationOriginTestCase{}),
          "empty"));
  ...
  overrideSome._storeValues = darbotdb::iresearch::ValueStorage::ID;
  // do not inherit fields to match jSon inheritance
  ...
  // without active vobcase (not fullAnalyzerDefinition)
  {
    ...
    std::unordered_set<std::string> expectedAnalyzers = {
        darbotdb::StaticStrings::SystemDatabase + "::empty", "identity"};
    darbotdb::velocypack::Builder builder;
    darbotdb::velocypack::Slice tmpSlice;
    ...
    for (darbotdb::velocypack::ObjectIterator itr(tmpSlice); itr.valid();
         ++itr) {
      auto key = itr.key();
      ...
      for (darbotdb::velocypack::ObjectIterator overrideItr(tmpSlice);
        ...
        if ("default" == fieldOverride.stringView()) {
          ...
          for (darbotdb::velocypack::ObjectIterator overrideFieldItr(tmpSlice);
          ...
          EXPECT_TRUE((true == tmpSlice.isArray() && 1 == tmpSlice.length() &&
                       tmpSlice.at(0).isString() &&
                       std::string(darbotdb::StaticStrings::SystemDatabase +
    ...
    for (darbotdb::velocypack::ArrayIterator analyzersItr(tmpSlice);
  ...
  // without active vobcase (with fullAnalyzerDefinition)
  {
    ...
    std::set<std::pair<std::string, std::string>> expectedAnalyzerDefinitions =
        {
            {darbotdb::StaticStrings::SystemDatabase + "::empty",
    ...
    EXPECT_EQ((std::vector<darbotdb::basics::AttributeName>{
...
TEST_F(IResearchLinkMetaTest, test_readMaskAll) {
  darbotdb::iresearch::IResearchLinkMeta meta;
  darbotdb::iresearch::IResearchLinkMeta::Mask mask;
  std::string tmpString;
  ...
  EXPECT_TRUE(meta.init(server.server(), json->slice(), tmpString, {},
                        darbotdb::iresearch::LinkVersion::MIN, &mask));
...
TEST_F(IResearchLinkMetaTest, test_writeMaskAll) {
  ...
  {
    darbotdb::iresearch::IResearchLinkMeta meta;
    darbotdb::iresearch::IResearchLinkMeta::Mask mask(true);
    darbotdb::velocypack::Builder builder;
...
TEST_F(IResearchLinkMetaTest, test_writeMaskNone) {
  ...
  {
    darbotdb::iresearch::IResearchLinkMeta meta;
    darbotdb::iresearch::IResearchLinkMeta::Mask mask(false);
    darbotdb::velocypack::Builder builder;
...
TEST_F(IResearchLinkMetaTest, test_readAnalyzerDefinitions) {
  ...
  // complex definition on single-server
  {
    ...
    EXPECT_EQ(1, meta._analyzers.size());
    {
      auto identity = darbotdb::iresearch::IResearchAnalyzerFeature::identity();
  ...
  {
    auto before = darbotdb::ServerState::instance()->getRole();
    darbotdb::ServerState::instance()->setRole(
        darbotdb::ServerState::ROLE_COORDINATOR);
    irs::Finally restore = [&before]() noexcept {
      darbotdb::ServerState::instance()->setRole(before);
    };
  ...
  {
    auto lhs = darbotdb::velocypack::Parser::fromJson(
    ...
    darbotdb::iresearch::IResearchLinkMeta meta2;
    ASSERT_TRUE(
        meta2.init(server.server(), lhs->slice(), errorField, vocbase.name()));
    ASSERT_EQ(meta, meta2);
    darbotdb::iresearch::IResearchLinkMeta meta_assigned;
    meta_assigned = meta;
    ASSERT_EQ(meta, meta_assigned);
    darbotdb::iresearch::IResearchLinkMeta meta_copied(meta);
    ASSERT_EQ(meta_copied, meta);
  }
}

// https://github.com/darbotdb/backlog/issues/581
...
TEST_F(IResearchLinkMetaTest, test_addNonUniqueAnalyzers) {
  ...
  const std::string analyzerCustomInSystem =
      darbotdb::StaticStrings::SystemDatabase + "::" + analyzerCustomName;
...

  {
    ...
    EXPECT_TRUE(meta.init(server.server(), json->slice(), errorField,
                          vocbase.name(), darbotdb::iresearch::LinkVersion::MIN,
                          &mask));
    EXPECT_TRUE(mask._analyzers);
...
// https://darbotdb.atlassian.net/browse/ES-700
...
class IResearchLinkMetaTestNoSystem
 ...
 protected:
  ...
  IResearchLinkMetaTestNoSystem() {
    ...
    server.getFeature<darbotdb::SystemDatabaseFeature>().unprepare();
  }
};
...
TEST_F(IResearchLinkMetaTest, test_collectioNameComparison) {
  darbotdb::iresearch::IResearchLinkMeta meta;
  darbotdb::iresearch::IResearchLinkMeta meta1;
...
void makeCachedColumnsTest(std::vector<irs::field_meta> const& mockedFields,
                           darbotdb::iresearch::IResearchLinkMeta const& meta,
                           std::set<irs::field_id> expected,
                           darbotdb::containers::FlatHashSet<std::string> const*
  ...
  darbotdb::containers::FlatHashSet<std::string> geoColumns;
  darbotdb::iresearch::collectCachedColumns(actual, geoColumns,
...
TEST_F(IResearchLinkMetaTest, test_cachedColumns) {
  ...
  ASSERT_TRUE(darbotdb::iresearch::hasHotFields(meta));
}
