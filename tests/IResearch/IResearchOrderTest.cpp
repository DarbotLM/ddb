...
void assertOrder(
    darbotdb::ArangodServer& server, bool parseOk, bool execOk,
  ...
  darbotdb::aql::AstNode* orderNode = nullptr;
  for (size_t i = 0; i < root->numMembers(); ++i) {
    auto* node = root->getMemberUnchecked(i);
    ASSERT_TRUE(node);

    if (darbotdb::aql::NODE_TYPE_SORT == node->type) {
      orderNode = node;
      break;
    }
  }
  ASSERT_TRUE((orderNode && darbotdb::aql::NODE_TYPE_SORT == orderNode->type));
  ...
  // optimization time check
  {
    ...
    for (size_t i = 0, count = sortNode->numMembers(); i < count; ++i) {
      ...
      EXPECT_TRUE(parseOk == darbotdb::iresearch::order_factory::scorer(
                                 nullptr, *expr, ctx));
    }
  }
  ...
  // execution time check
  {
    ...
    darbotdb::iresearch::QueryContext const ctx{&trx, ast, exprCtx,
                                                &irs::SubReader::empty(), ref};

    for (size_t i = 0, count = sortNode->numMembers(); i < count; ++i) {
      auto const* sort = sortNode->getMember(i);
      auto const* expr = sort->getMember(0);
      EXPECT_TRUE(execOk == darbotdb::iresearch::order_factory::scorer(
...
void assertOrderSuccess(
    darbotdb::ArangodServer& server, std::string const& queryString,
...
void assertOrderParseFail(darbotdb::ArangodServer& server,
...
class IResearchOrderTest
 ...
 protected:
  ...
  IResearchOrderTest()
    ...
    features.emplace_back(
        server.addFeature<darbotdb::aql::AqlFunctionFeature>(), true);
  ...
  ~IResearchOrderTest() {
    darbotdb::aql::AqlFunctionFeature(server)
...
TEST_F(IResearchOrderTest, test_FCall) {
  ...
  // undefined function (not a function registered with darbotdb)
  {
    std::string query =
...
TEST_F(IResearchOrderTest, test_FCall_tfidf) {
  ...
  // reference as an argument
  {
    ...
    ctx.vars.emplace("withNorms", darbotdb::aql::AqlValue(
  ...
  // deterministic expression as an argument
  {
    ...
    ExpressionContextMock ctx;
    ctx.vars.emplace(
        "x", darbotdb::aql::AqlValue(darbotdb::aql::AqlValueHintInt{5}));
  ...
  {
    auto obj = darbotdb::velocypack::Parser::fromJson("{}");

    ExpressionContextMock ctx;
    ctx.vars.emplace("c", darbotdb::aql::AqlValue(obj->slice()));
...
TEST_F(IResearchOrderTest, test_FCall_bm25) {
  ...
  // reference as k coefficient
  {
    ExpressionContextMock ctx;
    ctx.vars.emplace(
        "k", darbotdb::aql::AqlValue(darbotdb::aql::AqlValueHintDouble{0.99}));
  ...
  // deterministic expression as k coefficient
  {
    ExpressionContextMock ctx;
    ctx.vars.emplace(
        "x", darbotdb::aql::AqlValue(darbotdb::aql::AqlValueHintDouble{0.97}));
  ...
  // reference as k,b coefficients
  {
    ExpressionContextMock ctx;
    ctx.vars.emplace(
        "k", darbotdb::aql::AqlValue(darbotdb::aql::AqlValueHintDouble{0.97}));
    ctx.vars.emplace(
        "b", darbotdb::aql::AqlValue(darbotdb::aql::AqlValueHintDouble{1.2}));
  ...
  // deterministic expressions as k,b coefficients
  {
    ...
    ctx.vars.emplace(
        "x", darbotdb::aql::AqlValue(darbotdb::aql::AqlValueHintDouble{0.97}));
    ctx.vars.emplace(
        "y", darbotdb::aql::AqlValue(darbotdb::aql::AqlValueHintDouble{0.1}));
...
TEST_F(IResearchOrderTest, test_order) {
  ...
  // invalid field
  {
    ExpressionContextMock ctx;
    ctx.vars.emplace(
        "a", darbotdb::aql::AqlValue(darbotdb::aql::AqlValueHintInt{5}));
    std::string query =
