...
class IResearchViewCoordinatorTest : public ::testing::Test {
 protected:
  ...
  darbotdb::ResourceMonitor resourceMonitor{global};
...
TEST_F(IResearchViewCoordinatorTest, test_type) {
  static_assert(darbotdb::iresearch::IResearchViewCoordinator::typeInfo() ==
                std::pair{darbotdb::ViewType::kArangoSearch,
...
TEST_F(IResearchViewCoordinatorTest, test_rename) {
  ...
  darbotdb::LogicalView::ptr view;
  ASSERT_TRUE(
      (darbotdb::LogicalView::instantiate(view, vocbase, json->slice(), false)
           .ok()));
  ASSERT_TRUE(nullptr != view);
  ASSERT_TRUE(
      nullptr !=
      std::dynamic_pointer_cast<darbotdb::iresearch::IResearchViewCoordinator>(
          view));
  EXPECT_TRUE("testView" == view->name());
  EXPECT_TRUE(false == view->deleted());
  EXPECT_TRUE(1 == view->id().id());
  EXPECT_TRUE(darbotdb::ViewType::kArangoSearch == view->type());
  EXPECT_TRUE(darbotdb::LogicalDataSource::Category::kView == view->category());
...
TEST_F(IResearchViewCoordinatorTest, visit_collections) {
  ...
  std::string viewId("1");
  auto collectionJson0 = darbotdb::velocypack::Parser::fromJson(
      "{ \"name\": \"testCollection0\", \"shards\":{} }");
  auto collectionJson1 = darbotdb::velocypack::Parser::fromJson(
      "{ \"name\": \"testCollection1\", \"shards\":{} }");
  auto collectionJson2 = darbotdb::velocypack::Parser::fromJson(
      "{ \"name\": \"testCollection2\", \"shards\":{} }");
  auto linkJson = darbotdb::velocypack::Parser::fromJson("{ \"view\": \"1\" }");
  auto json = darbotdb::velocypack::Parser::fromJson(
      "{ \"name\": \"testView\", \"type\": \"arangosearch\", \"id\": \"1\" }");
  ...
  auto* view = dynamic_cast<darbotdb::iresearch::IResearchViewCoordinator*>(
      logicalView.get());
  ...
  EXPECT_NE(nullptr, factory.instantiate(*logicalCollection0, linkJson->slice(),
                                         darbotdb::IndexId{1}, false));
  EXPECT_NE(nullptr, factory.instantiate(*logicalCollection1, linkJson->slice(),
                                         darbotdb::IndexId{2}, false));
  EXPECT_NE(nullptr, factory.instantiate(*logicalCollection2, linkJson->slice(),
                                         darbotdb::IndexId{3}, false));

  // visit view
  darbotdb::DataSourceId expectedCollections[] = {darbotdb::DataSourceId{1},
                                                  darbotdb::DataSourceId{2},
                                                  darbotdb::DataSourceId{3}};
  auto* begin = expectedCollections;
  EXPECT_TRUE(true ==
              view->visitCollections([&begin](darbotdb::DataSourceId cid,
                                              darbotdb::LogicalView::Indexes*) {
                *begin++ = cid;
                return true;
              }));
...
TEST_F(IResearchViewCoordinatorTest, test_defaults) {
  ...
  // view definition with LogicalView (for persistence)
  {
    ...
    EXPECT_TRUE(
        (nullptr != std::dynamic_pointer_cast<
                        darbotdb::iresearch::IResearchViewCoordinator>(view)));
    EXPECT_TRUE(("testView" == view->name()));
    EXPECT_TRUE((false == view->deleted()));
    EXPECT_TRUE((1 == view->id().id()));
    EXPECT_TRUE((darbotdb::ViewType::kArangoSearch == view->type()));
    EXPECT_TRUE(
        (darbotdb::LogicalDataSource::Category::kView == view->category()));
    EXPECT_TRUE((&vocbase == &view->vocbase()));

    // visit default view
    EXPECT_TRUE(
        (true == view->visitCollections(
                     [](darbotdb::DataSourceId,
                        darbotdb::LogicalView::Indexes*) { return false; })));

    // for persistence
    {
      darbotdb::iresearch::IResearchViewMeta expectedMeta;
      ...
      darbotdb::iresearch::IResearchViewMeta meta;
      std::string error;
    ...
    // properties
    {
      ...
      EXPECT_TRUE(
          view->properties(
                  builder,
                  darbotdb::LogicalDataSource::Serialization::Properties)
              .ok());
      builder.close();
      auto slice = builder.slice();
      darbotdb::iresearch::IResearchViewMeta meta;
    ...
    // list
    {
      ...
      auto res = view->properties(
          builder, darbotdb::LogicalDataSource::Serialization::List);
      ASSERT_TRUE(res.ok());
      builder.close();
      auto slice = builder.slice();
      darbotdb::iresearch::IResearchViewMeta meta;
  ...
  {
    auto viewCreateJson = darbotdb::velocypack::Parser::fromJson(
        "{ \"name\": \"testView\", \"id\": \"42\", \"type\": "
        "\"arangosearch\", \"links\": { \"testCollection\": {} } }");
    auto viewId = "testView";

    darbotdb::LogicalView::ptr logicalView;
    auto res = darbotdb::iresearch::IResearchViewCoordinator::factory().create(
        logicalView, *vocbase, viewCreateJson->slice(), true);
    EXPECT_TRUE((TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND == res.errorNumber()));
  ...
  // new view definition with links with invalid definition
  {
    ...
    auto dropLogicalCollection = std::shared_ptr<darbotdb::ClusterInfo>(
        &ci, [vocbase, &collectionId](darbotdb::ClusterInfo* ci) -> void {
  ...
  // new view definition with links
  {
    ...
    darbotdb::LogicalView::ptr logicalView;
    EXPECT_TRUE(
        (darbotdb::iresearch::IResearchViewCoordinator::factory()
...
TEST_F(IResearchViewCoordinatorTest, test_create_drop_view) {
  ...
  // create and drop view (no id specified)
  {
    ...
    auto planVersion = darbotdb::tests::getCurrentPlanVersion(server.server());
    ...
    EXPECT_TRUE(darbotdb::ViewType::kArangoSearch == view->type());
    EXPECT_TRUE(darbotdb::LogicalDataSource::Category::kView ==
                view->category());
    EXPECT_TRUE(vocbase == &view->vocbase());

    // create duplicate view
    EXPECT_TRUE(
        (TRI_ERROR_DDB_DUPLICATE_NAME ==
         ci.createViewCoordinator(vocbase->name(), viewId, json->slice())
             .errorNumber()));
    EXPECT_TRUE(planVersion ==
                darbotdb::tests::getCurrentPlanVersion(server.server()));
...
TEST_F(IResearchViewCoordinatorTest, test_create_link_in_background) {
  ...
  auto viewCreateJson = darbotdb::velocypack::Parser::fromJson(
      "{ \"name\": \"testView\", \"id\": \"42\", \"type\": \"arangosearch\" "
      "}");
  auto viewUpdateJson = darbotdb::velocypack::Parser::fromJson(
  ...
  // check agency record
  {
    ...
    server.getFeature<darbotdb::ClusterFeature>().agencyCache().store().read(
  ...
  // check index definition in collection
  {
    ...
    ASSERT_EQ(darbotdb::Index::TRI_IDX_TYPE_IRESEARCH_LINK, index->type());
    VPackBuilder builder;
    index->toVelocyPack(builder, darbotdb::Index::makeFlags(
                                     darbotdb::Index::Serialize::Internals));
...
TEST_F(IResearchViewCoordinatorTest, test_update_properties) {
  ...
  // create view
  {
    ...
    // check default properties
    {
      ...
      EXPECT_TRUE(meta == darbotdb::iresearch::IResearchViewMeta::DEFAULT());
    }
    ...
    {
      auto props = darbotdb::velocypack::Parser::fromJson(
          "{ \"cleanupIntervalStep\": 42, \"consolidationIntervalMsec\": 50 }");
      EXPECT_TRUE(view->properties(props->slice(), true, false).ok());
      EXPECT_TRUE(planVersion < darbotdb::tests::getCurrentPlanVersion(
                                    server.server()));  // plan version changed
      planVersion = darbotdb::tests::getCurrentPlanVersion(server.server());
      ...
      ASSERT_TRUE(nullptr != std::dynamic_pointer_cast<
                                 darbotdb::iresearch::IResearchViewCoordinator>(
                                 fullyUpdatedView));
      ...
      EXPECT_TRUE(darbotdb::ViewType::kArangoSearch ==
      ...
      // check recently updated properties
      {
        ...
        darbotdb::iresearch::IResearchViewMeta meta;
        darbotdb::iresearch::IResearchViewMeta expected;
...
TEST_F(IResearchViewCoordinatorTest, test_properties_user_request) {
  ...
  // check serialization for inventory
  {
    ...
    builder.openObject();
    auto res = logicalView->properties(
        builder, darbotdb::LogicalDataSource::Serialization::Inventory);
    ASSERT_TRUE(res.ok());
    builder.close();
...
TEST_F(IResearchViewCoordinatorTest, test_overwrite_immutable_properties) {
  ...
  EXPECT_EQ(darbotdb::ViewType::kArangoSearch, view->type());
  EXPECT_EQ(darbotdb::LogicalDataSource::Category::kView, view->category());
  ...
  {
    auto newProperties = darbotdb::velocypack::Parser::fromJson(
        "{"
    ...
    EXPECT_EQ(planVersion,
              darbotdb::tests::getCurrentPlanVersion(
...

  {
    ...
    // update properties
    EXPECT_TRUE(view->properties(newProperties->slice(), true, false).ok());
    EXPECT_LT(planVersion, darbotdb::tests::getCurrentPlanVersion(
                               server.server()));  // plan version changed
    planVersion = darbotdb::tests::getCurrentPlanVersion(server.server());
    ...
    EXPECT_EQ(darbotdb::ViewType::kArangoSearch, fullyUpdatedView->type());
...
TEST_F(IResearchViewCoordinatorTest, test_update_links_partial_remove) {
  ...
  std::shared_ptr<darbotdb::LogicalCollection> logicalCollection3;
  {
  ...
  darbotdb::LogicalView::ptr view;
  ASSERT_TRUE(
      darbotdb::LogicalView::create(view, *vocbase, viewJson->slice(), false)
          .ok());
  ASSERT_TRUE(view);
  ...
  auto linksJson = darbotdb::velocypack::Parser::fromJson(
      "{ \"links\": {"
  ...
  // visit collections
  {
    std::set<darbotdb::DataSourceId> expectedLinks{logicalCollection1->id(),
                                                   logicalCollection2->id(),
                                                   logicalCollection3->id()};
    EXPECT_TRUE(view->visitCollections(
        [&expectedLinks](darbotdb::DataSourceId cid,
                         darbotdb::LogicalView::Indexes*) mutable {
  ...
  // check properties
  {
    ...
    EXPECT_TRUE(
        properties.hasKey(darbotdb::iresearch::StaticStrings::LinksField));
    auto const linksSlice =
        properties.get(darbotdb::iresearch::StaticStrings::LinksField);
  ...
  auto const updateJson = darbotdb::velocypack::Parser::fromJson(
      "{ \"links\": {"
      "  \"2\" : null "
      "} }");
...
TEST_F(IResearchViewCoordinatorTest, test_update_links_replace) {
  ...
  // visit collections
  {
    std::set<darbotdb::DataSourceId> expectedLinks{logicalCollection2->id()};
    EXPECT_TRUE(view->visitCollections(
        [&expectedLinks](darbotdb::DataSourceId cid,
                         darbotdb::LogicalView::Indexes*) mutable {
          return 1 == expectedLinks.erase(cid);
  ...
  updateJson = darbotdb::velocypack::Parser::fromJson(
      "{ \"links\": {"
      "  \"testCollection1\" : { \"id\": \"1\", \"includeAllFields\" : true "
      "}, "
      "  \"2\" : null "
...
TEST_F(IResearchViewCoordinatorTest, test_update_links_clear) {
  ...
  auto const updateJson =
      darbotdb::velocypack::Parser::fromJson("{ \"links\": {} }");
  EXPECT_TRUE(view->properties(updateJson->slice(), true, false).ok());
  EXPECT_TRUE(planVersion < darbotdb::tests::getCurrentPlanVersion(
                                server.server()));  // plan version changed
  planVersion = darbotdb::tests::getCurrentPlanVersion(server.server());
  ...
  // visit collections
  {
    EXPECT_TRUE(view->visitCollections(
        [](darbotdb::DataSourceId cid,
           darbotdb::LogicalView::Indexes*) mutable { return false; }));
  }
  ...
  // check properties
  {
    ...
    EXPECT_TRUE(
        (properties.hasKey(darbotdb::iresearch::StaticStrings::LinksField)));
    auto links = properties.get(darbotdb::iresearch::StaticStrings::LinksField);
...
TEST_F(IResearchViewCoordinatorTest, test_drop_link) {
  ...
  // update link
  {
    ...
    auto view = std::dynamic_pointer_cast<
        darbotdb::iresearch::IResearchViewCoordinator>(logicalView);
    ASSERT_TRUE(view);
    auto const viewId = std::to_string(view->planId().id());
    EXPECT_TRUE("42" == viewId);
    ...
    // visit collections
    {
      std::set<darbotdb::DataSourceId> expectedLinks{logicalCollection->id()};
      EXPECT_TRUE(view->visitCollections(
          [&expectedLinks](darbotdb::DataSourceId cid,
                           darbotdb::LogicalView::Indexes*) mutable {
            return 1 == expectedLinks.erase(cid);
    ...
    // check properties
    {
      ...
      EXPECT_TRUE(
          view->properties(
                  info, darbotdb::LogicalDataSource::Serialization::Properties)
              .ok());
      info.close();
    ...
    // drop link
    EXPECT_TRUE((darbotdb::methods::Indexes::drop(
                     *logicalCollection, darbotdb::velocypack::Parser::fromJson(
    ...
    // visit collections
    EXPECT_TRUE(view->visitCollections(
        [](darbotdb::DataSourceId, darbotdb::LogicalView::Indexes*) {
          return false;
        }));
...
TEST_F(IResearchViewCoordinatorTest, test_update_overwrite) {
  ...
  // modify meta params with links to missing collections
  {
    ...
    auto logicalView = ci.getView(vocbase->name(), viewId);
    ASSERT_TRUE((false == !logicalView));
    auto dropLogicalView = std::shared_ptr<darbotdb::ClusterInfo>(
        &ci, [vocbase, &viewId](darbotdb::ClusterInfo* ci) -> void {
    ...
    EXPECT_EQ(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND,
              logicalView->properties(viewUpdateJson->slice(), true, false)
                  .errorNumber());
    logicalView = ci.getView(vocbase->name(), viewId);
    ASSERT_TRUE((false == !logicalView));
  ...
  {
    auto collection0Json = darbotdb::velocypack::Parser::fromJson(
        "{ \"id\": \"1\", \"planId\": \"1\",  \"name\": \"testCollection0\", "
        "\"replicationFactor\": 1, \"type\": 1, \"shards\":{} }");
    auto collection1Json = darbotdb::velocypack::Parser::fromJson(
    ...
    auto logicalCollection0 = ci.getCollection(vocbase->name(), collectionId0);
    ASSERT_TRUE((false == !logicalCollection0));
    auto dropLogicalCollection0 = std::shared_ptr<darbotdb::ClusterInfo>(
        &ci, [vocbase, &collectionId0](darbotdb::ClusterInfo* ci) -> void {
    ...
    auto logicalCollection1 = ci.getCollection(vocbase->name(), collectionId1);
    ASSERT_TRUE((false == !logicalCollection1));
    auto dropLogicalCollection1 = std::shared_ptr<darbotdb::ClusterInfo>(
        &ci, [vocbase, &collectionId1](darbotdb::ClusterInfo* ci) -> void {
    ...
    // initial link creation
    {
      // simulate heartbeat thread (create index in current)
      {
        ...
        auto const path1 = "/Current/Collections/" + vocbase->name() + "/" +
                           std::to_string(logicalCollection1->id().id());
        auto const value0 = darbotdb::velocypack::Parser::fromJson(
            "{ \"s12345\": { \"indexes\" : [ { \"id\": \"3\" } ] } }");
        auto const value1 = darbotdb::velocypack::Parser::fromJson(
            "{ \"s12345\": { \"indexes\" : [ { \"id\": \"4\" } ] } }");
        EXPECT_TRUE(darbotdb::AgencyComm(server.server())
...
TEST_F(IResearchViewCoordinatorTest, IResearchViewNode_createBlock) {
  ...
  // create and drop view (no id specified)
  {
    ...
    auto query = darbotdb::aql::Query::create(
        ...
        darbotdb::aql::QueryString(std::string_view("RETURN 1")), nullptr);
    darbotdb::tests::waitForAsync(query->prepareQuery());

    darbotdb::aql::SingletonNode singleton(query->plan(),
                                           darbotdb::aql::ExecutionNodeId{0});

    darbotdb::aql::Variable const outVariable("variable", 0, false,
                                              resourceMonitor);

    darbotdb::iresearch::IResearchViewNode node(
        *query->plan(), darbotdb::aql::ExecutionNodeId{42},
        *vocbase,  // database
        view,      // view
        outVariable,
    ...
    singleton.setVarsUsedLater({darbotdb::aql::VarSet{&outVariable}});
    singleton.setVarsValid({{}});
    node.setVarsUsedLater({{}});
    node.setVarsValid({darbotdb::aql::VarSet{&outVariable}});
    ...
    ASSERT_TRUE(nullptr !=
                dynamic_cast<darbotdb::aql::ExecutionBlockImpl<
                    darbotdb::aql::NoResultsExecutor>*>(execBlock.get()));
