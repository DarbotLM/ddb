 ...
 protected:
  IResearchFilterBooleanTest() {
    ...
    analyzers.emplace(
        ...
        darbotdb::velocypack::Parser::fromJson("{ \"args\": \"abc\" }")
            ->slice(),
...
TEST_F(IResearchFilterBooleanTest, Ternary) {
  // can evaluate expression
  {
    ...
    ctx.vars.emplace("x", darbotdb::aql::AqlValue(darbotdb::aql::AqlValue(
                              darbotdb::aql::AqlValueHintInt{3})));
...
TEST_F(IResearchFilterBooleanTest, UnaryNot) {
  ...
  // string expression
  {
    darbotdb::aql::Variable var("c", 0, /*isFullDocumentFromCollection*/ false,
                                resourceMonitor);
    darbotdb::aql::AqlValue value(darbotdb::aql::AqlValueHintInt{41});
  ...
  // null expression
  {
    darbotdb::aql::Variable var("c", 0, /*isFullDocumentFromCollection*/ false,
                                resourceMonitor);
    darbotdb::aql::AqlValue value(darbotdb::aql::AqlValueHintNull{});
    darbotdb::aql::AqlValueGuard guard(value, true);
  ...
  // according to darbotdb rules, expression : not '1' == false
  {
    irs::Or expected;
  ...
  // nondeterministic expression -> wrap it
  {
    ...
    // iteratorForCondition
    {
      ...
      irs::Or expected;
      auto& root = expected.add<irs::And>().add<irs::Not>().filter<irs::And>();
      root.add<darbotdb::iresearch::ByExpression>().init(
          {.ast = ast},
  ...
  // nondeterministic expression -> wrap it
  {
    ...
    // iteratorForCondition
    {
      ...
      auto& root = expected.add<irs::And>().add<irs::Not>();
      root.boost(2.5);
      root.filter<irs::And>().add<darbotdb::iresearch::ByExpression>().init(
          {.ast = ast},
  ...
  // nondeterministic expression -> wrap it, boost
  {
    ...
    // iteratorForCondition
    {
      ...
      irs::Or expected;
      auto& root = expected.add<irs::And>().add<irs::Not>().filter<irs::And>();
      auto& expr = root.add<darbotdb::iresearch::ByExpression>();
      expr.boost(1.5);
      expr.init(
          {.ast = ast},
          *filterNode->getMember(0)->getMember(0)->getMember(0)->getMember(
