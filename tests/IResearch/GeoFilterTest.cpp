...
TEST(GeoFilterTest, options) {
  ...
  ASSERT_EQ(darbotdb::iresearch::GeoFilterType::INTERSECTS, opts.type);
}
...
TEST(GeoFilterTest, query) {
  ...
  // index data
  {
    constexpr auto formatId = darbotdb::iresearch::getFormat(LinkVersion::MAX);
...

  {
    ...
    *q.mutable_field() = "geometry";
    ...
    ASSERT_TRUE(darbotdb::iresearch::parseShape<
                darbotdb::iresearch::Parsing::OnlyPoint>(
        origin.get("geometry"), q.mutable_options()->shape, cache, false,
        darbotdb::geo::coding::Options::kInvalid, nullptr));
    q.mutable_options()->type = GeoFilterType::INTERSECTS;
...

  {
    ...
    darbotdb::geo::ShapeContainer shape;
    darbotdb::geo::ShapeContainer point;
    std::vector<S2LatLng> cache;
    ASSERT_TRUE(
        darbotdb::iresearch::parseShape<darbotdb::iresearch::Parsing::GeoJson>(
    ...
    for (auto doc : VPackArrayIterator(docs->slice())) {
      ...
      ASSERT_TRUE(darbotdb::iresearch::parseShape<
                  darbotdb::iresearch::Parsing::OnlyPoint>(
          geo, point, cache, false, darbotdb::geo::coding::Options::kInvalid,
          nullptr));
      ...
      expected.emplace(darbotdb::iresearch::getStringRef(name));
    }
