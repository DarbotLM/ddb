...
TEST(IResearchComparerTest, test_comparer_single_entry) {
  ...
  auto builder =
      darbotdb::basics::VelocyPackHelper::velocyPackFromFile(resource.string());
  ...
  for (auto doc : darbotdb::velocypack::ArrayIterator(docsSlice)) {
  ...
  // sort actual docs
  darbotdb::iresearch::VPackComparer<darbotdb::iresearch::IResearchViewSort>
      comparer;
