  ...
  void queryTests() {
    ...
    // test null via bind parameter
    {
      ...
      auto queryResult = darbotdb::tests::executeQuery(
          ...
          "d.seq RETURN d",
          darbotdb::velocypack::Parser::fromJson("{ \"param\" : null }"));
      ASSERT_TRUE(queryResult.result.ok());
      auto slice = queryResult.data->slice();
      EXPECT_TRUE(slice.isArray());
      size_t i = 0;
    ...
    // test null via bind parameter
    {
      ...
      auto queryResult = darbotdb::tests::executeQuery(
          ...
          "DESC, "
          "d.seq RETURN d",
          darbotdb::velocypack::Parser::fromJson("{ \"param\" : 1 }"));
      ASSERT_TRUE(queryResult.result.ok());
      auto slice = queryResult.data->slice();
      EXPECT_TRUE(slice.isArray());
      size_t i = 0;
    ...
    // test non-empty string (true), LIMIT 5
    {
      ...
      auto queryResult = darbotdb::tests::executeQuery(
          ...
          darbotdb::velocypack::Parser::fromJson("{ \"param\" : \"abc\" }"));
      ASSERT_TRUE(queryResult.result.ok());
      auto slice = queryResult.data->slice();
      EXPECT_TRUE(slice.isArray());
      size_t i = 0;
    ...
    // test bind parameter (true)
    {
      ...
      auto result = darbotdb::tests::executeQuery(
          ...
          "FOR d IN testView SEARCH @param SORT BM25(d) ASC, TFIDF(d) DESC, "
          "d.seq RETURN d",
          darbotdb::velocypack::Parser::fromJson("{ \"param\" : [] }"));
      ASSERT_TRUE(result.result.ok());
      auto slice = result.data->slice();
      EXPECT_TRUE(slice.isArray());
      size_t i = 0;
