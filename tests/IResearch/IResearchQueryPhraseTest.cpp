...
void testTerm(TRI_vocbase_t& vocbase,
              const std::vector<darbotdb::velocypack::Builder>& insertedDocs,
              unsigned flags) {
  ...
  if (flags & (kAnalyzerTest | kAnalyzerUserTest)) {
    std::vector<darbotdb::velocypack::Slice> expected = {
...
void testLevenshteinMatch(
  ...
  if (flags & (kAnalyzerTest | kAnalyzerUserTest)) {
    ...
    for (darbotdb::velocypack::ArrayIterator itr(slice); itr.valid();
         ++itr, ++i) {
      auto const resolved = itr.value().resolveExternals();
      ASSERT_TRUE(i < expected.size());
 ...
 protected:
  void create0() {
    auto& sysVocBaseFeature =
        server.getFeature<darbotdb::SystemDatabaseFeature>();
  ...
  void create1() {
    ...
    // create collection1
    {
      ...
      std::vector<std::shared_ptr<darbotdb::velocypack::Builder>>
          levenshtein_docs{
              darbotdb::velocypack::Parser::fromJson(
                  "{\"seq\": -7, \"foo\": \"abcd\" }"),
          };
