...
class QuerySelectAll : public QueryTest {
 protected:
  ...
  void populateData() {
    _insertedDocs =
        std::deque<std::shared_ptr<darbotdb::velocypack::Buffer<uint8_t>>>(101);
    ...
    for (; i < _insertedDocs.size() / 2; ++i) {
      auto const doc = darbotdb::velocypack::Parser::fromJson(
          "{ \"key\": " + std::to_string(i) + "}");
    ...
    // unordered
    {
      ...
      // check node estimation
      {
        auto explanationResult =
            darbotdb::tests::explainQuery(_vocbase, queryString);
      ...
      for (auto const actualDoc : darbotdb::velocypack::ArrayIterator(result)) {
    ...
    // check full stats with optimization
    {
      ...
      EXPECT_TRUE(darbotdb::tests::assertRules(
          ...
          {darbotdb::aql::OptimizerRule::handleArangoSearchViewsRule,
           darbotdb::aql::OptimizerRule::applySortLimitRule}));
