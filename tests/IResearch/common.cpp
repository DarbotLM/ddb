...
std::ostream& operator<<(std::ostream& os, filter const& filter) {
  ...
  } else if (type == irs::type<darbotdb::iresearch::ByExpression>::id()) {
...
struct CustomScorer final : public irs::ScorerBase<void> {
  ...
  static irs::Scorer::ptr make(std::string_view args) {
    ...
    auto json =
        darbotdb::velocypack::Parser::fromJson(args.data(), args.size());
    auto slice = json ? json->slice() : darbotdb::velocypack::Slice();

    if (!slice.isArray()) {
      return nullptr;  // incorrect argument format
    }

    darbotdb::velocypack::ArrayIterator itr{slice};
...
void init(bool withICU /*= false*/) {
  darbotdb::transaction::Methods::clearDataSourceRegistrationCallbacks();
...
bool assertRules(
  ...
  for (auto ruleId : expectedRulesIds) {
    std::string_view translated =
        darbotdb::aql::OptimizerRulesFeature::translateRule(ruleId);
  ...
  if (res.data) {
    ...
    darbotdb::velocypack::ArrayIterator rules(explanation.get("rules"));
...
darbotdb::aql::QueryResult explainQuery(
  ...
  auto ctx = std::make_shared<darbotdb::transaction::StandaloneContext>(
      vocbase, transaction::OperationOriginTestCase{});
  auto query = darbotdb::aql::Query::create(
      std::move(ctx), darbotdb::aql::QueryString(queryString), bindVars,
      darbotdb::aql::QueryOptions(
          darbotdb::velocypack::Parser::fromJson(optionsString)->slice()));
...
std::unique_ptr<darbotdb::aql::ExecutionPlan> planFromQuery(
  ...
  return darbotdb::aql::ExecutionPlan::instantiateFromAst(query->ast(), false);
}
...
uint64_t getCurrentPlanVersion(darbotdb::ArangodServer& server) {
  auto const result = darbotdb::AgencyComm(server).getValues("Plan");
  auto const planVersionSlice = result.slice()[0].get<std::string>(
      {darbotdb::AgencyCommHelper::path(), "Plan", "Version"});
...
void setDatabasePath(darbotdb::DatabasePathFeature& feature) {
  ...
  path /= std::string("darbotdb_tests.") + std::to_string(TRI_microtime());
...
void expectEqualSlices_(const VPackSlice& lhs, const VPackSlice& rhs,
  ...
  SCOPED_TRACE(where);
  EXPECT_EQ(0, darbotdb::basics::VelocyPackHelper::compare(lhs, rhs, true));
}
...
std::string mangleType(std::string name) {
  darbotdb::iresearch::kludge::mangleType(name);
  return name;
}

std::string mangleAnalyzer(std::string name) {
  darbotdb::iresearch::kludge::mangleAnalyzer(name);
  return name;
}

std::string mangleBool(std::string name) {
  darbotdb::iresearch::kludge::mangleBool(name);
  return name;
}

std::string mangleNull(std::string name) {
  darbotdb::iresearch::kludge::mangleNull(name);
  return name;
}

std::string mangleNumeric(std::string name) {
  darbotdb::iresearch::kludge::mangleNumeric(name);
  return name;
}

std::string mangleNested(std::string name) {
  darbotdb::iresearch::kludge::mangleNested(name);
  return name;
}
...
std::string mangleStringIdentity(std::string name) {
  darbotdb::iresearch::kludge::mangleField(
      name, true,
      darbotdb::iresearch::FieldMeta::Analyzer{
          darbotdb::iresearch::IResearchAnalyzerFeature::identity()});
...
void assertFilterOptimized(
    ...
    std::shared_ptr<darbotdb::velocypack::Builder> bindVars /* = nullptr */) {
  auto options = darbotdb::velocypack::Parser::fromJson(
      //    "{ \"tracing\" : 1 }"
      " { } ");
  ...
  auto plan = const_cast<darbotdb::aql::ExecutionPlan*>(query->plan());
  ...
  // execution time
  {
    ...
    irs::Or actualFilter;
    darbotdb::iresearch::QueryContext const ctx{
        .trx = &trx,
        .ast = plan->getAst(),
        .ctx = exprCtx,
        ...
        .isSearchQuery = true};
    darbotdb::iresearch::FieldMeta::Analyzer analyzer{
        darbotdb::iresearch::IResearchAnalyzerFeature::identity()};
    darbotdb::iresearch::FilterContext const filterCtx{
        .query = ctx, .contextAnalyzer = analyzer};
    EXPECT_TRUE(darbotdb::iresearch::FilterFactory::filter(
...
void assertExpressionFilter(
  ...
  auto ctx = std::make_shared<darbotdb::transaction::StandaloneContext>(
      vocbase, transaction::OperationOriginTestCase{});
  auto query = darbotdb::aql::Query::create(
      std::move(ctx), darbotdb::aql::QueryString(queryString), nullptr);
  ...
  // supportsFilterCondition
  {
    ...
    darbotdb::iresearch::FilterContext const filterCtx{
        .query = ctx, .contextAnalyzer = analyzer};
    EXPECT_TRUE((darbotdb::iresearch::FilterFactory::filter(nullptr, filterCtx,
                                                            *filterNode)
                     .ok()));
  }
  ...
  // iteratorForCondition
  {
    ...
    irs::Or expected;
    expected.add<darbotdb::iresearch::ByExpression>().init(
    ...
    darbotdb::iresearch::FilterContext const filterCtx{
        .query = ctx, .contextAnalyzer = analyzer};
    EXPECT_TRUE(darbotdb::iresearch::FilterFactory::filter(&actual, filterCtx,
                                                           *filterNode)
                    .ok());
...
void buildActualFilter(
  ...
  // optimization time
  {
    ...
    darbotdb::iresearch::FilterContext const filterCtx{
        .query = ctx, .contextAnalyzer = analyzer};
    ASSERT_TRUE(darbotdb::iresearch::FilterFactory::filter(nullptr, filterCtx,
                                                           *filterNode)
                    .ok());
  }
  ...
  // execution time
  {
    ...
    darbotdb::iresearch::FilterContext const filterCtx{
        .query = ctx, .contextAnalyzer = analyzer};
    ASSERT_TRUE(
        darbotdb::iresearch::FilterFactory::filter(
...
void assertFilter(
    ...
    std::shared_ptr<darbotdb::velocypack::Builder> bindVars /*= nullptr*/,
    std::string const& refName /*= "d"*/,
    darbotdb::iresearch::FilterOptimization filterOptimization /*= NONE*/,
  ...
  // optimization time
  {
    ...
    darbotdb::iresearch::QueryContext const ctx{
        ...
        .namePrefix = darbotdb::iresearch::nestedRoot(hasNested),
    ...
    darbotdb::iresearch::FilterContext const filterCtx{
        .query = ctx, .contextAnalyzer = analyzer};
    EXPECT_EQ(parseOk, darbotdb::iresearch::FilterFactory::filter(
  ...
  // execution time
  {
    ...
    darbotdb::iresearch::FilterContext const filterCtx{
        .query = ctx, .contextAnalyzer = analyzer};
    auto r = darbotdb::iresearch::FilterFactory::filter(&actual, filterCtx,
                                                        *filterNode);
    if (execOk) {
...
VPackBuilder getInvertedIndexPropertiesSlice(
  ...
  VPackBuilder vpack;
  {
    ...
    if (!name.empty()) {
      vpack.add(darbotdb::StaticStrings::IndexName, VPackValue{name});
    }
    vpack.add(darbotdb::StaticStrings::IndexId, VPackValue(iid.id()));
    vpack.add(darbotdb::StaticStrings::IndexType, VPackValue("inverted"));

    // FIXME: maybe this should be set by index internally ?
    vpack.add(darbotdb::StaticStrings::IndexUnique, VPackValue(false));
    vpack.add(darbotdb::StaticStrings::IndexSparse, VPackValue(true));

    {
      VPackArrayBuilder arrayFields(&vpack,
                                    darbotdb::StaticStrings::IndexFields);
