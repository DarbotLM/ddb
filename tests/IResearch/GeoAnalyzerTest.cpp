...
TEST(GeoPointAnalyzerTest, tokenizePointFromArray) {
  ...
  // tokenize point
  {
    ...
    auto* term = irs::get<irs::term_attribute>(a);
    ASSERT_NE(nullptr, term);
    ASSERT_TRUE(a.reset(darbotdb::iresearch::ref<char>(json->slice())));
...
TEST(GeoPointAnalyzerTest, tokenizePointFromObject) {
  ...
  // tokenize point
  {
    ...
    auto* term = irs::get<irs::term_attribute>(a);
    ASSERT_NE(nullptr, term);
    ASSERT_TRUE(a.reset(darbotdb::iresearch::ref<char>(jsonObject->slice())));
...
TEST(GeoVPackAnalyzerTest, tokenizeLatLngRect) {
  ...
  // tokenize point
  {
    ...
    auto* term = irs::get<irs::term_attribute>(a);
    ASSERT_NE(nullptr, term);
    ASSERT_FALSE(a.reset(darbotdb::iresearch::ref<char>(json->slice())));
    ASSERT_FALSE(a.next());
  }
}
...
TEST(GeoVPackAnalyzerTest, tokenizePointGeoJSONArray) {
  ...
  ASSERT_TRUE(parseShape<darbotdb::iresearch::Parsing::OnlyPoint>(
...
TEST(GeoVPackAnalyzerTest, invalidGeoJson) {
  // tokenize shape
  {
    ...
    ASSERT_FALSE(
        a.reset(darbotdb::iresearch::ref<char>(VPackSlice::emptyArraySlice())));
    ASSERT_FALSE(
        a.reset(darbotdb::iresearch::ref<char>(VPackSlice::noneSlice())));
    ASSERT_FALSE(
        a.reset(darbotdb::iresearch::ref<char>(VPackSlice::illegalSlice())));
    ASSERT_FALSE(
        a.reset(darbotdb::iresearch::ref<char>(VPackSlice::falseSlice())));
    ASSERT_FALSE(
        a.reset(darbotdb::iresearch::ref<char>(VPackSlice::trueSlice())));
    ASSERT_FALSE(
        a.reset(darbotdb::iresearch::ref<char>(VPackSlice::zeroSlice())));
    ASSERT_FALSE(
        a.reset(darbotdb::iresearch::ref<char>(VPackSlice::nullSlice())));
  }
