...
TEST(GeoDistanceFilterTest, query) {
  ...
  // index data
  {
    constexpr auto formatId =
        darbotdb::iresearch::getFormat(darbotdb::iresearch::LinkVersion::MAX);
...

  {
    std::set<std::string> expected;
    for (auto doc : VPackArrayIterator(docs->slice())) {
      auto name = darbotdb::iresearch::getStringRef(doc.get("name"));
...

  {
    ...
    darbotdb::geo::ShapeContainer lhs, rhs;
    std::vector<S2LatLng> cache;
    ASSERT_TRUE(darbotdb::iresearch::parseShape<
                darbotdb::iresearch::Parsing::OnlyPoint>(
        origin, lhs, cache, false, darbotdb::geo::coding::Options::kInvalid,
        nullptr));
    std::set<std::string> expected;
    for (auto doc : VPackArrayIterator(docs->slice())) {
      ...
      ASSERT_TRUE(darbotdb::iresearch::parseShape<
                  darbotdb::iresearch::Parsing::OnlyPoint>(
          geo, rhs, cache, false, darbotdb::geo::coding::Options::kInvalid,
          nullptr));
