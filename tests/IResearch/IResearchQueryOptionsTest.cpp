 ...
 protected:
  ...
  void queryTestCollections() {
    ...
    // collection by id
    {
      ...
      {
        auto explanationResult = darbotdb::tests::explainQuery(_vocbase, query);
    ...
    // multiple collections
    {
      ...
      std::map<
          std::string_view,
          std::vector<std::shared_ptr<darbotdb::velocypack::Buffer<uint8_t>>>>
    ...
    // join restricted views
    {
      ...
      for (auto const actualValue : resultIt) {
        ...
        auto const keyd = darbotdb::iresearch::getStringRef(keySliced);
        auto const keySlicex = resolvedx.get("name");
        auto const keyx = darbotdb::iresearch::getStringRef(keySlicex);
        EXPECT_EQ(keyd, keyx);

        auto it = expectedDocs.find(keyd);
        ASSERT_NE(it, expectedDocs.end());
        ASSERT_EQ(2U, it->second.size());
        ASSERT_TRUE(0 == darbotdb::basics::VelocyPackHelper::compare(
                             darbotdb::velocypack::Slice(it->second[0]->data()),
                             resolvedd, true));
        ASSERT_TRUE(0 == darbotdb::basics::VelocyPackHelper::compare(
                             darbotdb::velocypack::Slice(it->second[1]->data()),
                             resolvedx, true));
  ...
  void queryTestWaitForSync() {
    ...
    // do sync, bind parameter
    {
      ...
      EXPECT_TRUE(darbotdb::tests::assertRules(
          _vocbase, query,
          {darbotdb::aql::OptimizerRule::handleArangoSearchViewsRule},
          darbotdb::velocypack::Parser::fromJson("{ \"doSync\" : true }")));
...
class QueryOptionsView : public QueryOptions {
 protected:
  ...
  void createView(bool storedValues) {
    auto createJson = darbotdb::velocypack::Parser::fromJson(absl::Substitute(
