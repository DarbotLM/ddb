...
struct MockQuery final : darbotdb::aql::Query {
  MockQuery(std::shared_ptr<darbotdb::transaction::Context> const& ctx,
            darbotdb::aql::QueryString const& queryString)
      : darbotdb::aql::Query{ctx, queryString, nullptr, {}, nullptr} {}
  ...
  darbotdb::transaction::Methods& trxForOptimization() final {
...
TEST_F(IResearchViewNodeTest, constructSortedView) {
  ...
  MockQuery query(
      ...
      darbotdb::aql::QueryString(std::string_view("RETURN 1")));
  darbotdb::tests::waitForAsync(query.prepareQuery());
  darbotdb::aql::Variable const outVariable("variable", 1, false,
                                            resourceMonitor);

  {
    ...
    darbotdb::iresearch::IResearchViewNode node(*query.plan(),  // plan
                                                json->slice());
    ...
    EXPECT_EQ(darbotdb::aql::ExecutionNode::ENUMERATE_IRESEARCH_VIEW,
              node.getType());
    ...
    EXPECT_EQ(darbotdb::aql::ExecutionNodeId{42}, node.id());
...
TEST_F(IResearchViewNodeTest, construct) {
  ...
  // no options
  {
    ...
    darbotdb::iresearch::IResearchViewNode node(
        *query.plan(),  // plan
        darbotdb::aql::ExecutionNodeId{42},
        vocbase,      // database
        logicalView,  // view
  ...
  {
    // build options node
    darbotdb::aql::AstNode attributeValue(darbotdb::aql::NODE_TYPE_VALUE);
    attributeValue.setValueType(darbotdb::aql::VALUE_TYPE_BOOL);
    attributeValue.setBoolValue(true);
    darbotdb::aql::AstNode attributeName(
        darbotdb::aql::NODE_TYPE_OBJECT_ELEMENT);
    attributeName.addMember(&attributeValue);
    attributeName.setStringValue("waitForSync", strlen("waitForSync"));
    darbotdb::aql::AstNode options(darbotdb::aql::NODE_TYPE_OBJECT);
    ...
    EXPECT_EQ(&outVariable, setHere[0]);
    EXPECT_TRUE(node.options().forceSync);
    EXPECT_EQ(darbotdb::aql::ConditionOptimization::kAuto,
  ...
  // with options default optimization
  {
    ...
    darbotdb::aql::AstNode attributeValue(darbotdb::aql::NODE_TYPE_VALUE);
    attributeValue.setValueType(darbotdb::aql::VALUE_TYPE_STRING);
  ...
  // with options none optimization
  {
    ...
    EXPECT_EQ(&outVariable, setHere[0]);
    EXPECT_FALSE(node.options().forceSync);
    EXPECT_EQ(darbotdb::aql::ConditionOptimization::kNone,
              node.options().conditionOptimization);
    EXPECT_EQ(node.options().countApproximate,
              darbotdb::iresearch::CountApproximate::Exact);
    EXPECT_EQ(0., node.getCost().estimatedCost);    // no dependencies
  ...
  // with options noneg optimization
  {
    ...
    EXPECT_EQ(&outVariable, setHere[0]);
    EXPECT_FALSE(node.options().forceSync);
    EXPECT_EQ(darbotdb::aql::ConditionOptimization::kNoNegation,
              node.options().conditionOptimization);
    EXPECT_EQ(node.options().countApproximate,
              darbotdb::iresearch::CountApproximate::Exact);
    EXPECT_EQ(0., node.getCost().estimatedCost);    // no dependencies
  ...
  // with options nodnf optimization
  {
    ...
    EXPECT_EQ(&outVariable, setHere[0]);
    EXPECT_FALSE(node.options().forceSync);
    EXPECT_EQ(darbotdb::aql::ConditionOptimization::kNoDNF,
              node.options().conditionOptimization);
    EXPECT_EQ(node.options().countApproximate,
              darbotdb::iresearch::CountApproximate::Exact);
    EXPECT_EQ(0., node.getCost().estimatedCost);    // no dependencies
  ...
  // invalid options
  {
    ...
    EXPECT_ANY_THROW(darbotdb::iresearch::IResearchViewNode(
        *query.plan(),  // plan
        darbotdb::aql::ExecutionNodeId{42},
        vocbase,         // database
        logicalView,     // view
  ...
  }
  {
    ...
    std::vector<darbotdb::iresearch::HeapSortElement> scorersSort{
