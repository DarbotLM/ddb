...
class IResearchInvertedIndexConditionTest
 ...
 protected:
  ...
  void assertProjectionsCoverageSuccess(
      ...
      std::vector<darbotdb::aql::AttributeNamePath> const& attributes,
    ...
    auto* index = dynamic_cast<darbotdb::iresearch::IResearchInvertedIndex*>(
        inverted.get());
    ASSERT_TRUE(index);
  ...
  void estimateFilterCondition(
      ...
      darbotdb::Index::FilterCosts const& expectedCosts,
    ...
    auto indexFields =
        darbotdb::iresearch::IResearchInvertedIndex::fields(index->meta());
    ...
    // optimization time
    {
      ...
      darbotdb::transaction::Methods trx{ctx};
  ...
  void estimateSortCondition(
      ...
      darbotdb::Index::SortCosts const& expectedCosts,
      ExpressionContext* exprCtx = nullptr,
      std::shared_ptr<darbotdb::velocypack::Builder> bindVars = nullptr,
      std::string const& refName = "d") {
    ...
    {
      darbotdb::containers::SmallVector<ExecutionNode*, 8> sortNodes;
      plan->findNodesOfType(sortNodes, {ExecutionNode::SORT}, false);
      for (auto s : sortNodes) {
        auto sortNode = darbotdb::aql::ExecutionNode::castTo<SortNode*>(s);
    ...
    {
      darbotdb::containers::SmallVector<ExecutionNode*, 8> setNodes;
    ...
    darbotdb::containers::HashSet<std::vector<darbotdb::basics::AttributeName>>
        nonNullAttributes;
    {
...
TEST_F(IResearchInvertedIndexConditionTest, test_with_equality) {
  ...
  std::vector<std::string> fields = {"a"};
  auto expected = darbotdb::Index::FilterCosts::defaultCosts(0);
...
TEST_F(IResearchInvertedIndexConditionTest, test_with_nondeterm_expression) {
  ...
  estimateFilterCondition(queryString, fields,
                          darbotdb::Index::FilterCosts::defaultCosts(0));
}
...
TEST_F(IResearchInvertedIndexConditionTest, test_with_range_func) {
  ...
  ctx.vars.emplace("a", darbotdb::aql::AqlValue(obj->slice()));
  std::vector<std::string> fields = {"a"};
  auto expected = darbotdb::Index::FilterCosts::defaultCosts(0);
  expected.supportsCondition = true;
  estimateFilterCondition(queryString, fields, expected, &ctx);
...
TEST_F(IResearchInvertedIndexConditionTest, test_with_range_func_bind) {
  ...
  ctx.vars.emplace("a", darbotdb::aql::AqlValue(obj2->slice()));
  std::string queryString =
...
TEST_F(IResearchInvertedIndexConditionTest, sort_support) {
  ...
  std::vector<std::pair<std::string, bool>> fields = {{"a", true},
                                                      {"b", false}};
  auto expected = darbotdb::Index::SortCosts::zeroCosts(2);
...
TEST_F(IResearchInvertedIndexConditionTest, sort_support_subset) {
  ...
  std::vector<std::pair<std::string, bool>> fields = {{"a", true},
                                                      {"b", false}};
  auto expected = darbotdb::Index::SortCosts::zeroCosts(1);
  estimateSortCondition(queryString, fields, expected, &ctx);
}
...
TEST_F(IResearchInvertedIndexConditionTest, sort_support_invalid_direct) {
  ...
  auto expected = darbotdb::Index::SortCosts::defaultCosts(0);
  estimateSortCondition(queryString, fields, expected, &ctx);
}
