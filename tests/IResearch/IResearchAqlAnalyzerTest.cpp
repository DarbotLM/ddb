...
void assert_analyzer(irs::analysis::analyzer* analyzer, const std::string& data,
  ...
  auto* vpack_term =
      irs::get<darbotdb::iresearch::VPackTermAttribute>(*analyzer);
  auto* value_type =
      irs::get<darbotdb::iresearch::AnalyzerValueTypeAttribute>(*analyzer);
  ASSERT_TRUE(term);
  ...
  ASSERT_EQ(
      dynamic_cast<darbotdb::iresearch::AqlAnalyzer*>(analyzer)->isOptimized(),
      shouldBeOptimized);
  ...
  while (analyzer->next()) {
    ...
    if (value_type->value == darbotdb::iresearch::AnalyzerValueType::String) {
      ...
      ASSERT_EQ(0, darbotdb::basics::VelocyPackHelper::compare(
                       vpack_term->value,
...
TEST_F(IResearchAqlAnalyzerTest, test_create_valid) {
  // const value
  {
    ...
    ASSERT_FALSE(dynamic_cast<darbotdb::iresearch::AqlAnalyzer*>(ptr.get())
                     ->isOptimized());
  ...
  // cycle with array
  {
    ...
    ASSERT_FALSE(dynamic_cast<darbotdb::iresearch::AqlAnalyzer*>(ptr.get())
                     ->isOptimized());
    assert_analyzer(ptr.get(), "darbotdb",
                    {{"darbotdb", 0}, {"darbotdb", 1}, {"darbotdb", 2}}, false);
  ...
  // nested cycles
  {
    auto ptr = irs::analysis::analyzers::get(
        AQL_ANALYZER_NAME, irs::type<irs::text_format::vpack>::get(),
        darbotdb::iresearch::ref<char>(VPackParser::fromJson("\
  ...
  // only null
  {
    ...
    ASSERT_TRUE(dynamic_cast<darbotdb::iresearch::AqlAnalyzer*>(ptr.get())
                    ->isOptimized());
...
TEST_F(IResearchAqlAnalyzerTest, test_numeric_return) {
  ...
  darbotdb::aql::AqlValue val{darbotdb::aql::AqlValueHintDouble(2)};
...
TEST_F(IResearchAqlAnalyzerTest, test_numeric_return_array) {
  ...
  for (uint32_t i = 1; i <= 3; ++i) {
    darbotdb::aql::AqlValue val{darbotdb::aql::AqlValueHintDouble(i)};
    analyzer_token token;
    token.pos = i - 1;
    token.value.assign(val.slice().startAs<char>(), val.slice().byteSize());
    expected_tokens.push_back(std::move(token));
...
TEST_F(IResearchAqlAnalyzerTest, test_bool_return) {
  ...
  analyzer_tokens expected_tokens;
  darbotdb::aql::AqlValue val{darbotdb::aql::AqlValueHintBool(true)};
  analyzer_token token;
  token.pos = 0;
  token.value.assign(val.slice().startAs<char>(), val.slice().byteSize());
  expected_tokens.push_back(std::move(token));
...
TEST_F(IResearchAqlAnalyzerTest, test_bool_return_array) {
  ...
  for (uint32_t i = 1; i <= 3; ++i) {
    darbotdb::aql::AqlValue val{darbotdb::aql::AqlValueHintBool(i == 2)};
    analyzer_token token;
    token.pos = i - 1;
    token.value.assign(val.slice().startAs<char>(), val.slice().byteSize());
    expected_tokens.push_back(std::move(token));
...
TEST_F(IResearchAqlAnalyzerTest, test_number_return_array_keep_null) {
  ...
  for (uint32_t i = 1; i <= 30; ++i) {
    darbotdb::aql::AqlValue val{
        darbotdb::aql::AqlValueHintDouble(i <= 5 ? i : 0)};
    analyzer_token token;
    token.pos = i - 1;
    token.value.assign(val.slice().startAs<char>(), val.slice().byteSize());
    expected_tokens.push_back(std::move(token));
...
TEST_F(IResearchAqlAnalyzerTest, test_bool_return_array_keep_null) {
  ...
  for (uint32_t i = 1; i <= 30; ++i) {
    darbotdb::aql::AqlValue val{darbotdb::aql::AqlValueHintBool(i <= 5)};
    analyzer_token token;
    token.pos = i - 1;
    token.value.assign(val.slice().startAs<char>(), val.slice().byteSize());
    expected_tokens.push_back(std::move(token));
