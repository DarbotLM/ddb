...
TEST_F(IResearchFilterArrayIntervalTest, Interval) {
  ...
  // invalid dynamic attribute name (bool value)
  {
    for (auto operation : intervalOperations) {
      ...
      ExpressionContextMock ctx;
      ctx.vars.emplace(
          "a", darbotdb::aql::AqlValue(darbotdb::aql::AqlValueHintBool{false}));
      ctx.vars.emplace("c",
                       darbotdb::aql::AqlValue(darbotdb::aql::AqlValue{"c"}));
      ctx.vars.emplace("offsetInt",
                       darbotdb::aql::AqlValue(darbotdb::aql::AqlValue(
  ...
  // reference in array
  {
    for (auto operation : intervalOperations) {
      ...
      darbotdb::aql::Variable var("c", 0,
                                  /*isFullDocumentFromCollection*/ false,
                                  resourceMonitor);
      darbotdb::aql::AqlValue value(darbotdb::aql::AqlValue("2"));
  ...
  // array as reference, boost, analyzer
  {
    for (auto operation : intervalOperations) {
      ...
      auto obj =
          darbotdb::velocypack::Parser::fromJson("[ \"1\", \"2\", \"3\"]");
  ...
  // heterogeneous references and expression in array, analyzer, boost
  {
    for (auto operation : intervalOperations) {
      ...
      ctx.vars.emplace("nullVal", darbotdb::aql::AqlValue(
                                      darbotdb::aql::AqlValueHintNull{}));
      irs::Or actual;
      buildActualFilter(vocbase(), queryString, actual, &ctx);
      auto subFiltersIterator = operation.second.first(actual, 2.5);
