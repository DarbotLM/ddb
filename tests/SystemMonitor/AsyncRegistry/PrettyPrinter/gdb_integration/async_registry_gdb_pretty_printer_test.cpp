...
using namespace darbotdb::async_registry;

auto breakpoint() { raise(SIGINT); }

auto format(darbotdb::basics::SourceLocationSnapshot const& loc)
    -> std::string {
  return fmt::format("\"{}\" (\"{}\":{})", loc.function_name, loc.file_name,
                     loc.line);
}
auto format(darbotdb::basics::ThreadId const& thread) -> std::string {
  return fmt::format("LWPID {} (pthread {})", thread.kernel_id,
                     thread.posix_id);
}
auto format(darbotdb::basics::ThreadInfo const& thread) -> std::string {
...
auto format(PromiseSnapshot const& snapshot) -> std::string {
  if (snapshot.thread == std::nullopt) {
    return fmt::format("{}, owned by {}, {}", format(snapshot.source_location),
                       ...
                       darbotdb::inspection::json(snapshot.state));
  } else {
...
int main() {
  ...
  auto current_thread = darbotdb::basics::ThreadInfo::current();
  ...
  auto* second_parent = thread_registry->add([&]() {
    return Promise{{darbotdb::basics::ThreadInfo::current()},
                   std::source_location::current()};
  });
  expected = fmt::format(
      "async registry = {{\n"
  ...
  auto other_thread =
      darbotdb::basics::ThreadInfo{};  // simulate another thread
