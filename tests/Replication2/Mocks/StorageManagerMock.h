...
struct StorageManagerMock
    : darbotdb::replication2::replicated_log::IStorageManager {
  MOCK_METHOD(std::unique_ptr<
                  darbotdb::replication2::replicated_log::IStorageTransaction>,
  ...
  MOCK_METHOD(std::unique_ptr<LogIterator>, getLogIterator, (LogIndex),
              (const, override));
  MOCK_METHOD(darbotdb::replication2::replicated_log::TermIndexMapping,
              getTermIndexMapping, (), (const, override));
  MOCK_METHOD(storage::PersistedStateInfo, getCommittedMetaInfo, (),
              (const, override));
  MOCK_METHOD(
      std::unique_ptr<
          darbotdb::replication2::replicated_log::IStateInfoTransaction>,
