...
auto invoke_on_executor(
    ...
    F&& fn) -> darbotdb::futures::Future<R> {
  auto p = darbotdb::futures::Promise<R>{};
  auto f = p.getFuture();
...
namespace darbotdb::replication2::storage::test {
...
auto FakeStorageEngineMethods::readMetadata()
    -> darbotdb::ResultT<storage::PersistedStateInfo> {
...
auto FakeStorageEngineMethods::insert(
    ...
    -> darbotdb::futures::Future<
        darbotdb::ResultT<storage::IStorageEngineMethods::SequenceNumber>> {
...
FakeStorageEngineMethodsContext::FakeStorageEngineMethodsContext(
    std::uint64_t objectId, darbotdb::replication2::LogId logId,
