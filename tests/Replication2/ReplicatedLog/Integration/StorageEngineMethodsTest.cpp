...
namespace darbotdb::replication2::storage::rocksdb::test {
...
struct RocksDBInstance : public ICompactKeyRange {
  explicit RocksDBInstance(std::string path) : _path(std::move(path)) {
    ...
    if (auto s = ::rocksdb::DB::Open(options, _path, &_db); !s.ok()) {
      ...
      THROW_DDB_EXCEPTION_MESSAGE(res.errorNumber(), res.errorMessage());
    }
  }
...
struct StorageEngineMethodsTest : ::testing::Test {
  ...
  std::shared_ptr<darbotdb::replication2::storage::rocksdb::
                      AsyncLogWriteBatcher::IAsyncExecutor>
      executor = std::make_shared<darbotdb::replication2::storage::rocksdb::
...
namespace darbotdb::replication2::storage::rocksdb::tests {
...
struct AsyncLogWriteBatcherMetricsMock : AsyncLogWriteBatcherMetrics {
  AsyncLogWriteBatcherMetricsMock() {
    ...
    numWorkerThreadsWaitForSync =
        makeMetric<darbotdb_replication2_rocksdb_num_persistor_worker>();
    numWorkerThreadsNoWaitForSync =
        makeMetric<darbotdb_replication2_rocksdb_num_persistor_worker>();
    queueLength = makeMetric<darbotdb_replication2_rocksdb_queue_length>();
    writeBatchSize =
        makeMetric<darbotdb_replication2_rocksdb_write_batch_size>();
    rocksdbWriteTimeInUs =
        makeMetric<darbotdb_replication2_rocksdb_write_time>();
    rocksdbSyncTimeInUs = makeMetric<darbotdb_replication2_rocksdb_sync_time>();
    operationLatencyInsert =
        makeMetric<darbotdb_replication2_storage_operation_latency>();
...
using MyTypes = ::testing::Types<
    darbotdb::replication2::storage::rocksdb::tests::RocksDBFactory,
    darbotdb::replication2::storage::rocksdb::tests::FakeFactory>;
...
TYPED_TEST(StorageEngineMethodsTest, read_meta_data_not_found) {
  ...
  ASSERT_EQ(result.errorNumber(), TRI_ERROR_DDB_DOCUMENT_NOT_FOUND);
}
