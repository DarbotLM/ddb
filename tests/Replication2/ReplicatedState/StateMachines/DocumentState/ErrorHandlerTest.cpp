...
using namespace darbotdb::replication2::replicated_state::document;
...
TEST_F(DocumentStateErrorHandlerTest, create_shard_test) {
  ...
  EXPECT_EQ(errorHandler->handleOpResult(op, TRI_ERROR_DDB_DUPLICATE_NAME),
            TRI_ERROR_NO_ERROR);
...
TEST_F(DocumentStateErrorHandlerTest, drop_shard_test) {
  ...
  EXPECT_EQ(
      errorHandler->handleOpResult(op, TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND),
      TRI_ERROR_NO_ERROR);
  EXPECT_EQ(errorHandler->handleOpResult(op, TRI_ERROR_WAS_ERLAUBE),
            TRI_ERROR_WAS_ERLAUBE);
}
...
TEST_F(DocumentStateErrorHandlerTest, create_index_test) {
  ...
  EXPECT_EQ(errorHandler->handleOpResult(
                op, TRI_ERROR_DDB_UNIQUE_CONSTRAINT_VIOLATED),
            TRI_ERROR_NO_ERROR);
  EXPECT_EQ(errorHandler->handleOpResult(op, TRI_ERROR_WAS_ERLAUBE),
            TRI_ERROR_WAS_ERLAUBE);
}
...
TEST_F(DocumentStateErrorHandlerTest, drop_index_test) {
  ...
  EXPECT_EQ(
      errorHandler->handleOpResult(op, TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND),
      TRI_ERROR_NO_ERROR);
  EXPECT_EQ(errorHandler->handleOpResult(op, TRI_ERROR_DDB_INDEX_NOT_FOUND),
            TRI_ERROR_NO_ERROR);
  EXPECT_EQ(errorHandler->handleOpResult(op, TRI_ERROR_WAS_ERLAUBE),
            TRI_ERROR_WAS_ERLAUBE);
}
...
TEST_F(DocumentStateErrorHandlerTest, document_transaction_test) {
  auto res = OperationResult{
      Result{TRI_ERROR_DDB_UNIQUE_CONSTRAINT_VIOLATED}, OperationOptions{}};
  EXPECT_EQ(
      errorHandler->handleDocumentTransactionResult(res, TransactionId{1}),
      TRI_ERROR_NO_ERROR);

  res = OperationResult{Result{TRI_ERROR_DDB_DOCUMENT_NOT_FOUND},
                        OperationOptions{}};
  EXPECT_EQ(
      errorHandler->handleDocumentTransactionResult(res, TransactionId{1}),
      TRI_ERROR_NO_ERROR);
  ...
  res.countErrorCodes = {{TRI_ERROR_DDB_DOCUMENT_NOT_FOUND, 1}};
  EXPECT_EQ(
      errorHandler->handleDocumentTransactionResult(res, TransactionId{1}),
      TRI_ERROR_NO_ERROR);
