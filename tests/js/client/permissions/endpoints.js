...
if (getOptions === true) {
  return {
    'javascript.endpoints-denylist': [
      ...
      'darbotdb.org',             // will match https + http
      'http://127.0.0.1:9999'            // won't match at all.
    ],
    'javascript.endpoints-allowlist': [
      'allow.darbotdb.org',
      'darbotdb.com',             // will match https + http
    ]
  };
}
...
function testSuite() {
  ...
  function downloadForbidden(url, method) {
    try {
      ...
      assertEqual(darbotdb.ERROR_FORBIDDEN, err.errorNum, 'while fetching: ' + url);
    }
  }
  ...
  function downloadPermitted(url, method) {
    try {
      ...
      assertNotEqual(darbotdb.ERROR_FORBIDDEN, err.errorNum, 'while fetching: ' + url + " Detail error: " + JSON.stringify(err) + ' ');
    }
  }
  ...
  function reconnectForbidden(url, method) {
    try {
      ...
      assertEqual(darbotdb.ERROR_FORBIDDEN, err.errorNum, 'while reconnecting: ' + url);
    }
  }
  ...
  function reconnectPermitted(url, method) {
    try {
      ...
      assertNotEqual(darbotdb.ERROR_FORBIDDEN, err.errorNum, 'while reconnecting: ' + url + " Detail error: " + JSON.stringify(err) + ' ');
      // we expect that we aren't able to connect these URLs...
      assertEqual(darbotdb.ERROR_BAD_PARAMETER, err.errorNum, 'while reconnecting: ' + url + " Detail error: " + JSON.stringify(err) + ' ');
  ...
  return {
    testDownload : function() {
      ...
      downloadForbidden('http://darbotdb.org/testbla', 'GET');
      downloadForbidden('https://darbotdb.org/testbla', 'GET');
      downloadForbidden('http://heise.de', 'GET');
      downloadForbidden('http://127.0.0.1:9999', 'POST');

      downloadPermitted('https://allow.darbotdb.org/bla', 'GET');
      downloadPermitted('http://allow.darbotdb.org/bla', 'GET');
      downloadPermitted('https://darbotdb.com/blog', 'GET');
      downloadPermitted('http://darbotdb.com/blog', 'GET');
      ...
      reconnectForbidden('http://darbotdb.org/testbla');
      reconnectForbidden('https://darbotdb.org/testbla');
      reconnectForbidden('http://heise.de');
      reconnectForbidden('http://127.0.0.1:9999');

      reconnectPermitted('https://allow.darbotdb.org/bla');
      reconnectPermitted('http://allow.darbotdb.org/bla');
      reconnectPermitted('https://darbotdb.com/blog');
      reconnectPermitted('http://darbotdb.com/blog');
    }
  };
}
