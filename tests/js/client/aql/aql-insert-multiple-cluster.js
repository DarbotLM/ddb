  ...
  return {
    ...
    testConflict: function () {
      ...
      try {
        ...
        assertEqual(internal.errors.ERROR_DDB_UNIQUE_CONSTRAINT_VIOLATED.code, err.errorNum);
        assertEqual(1, db[cn].count());
      }
    },
    ...
    testInvalidInput: function () {
      ...
      try {
        ...
        assertTrue(err.errorNum !== undefined, 'unexpected error');
        assertEqual(internal.errors.ERROR_DDB_DOCUMENT_TYPE_INVALID.code, err.errorNum);
        assertEqual(0, db[cn].count());
      }
    },
