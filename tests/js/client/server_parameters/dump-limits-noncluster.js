...
// /
/// @author Jan Steemann
/// @author Copyright 2021, darbotdb Inc, Cologne, Germany
// //////////////////////////////////////////////////////////////////////////////
...
const { getMetricSingle } = require('@darbotdb/test-helper');
...
function testSuite() {
  ...
  return {
    ...
    testSingleDumpWithoutBlocking: function() {
      const originalBlocked = getMetricSingle("darbotdb_dump_threads_blocked_total");
      ...
      try {
        ...
        while (++tries < 120) {
          ...
          let ongoing = getMetricSingle("darbotdb_dump_ongoing");
          assertEqual(1, ongoing);
          
          // we expect no further blocking by our small dump
          let blocked = getMetricSingle("darbotdb_dump_threads_blocked_total");
          assertEqual(originalBlocked, blocked);

          // memory usage should be below 16 MB, but with a bit of leeway. 
          let memoryUsage = getMetricSingle("darbotdb_dump_memory_usage");
    ...
    testSingleDumpWithBlocking: function() {
      ...
      try {
        ...
        while (++tries < 120) {
          ...
          // at some point
          blocked = getMetricSingle("darbotdb_dump_threads_blocked_total");
