...
let request = require("@darbotdb/request");
const isEnterprise = require("internal").isEnterprise();
let fs = require('fs');
let pu = require('@darbotdb/testutils/process-utils');
let db = darbotdb.db;

let { getEndpointsByType, versionHas } = require('@darbotdb/test-helper');
...
const {
  ...
  joinBGShells,
  cleanupBGShells
} = require('@darbotdb/testutils/client-tools').run;

const graphModule = require('@darbotdb/general-graph');
const { expect } = require('chai');
const toArgv = require('internal').toArgv;


const getMetric = require('@darbotdb/test-helper').getMetricSingle;
let { debugResetRaceControl } = require('@darbotdb/test-helper');
let { instanceRole } = require('@darbotdb/testutils/instance');
...
function CommunicationSuite() {
  ...
  const cn = "UnitTests" + require("@darbotdb/crypto").md5(internal.genRandomAlphaNumbers(32));
...
`;
  ...
  const documentWrite = `
    let internal = require('internal');
    let crypto = require("@darbotdb/crypto");
    const col = db["${twoShardColName}"];
  ...
  const addTestCase = (suite, first, second) => {
    ...
    suite[`testAqlSetupPathDeadLock${fName}${sName}${type}`] = function () {
      ...
      const seqLocksBefore = getMetric("darbotdb_collection_lock_sequential_mode");
      // run both queries in parallel
      singleRun(tests);

      const seqLocksAfter = getMetric("darbotdb_collection_lock_sequential_mode");
