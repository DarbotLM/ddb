...
const { executeExternalAndWaitWithSanitizer } = require('@darbotdb/test-helper');
...
function arangoSecureInstallationSuite () {
  ...
  const arangoSecureInstallation = pu.DDB_SECURE_INSTALLATION_BIN;

  assertTrue(fs.isFile(arangoSecureInstallation), "arango-secure-installation not found!");

  return {
    setUpAll: function () {
      oldPasswd = internal.env['darbotdb_DEFAULT_ROOT_PASSWORD'];
    },
    
    tearDownAll: function () {
      internal.env['darbotdb_DEFAULT_ROOT_PASSWORD'] = oldPasswd;
    },

    setUp: function () {
      delete internal.env['darbotdb_DEFAULT_ROOT_PASSWORD'];
    },
    ...
    testInvokeArangoSecureInstallationWithPassword: function () {
      ...
      internal.env['darbotdb_DEFAULT_ROOT_PASSWORD'] = 'haxxmann';
      try {
