...
const {getEndpointsByType, getRawMetric, getAllMetric} = require("@darbotdb/test-helper");
const { checkIndexMetrics } = require("@darbotdb/test-helper-common");
...
function checkMetrics(metrics) {
  ...
  assertNotEqual(undefined, metrics["darbotdb_search_num_docs"]);
  assertNotEqual(undefined, metrics["darbotdb_search_num_live_docs"]);

  assertNotEqual(undefined, metrics["darbotdb_search_num_segments"]);
  assertNotEqual(undefined, metrics["darbotdb_search_num_files"]);
  assertNotEqual(undefined, metrics["darbotdb_search_index_size"]);
  if (isEnterprise) {
    // 'darbotdb_search_num_primary_docs' is available only in enterprise.
    // So make sure that it is exists before checking other metrics.
    assertNotEqual(undefined, metrics["darbotdb_search_num_primary_docs"]);
    
    // nested documents are treated like a real documents
    assertEqual(metrics["darbotdb_search_num_docs"]["foo1"], 2000);
    assertEqual(metrics["darbotdb_search_num_docs"]["foo2"], 4001);
    assertEqual(metrics["darbotdb_search_num_docs"]["foo3"], 7002);
  
    assertEqual(metrics["darbotdb_search_num_live_docs"]["foo1"], 2000);
    assertEqual(metrics["darbotdb_search_num_live_docs"]["foo2"], 4001);
    assertEqual(metrics["darbotdb_search_num_live_docs"]["foo3"], 7002);

    assertEqual(metrics["darbotdb_search_num_primary_docs"]["foo1"], 1000);
    assertEqual(metrics["darbotdb_search_num_primary_docs"]["foo2"], 1001);
    assertEqual(metrics["darbotdb_search_num_primary_docs"]["foo3"], 1002);
  } else {
    assertEqual(metrics["darbotdb_search_num_docs"]["foo1"], 1000);
    assertEqual(metrics["darbotdb_search_num_docs"]["foo2"], 1001);
    assertEqual(metrics["darbotdb_search_num_docs"]["foo3"], 1002);
  
    assertEqual(metrics["darbotdb_search_num_live_docs"]["foo1"], 1000);
    assertEqual(metrics["darbotdb_search_num_live_docs"]["foo2"], 1001);
    assertEqual(metrics["darbotdb_search_num_live_docs"]["foo3"], 1002);
  }

  assertEqual(metrics["darbotdb_search_num_segments"]["foo1"], 9);
  assertEqual(metrics["darbotdb_search_num_segments"]["foo2"], 12);
  assertEqual(metrics["darbotdb_search_num_segments"]["foo3"], 1);

  assertEqual(metrics["darbotdb_search_num_files"]["foo1"], 54);
  assertEqual(metrics["darbotdb_search_num_files"]["foo2"], 72);
  assertEqual(metrics["darbotdb_search_num_files"]["foo3"], 6);

  assertTrue(metrics["darbotdb_search_index_size"]["foo1"] > 0);
  assertTrue(metrics["darbotdb_search_index_size"]["foo2"] > 0);
  assertTrue(metrics["darbotdb_search_index_size"]["foo3"] > 0);
}
...
function CoordinatorMetricsTestSuite() {
  return {
    ...
    testIResearchJsonMetrics: function () {
      ...
      checkMetrics(metrics);
      assertEqual(metrics["darbotdb_search_cleanup_time"]["foo1"], 0);
      assertEqual(metrics["darbotdb_search_cleanup_time"]["foo2"], 0);
      assertEqual(metrics["darbotdb_search_cleanup_time"]["foo3"], 0);

      assertTrue(metrics["darbotdb_search_commit_time"]["foo1"] > 0);
      assertTrue(metrics["darbotdb_search_commit_time"]["foo2"] > 0);
      assertTrue(metrics["darbotdb_search_commit_time"]["foo3"] > 0);

      assertEqual(metrics["darbotdb_search_consolidation_time"]["foo1"], 0);
      assertEqual(metrics["darbotdb_search_consolidation_time"]["foo2"], 0);
      assertEqual(metrics["darbotdb_search_consolidation_time"]["foo3"], 0);

      assertEqual(metrics["darbotdb_search_num_failed_cleanups"]["foo1"], 0);
      assertEqual(metrics["darbotdb_search_num_failed_cleanups"]["foo2"], 0);
      assertEqual(metrics["darbotdb_search_num_failed_cleanups"]["foo3"], 0);

      assertEqual(metrics["darbotdb_search_num_failed_commits"]["foo1"], 0);
      assertEqual(metrics["darbotdb_search_num_failed_commits"]["foo2"], 0);
      assertEqual(metrics["darbotdb_search_num_failed_commits"]["foo3"], 0);

      assertEqual(metrics["darbotdb_search_num_failed_consolidations"]["foo1"], 0);
      assertEqual(metrics["darbotdb_search_num_failed_consolidations"]["foo2"], 0);
      assertEqual(metrics["darbotdb_search_num_failed_consolidations"]["foo3"], 0);
    },
