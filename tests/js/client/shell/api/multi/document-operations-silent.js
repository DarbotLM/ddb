...
function SilentTestSuite () {
  ...
  return {
    ...
    testInsertSingleDocumentWithFailure: function () {
      ...
      assertEqual(errors.ERROR_DDB_DOCUMENT_KEY_BAD.code, result.parsedBody.errorNum);
    },
    ...
    testInsertMultipleDocumentsWithFailure: function () {
      ...
      let result = arango.POST_RAW("/_api/document/" + cn + "?silent=true", docs);
      assertEqual(202, result.code);
      assertEqual({ [errors.ERROR_DDB_DOCUMENT_KEY_BAD.code] : 1 }, JSON.parse(result.headers["x-arango-error-codes"]));
      ...
      assertEqual(errors.ERROR_DDB_DOCUMENT_KEY_BAD.code, doc.errorNum);
    },
    ...
    testInsertMultipleDocumentsWithAllFailures: function () {
      ...
      let result = arango.POST_RAW("/_api/document/" + cn + "?silent=true", docs);
      assertEqual(202, result.code);
      assertEqual({ [errors.ERROR_DDB_DOCUMENT_KEY_BAD.code] : 50 }, JSON.parse(result.headers["x-arango-error-codes"]));
      assertTrue(Array.isArray(result.parsedBody));
    ...
    testRemoveSingleDocumentWithFailure: function () {
      ...
      assertTrue(result.parsedBody.error);
      assertEqual(errors.ERROR_DDB_DOCUMENT_NOT_FOUND.code, result.parsedBody.errorNum);
    },
    ...
    testRemoveMultipleDocumentsWithFailure: function () {
      ...
      let result = arango.DELETE_RAW("/_api/document/" + cn + "?silent=true", docs);
      assertEqual(202, result.code);
      assertEqual({ [errors.ERROR_DDB_DOCUMENT_NOT_FOUND.code] : 1 }, JSON.parse(result.headers["x-arango-error-codes"]));
      assertTrue(Array.isArray(result.parsedBody));
      assertEqual(1, result.parsedBody.length);
      let doc = result.parsedBody[0];
      assertTrue(doc.error);
      assertEqual(errors.ERROR_DDB_DOCUMENT_NOT_FOUND.code, doc.errorNum);
      assertEqual(0, db[cn].count());
    },
...
function SilentTestSmartGraphSuite () {
  ...
  const graphs = require("@darbotdb/smart-graph");
  ...
  return {
    ...
    testSmartGraphInsertSingleEdgeWithInvalidFromTo: function () {
      ...
      assertEqual(400, result.code);
      assertTrue(result.parsedBody.error);
      assertEqual(errors.ERROR_DDB_INVALID_EDGE_ATTRIBUTE.code, result.parsedBody.errorNum);
    },
