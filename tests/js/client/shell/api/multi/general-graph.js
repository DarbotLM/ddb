...
const testHelper = require('@darbotdb/test-helper');
  ...
  return {
    ...
    test_cannot_create_an_edge_with_non_strings_in_from_and_to: function () {
      ...
      assertEqual(
        ...
        internal.errors.ERROR_DDB_INVALID_EDGE_ATTRIBUTE.code
      );
    },
...
function check404CRUD(doc) {
  ...
  assertTrue(doc.parsedBody['error'], doc);
  assertEqual(doc.parsedBody['code'], internal.errors.ERROR_HTTP_NOT_FOUND.code, doc);
  assertEqual(doc.parsedBody['errorNum'], internal.errors.ERROR_DDB_DATA_SOURCE_NOT_FOUND.code);
...
function check400CRUD(doc) {
  ...
  assertEqual(doc.parsedBody['errorNum'], internal.errors.ERROR_DDB_INVALID_EDGE_ATTRIBUTE.code);
}
...
function check404(doc) {
  ...
  assertTrue(doc.parsedBody['error'], doc);
  assertEqual(doc.parsedBody['code'], internal.errors.ERROR_HTTP_NOT_FOUND.code, doc);
  assertEqual(doc.parsedBody['errorNum'], internal.errors.ERROR_DDB_DOCUMENT_NOT_FOUND.code, doc);
  assertMatch(/.*document not found.*/, doc.parsedBody['errorMessage'], doc);
}
