...
function error_handlingSuite () {
  ...
  return {
    ...
    test_use_non_system_database: function() {
      ...
      assertEqual(response["errorNum"], internal.errors.ERROR_DDB_USE_SYSTEM_DATABASE.code);
    },
    ...
    test_use_non_existent_database: function() {
      ...
      let response = doc.parsedBody;
      assertTrue(response["error"]);
      assertEqual(response["errorNum"], internal.errors.ERROR_DDB_DATABASE_NOT_FOUND.code);
    },
