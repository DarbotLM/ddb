...
const { deriveTestSuite } = require('@darbotdb/test-helper');
const {
...
function AbortLongRunningOperationsWhenCollectionIsDroppedSuite() {
  ...
  deriveTestSuite(BaseTestConfig(dropCb, ERRORS.ERROR_DDB_DATA_SOURCE_NOT_FOUND.code), suite, '_collection');
  return suite;
}
...
function AbortLongRunningOperationsWhenDatabaseIsDroppedSuite() {
  ...
  deriveTestSuite(BaseTestConfig(dropCb, ERRORS.ERROR_DDB_DATABASE_NOT_FOUND.code), suite, '_database');
  return suite;
}
