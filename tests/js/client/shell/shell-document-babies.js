...
function CollectionDocumentSuiteBabies() {
  ...
  return {
    ...
    testInsertErrorUniqueConstraint: function () {
      ...
      assertEqual(docs[1].errorNum, ERRORS.ERROR_DDB_UNIQUE_CONSTRAINT_VIOLATED.code);
      ...
      assertEqual(docs[0].errorNum, ERRORS.ERROR_DDB_UNIQUE_CONSTRAINT_VIOLATED.code);
    ...
    testInsertErrorBadKey: function () {
      ...
      l.forEach(function (k) {
        ...
        assertTrue(docs[1].error);
        assertEqual(docs[1].errorNum, ERRORS.ERROR_DDB_DOCUMENT_KEY_BAD.code);
        collection.remove("a");
        assertEqual(collection.count(), 0);
      });
    },
    ...
    testErrorDatabaseMethodsNoBabies: function () {
      ...
      try {
        ...
        assertEqual(ERRORS.ERROR_DDB_DOCUMENT_TYPE_INVALID.code,
          err.errorNum);
      }
      try {
        db._update([cn + "/b"], [{
      ...
      try {
        ...
        assertEqual(ERRORS.ERROR_DDB_DOCUMENT_HANDLE_BAD.code,
          err.errorNum);
      }
    ...
    testReplaceMulti: function () {
      ...
      for (i = 0; i < result.length; i++) {
        ...
        assertEqual(result[i].errorNum, ERRORS.ERROR_DDB_CONFLICT.code);
      }
      assertEqual(collection.count(), 3);
    ...
    testReplaceMultiPrecondition: function () {
      ...
      for (var i = 0; i < docs3.length; ++i) {
        ...
        assertEqual(docs3[i].errorNum, ERRORS.ERROR_DDB_CONFLICT.code);
      }
      ...
      assertEqual(docs3[2].errorNum, ERRORS.ERROR_DDB_CONFLICT.code);
      ...
      assertEqual(docs3[0].errorNum, ERRORS.ERROR_DDB_CONFLICT.code);
      assertEqual(docs3[1].error, true);
      assertEqual(docs3[1].errorNum, ERRORS.ERROR_DDB_CONFLICT.code);
    ...
    testRemoveMultiPrecondition: function () {
      ...
      assertEqual(docs3[0].errorNum, ERRORS.ERROR_DDB_DOCUMENT_NOT_FOUND.code);
      assertTrue(docs3[1].error);
      assertEqual(docs3[1].errorNum, ERRORS.ERROR_DDB_DOCUMENT_NOT_FOUND.code);
    ...
    testDocumentMultiPrecondition: function () {
      ...
      for (var i = 0; i < docs3.length; ++i) {
        assertTrue(docs3[i].error);
        assertEqual(ERRORS.ERROR_DDB_CONFLICT.code, docs3[i].errorNum);
      }
      ...
      assertTrue(docs3[2].error);
      assertEqual(ERRORS.ERROR_DDB_CONFLICT.code, docs3[2].errorNum);
      ...
      assertEqual(docs3.length, 3);
      assertTrue(docs3[0].error);
      assertEqual(ERRORS.ERROR_DDB_CONFLICT.code, docs3[0].errorNum);
      assertEqual(docs3[1].value, 5);
    ...
    testBadArguments: function () {
      ...
      values.forEach(function (x) {
        ...
        assertEqual(docs[0].errorNum, ERRORS.ERROR_DDB_DOCUMENT_TYPE_INVALID.code);
      });
      ...
      values.forEach(function (x) {
        ...
        for (var i = 0; i < expectedLength; ++i) {
          ...
          assertEqual(docs[i].errorNum, ERRORS.ERROR_DDB_DOCUMENT_TYPE_INVALID.code);
        }
        ...
        if (typeof x !== "string") {
          try {
            ...
            for (i = 0; i < expectedLength; ++i) {
              assertEqual(docs[i].error, true);
              if (typeof x === "string") {
                assertEqual(docs[i].errorNum, ERRORS.ERROR_DDB_DOCUMENT_NOT_FOUND.code);
              } else {
                assertEqual(docs[i].errorNum, ERRORS.ERROR_DDB_DOCUMENT_HANDLE_BAD.code);
              }
            }
          } catch (err) {
