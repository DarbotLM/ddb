...
const fs = require('fs');
const pu = require('@darbotdb/testutils/process-utils');
...
function dumpIntegrationSuite() {
  ...
  const dbdump = pu.dbdump_BIN;

  assertTrue(fs.isFile(dbdump), "dbdump not found!");
  ...
  let runDump = function (path, args, expectRc) {
    ...
    const actualRc = executeExternalAndWaitWithSanitizer(dbdump, args, 'shell-dump-integration');
  ...
  let structureFile = function (path, cn, escapedName = undefined) {
    ...
    const prefix = escapedName + "_" + require("@darbotdb/crypto").md5(cn);
  ...
  return {
    ...
    testDumpOnlyOneShard: function () {
      ...
      const prefix = cn + "_" + require("@darbotdb/crypto").md5(cn);
