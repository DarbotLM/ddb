...
const request = require("@darbotdb/request");
const db = darbotdb.db;
const { getMetric, getDBServers, runParallelArangoshTests, waitForShardsInSync, versionHas } = require('@darbotdb/test-helper');
...
function DeadlockSuite() { 
  ...
  const cn = "UnitTests" + require("@darbotdb/crypto").md5(internal.genRandomAlphaNumbers(32)).substring(0, 8);
  ...
  return {
    ...
    testWorkInParallel: function () {
      let code = (testOpts) => {
        ...
        for (let op = 0; op < ops; ++op) {
          try {
            ...
            if (require("@darbotdb").errors.ERROR_CLUSTER_TIMEOUT.code === err.errorNum ||
                require("@darbotdb").errors.ERROR_CLUSTER_BACKEND_UNAVAILABLE === err.errorNum ||
                require("@darbotdb").errors.ERROR_CLUSTER_CONNECTION_LOST === err.errorNum ||
                require("@darbotdb").errors.ERROR_LOCK_TIMEOUT.code === err.errorNum ||
                require("@darbotdb").errors.ERROR_QUERY_PARSE === err.errorNum ||
                require("@darbotdb").errors.ERROR_QUERY_EMPTY === err.errorNum ||
                require("@darbotdb").errors.ERROR_QUERY_COMPILE_TIME_OPTIONS === err.errorNum ||
                require("@darbotdb").errors.ERROR_QUERY_ACCESS_AFTER_MODIFICATION === err.errorNum ||
                require("@darbotdb").errors.ERROR_QUERY_BIND_PARAMETERS_INVALID === err.errorNum ||
                require("@darbotdb").errors.ERROR_QUERY_BIND_PARAMETER_MISSING === err.errorNum ||
                require("@darbotdb").errors.ERROR_QUERY_BIND_PARAMETER_UNDECLARED === err.errorNum ||
                require("@darbotdb").errors.ERROR_QUERY_BIND_PARAMETER_TYPE === err.errorNum ||
                require("@darbotdb").errors.ERROR_QUEUE_FULL === err.errorNum) {
              throw err;
            }
          }
        }
        ...
        while (trx) {
          try {
            ...
            if (require("@darbotdb").errors.ERROR_TRANSACTION_NOT_FOUND.code === e.errorNum) {
              log(`aborting trx abort/commit loop because trx was not found`);
              break;
            }
            // due to contention we could have a lock timeout here
            if (require("@darbotdb").errors.ERROR_LOCKED.code !== e.errorNum) {
      ...
      getDBServers().forEach((s) => {
        droppedFollowersBefore[s.id] = getMetric(s.url, "darbotdb_dropped_followers_total");
      });
      ...
      getDBServers().forEach((s) => {
        droppedFollowersAfter[s.id] = getMetric(s.url, "darbotdb_dropped_followers_total");
      });
