...
// /
/// @author Jan Steemann
/// @author Copyright 2022, darbotdb GmbH, Cologne, Germany
// //////////////////////////////////////////////////////////////////////////////
...
const getCoordinatorEndpoints = require('@darbotdb/test-helper').getCoordinatorEndpoints;
...
function FoxxQueuesSuite () {
  ...
  return {
    ...
    setUpAll: function() {
      ...
      // create queues
      let req, result;
      req = `require("@darbotdb/foxx/queues").create("${qn}"); return "ok";`;
    ...
    testPushIntoDBQueuesOnNonFoxxmaster: function() {
      ...
      // create queue jobs
      for (let i = 0; i < 5; ++i) {
        req = `require("@darbotdb/foxx/queues").get("${qn}").push({ name: 'nonmastertest${i}', mount: '/nonmastertest${i}' }, {}, { delay: 1 }); return "ok";`;
    ...
    testPushIntoDBQueuesOnTwoCoordinators: function() {
      ...
      let usePrimary = true;
      for (let i = 0; i < 25; ++i) {
        req = `require("@darbotdb/foxx/queues").get("${qn}").push({ name: 'multitest${i}', mount: '/multitest${i}' }, {}, { delay: 1 }); return "ok";`;
