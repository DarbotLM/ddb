...
using WeightCallback = std::function<double(double originalWeight,
                                            darbotdb::velocypack::Slice edge)>;
...
class MockGraphProvider {
  using VertexType = darbotdb::velocypack::HashedStringRef;
 ...
 public:
  ...
  class Step : public darbotdb::graph::BaseStep<Step> {
   public:
    using EdgeType = darbotdb::velocypack::HashedStringRef;
    using VertexType = darbotdb::velocypack::HashedStringRef;
    ...
    class Edge {
     public:
      Edge(MockEdgeType e) : _edge(e) {
        ...
        _idRef = darbotdb::velocypack::HashedStringRef{
      ...
      darbotdb::velocypack::HashedStringRef const& getID() const {
        return _idRef;
      }
    ...
    darbotdb::velocypack::HashedStringRef getEdgeIdentifier() const {
      return _edge.getID();
    }
    ...
    std::string getCollectionName() const {
      ...
      if (collectionNameResult.fail()) {
        THROW_DDB_EXCEPTION(collectionNameResult.result());
      }
  ...
  MockGraphProvider(darbotdb::aql::QueryContext& queryContext, Options opts,
  ...
  darbotdb::graph::EdgeDocumentToken getEdgeDocumentToken(
  ...
  void prepareIndexExpressions(darbotdb::aql::Ast* ast);
  void prepareContext(darbotdb::aql::InputAqlItemRow input);
  ...
  darbotdb::aql::TraversalStats stealStats();
 ...
 private:
  ...
  darbotdb::transaction::Methods _trx;
  bool _reverse;
  LooseEndBehaviour _looseEnds;
  darbotdb::aql::TraversalStats _stats;
  // Optional callback to compute the weight of an edge.
  std::optional<WeightCallback> _weightCallback;
};
}  // namespace graph
