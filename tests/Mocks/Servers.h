...
class MockServer {
 public:
  ...
  MockServer(darbotdb::ServerState::RoleEnum =
                 darbotdb::ServerState::RoleEnum::ROLE_SINGLE,
 ...
 protected:
  darbotdb::application_features::ApplicationServer::State
      _oldApplicationServerState = darbotdb::application_features::
          ApplicationServer::State::UNINITIALIZED;
  darbotdb::ArangodServer _server;
  std::unique_ptr<StorageEngineMock> _engine;
  std::unordered_map<darbotdb::application_features::ApplicationFeature*, bool>
      _features;
  std::string _testFilesystemPath;
  darbotdb::RebootId _oldRebootId;

 private:
  bool _started;
  darbotdb::ServerState::RoleEnum _oldRole;
...
class MockAqlServer
 ...
 public:
  ...
  std::shared_ptr<darbotdb::transaction::Methods> createFakeTransaction() const;
  // runBeforePrepare gives an entry point to modify the list of collections one
  // want to use within the Query.
  std::shared_ptr<darbotdb::aql::Query> createFakeQuery(
      bool activateTracing = false, std::string queryString = "",
      std::function<void(darbotdb::aql::Query&)> runBeforePrepare =
          [](darbotdb::aql::Query&) {}) const;
  std::shared_ptr<darbotdb::aql::Query> createFakeQuery(
...
class MockClusterServer
 ...
 protected:
  MockClusterServer(bool useAgencyMockConnection,
                    darbotdb::ServerState::RoleEnum role, ServerID serverId,
                    bool injectClusterIndexes = false);
  ~MockClusterServer();
 ...
 protected:
  std::unique_ptr<darbotdb::network::ConnectionPool> _pool;
...
class MockCoordinator : public MockClusterServer {
 public:
  ...
  darbotdb::network::ConnectionPool* getPool();
};
