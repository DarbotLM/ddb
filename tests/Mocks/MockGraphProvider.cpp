...
MockGraphProvider::Step::Step(VertexType v, bool isProcessable)
    : darbotdb::graph::BaseStep<Step>{std::numeric_limits<size_t>::max(), 0,
                                      0.0},
      _vertex(v),
      _edge({}),
...
MockGraphProvider::Step::Step(size_t prev, VertexType v, MockEdgeType e,
                              bool isProcessable)
    : darbotdb::graph::BaseStep<Step>{prev},
      _vertex(v),
      _edge(e),
      _isProcessable(isProcessable) {}
...
MockGraphProvider::Step::Step(size_t prev, VertexType v, bool isProcessable,
                              size_t depth)
    : darbotdb::graph::BaseStep<Step>{prev, depth},
      _vertex(v),
      _edge({}),
      _isProcessable(isProcessable) {}
...
MockGraphProvider::Step::Step(size_t prev, VertexType v, bool isProcessable,
    ...
    : darbotdb::graph::BaseStep<Step>{prev, depth, weight},
      _vertex(v),
      _edge({}),
      _isProcessable(isProcessable) {}
...
MockGraphProvider::MockGraphProvider(darbotdb::aql::QueryContext& queryContext,
                                     MockGraphProviderOptions opts,
                                     darbotdb::ResourceMonitor&)
...
auto MockGraphProvider::addVertexToBuilder(
    const Step::Vertex& vertex, darbotdb::velocypack::Builder& builder)
    -> void {
...
void MockGraphProvider::addEdgeToLookupMap(
    typename Step::Edge const& edge, darbotdb::velocypack::Builder& builder) {
