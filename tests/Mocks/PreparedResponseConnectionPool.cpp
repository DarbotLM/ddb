...
bool IsRestVerbEqual(fuerte::RestVerb const& fuerteVerb,
                     darbotdb::rest::RequestType const& arangoVerb) {
  auto adb = std::string(requestToString(arangoVerb));
  auto fue = darbotdb::fuerte::from_string(adb);
...
PreparedRequestResponse::PreparedRequestResponse(TRI_vocbase_t& vocbase)
    : _vocbase{vocbase},
      ...
      _type(darbotdb::rest::RequestType::GET),
      _suffixes{},
      _payload{nullptr},
      _response{nullptr} {}

void PreparedRequestResponse::setRequestType(darbotdb::rest::RequestType type) {
  _type = type;
}
...
std::unique_ptr<fuerte::Response> PreparedRequestResponse::generateResponse()
  ...
  // TODO use code
  header.responseCode = darbotdb::fuerte::StatusOK;
...
PreparedResponseConnectionPool::PreparedResponseConnectionPool(
    darbotdb::AgencyCache& agencyCache, ConnectionPool::Config const& config)
