...
namespace {
darbotdb::AgencyOperation IncreaseVersion() {
  return darbotdb::AgencyOperation{
      "Plan/Version", darbotdb::AgencySimpleOperationType::INCREMENT_OP};
}
...
darbotdb::AgencyOperation CreateCollectionOrder(std::string_view dbName,
                                                std::string_view collection,
                                                VPackSlice info) {
#ifdef darbotdb_ENABLE_MAINTAINER_MODE
  if (!info.get("shards").isEmptyObject() &&
      !darbotdb::basics::VelocyPackHelper::getBooleanValue(
          info, darbotdb::StaticStrings::IsSmart, false)) {
    TRI_ASSERT(info.hasKey(darbotdb::StaticStrings::AttrIsBuilding));
    TRI_ASSERT(info.get(darbotdb::StaticStrings::AttrIsBuilding).isBool());
    TRI_ASSERT(info.get(darbotdb::StaticStrings::AttrIsBuilding).getBool() ==
               true);
  }
#endif
  return darbotdb::AgencyOperation{collectionPath(dbName, collection),
                                   darbotdb::AgencyValueOperationType::SET,
                                   info};
}

darbotdb::AgencyPrecondition CreateCollectionOrderPrecondition(
    std::string_view dbName, std::string_view collection, VPackSlice info) {
  return darbotdb::AgencyPrecondition{collectionPath(dbName, collection),
                                      darbotdb::AgencyPrecondition::Type::VALUE,
                                      info};
}

darbotdb::AgencyOperation CreateCollectionSuccess(std::string_view dbName,
                                                  std::string_view collection,
                                                  VPackSlice info) {
  TRI_ASSERT(!info.hasKey(darbotdb::StaticStrings::AttrIsBuilding));
  return darbotdb::AgencyOperation{collectionPath(dbName, collection),
...
auto ClusterInfo::deleteReplicatedStates(
  ...
  auto replicatedStateMethods =
      darbotdb::replication2::ReplicatedLogMethods::createInstance(databaseName,
                                                                   _server);
...
////////////////////////////////////////////////////////////////////////////////
/// @brief create collection in coordinator, the return value is an darbotdb
...
Result ClusterInfo::createCollectionsCoordinator(
    ...
    replication::Version replicationVersion) {
  TRI_ASSERT(ServerState::instance()->isCoordinator());
  using darbotdb::velocypack::Slice;
      ...
      if (result.isObject() &&
        ...
        for (auto const& p : VPackObjectIterator(result)) {
          ...
          if (darbotdb::basics::VelocyPackHelper::getBooleanValue(
            ...
            tmpError += darbotdb::basics::VelocyPackHelper::getStringValue(
