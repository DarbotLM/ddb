...
### Testsuites
Testsuites live in `js/client/modules/@darbotdb/testsuites/`. This folder is spidered during the startup of `testing.js` and all files in it are loaded automatically.
...
since spawning an arangosh requires way less resources than spawning a full darbotdb setup.
...
The dump family of testsuites lives in `js/client/modules/@darbotdb/testsuites/dump.js`
...
- jobProperties - options to the suites on circle-ci in this file - used for driver tests
  ...
  - second_repo: a secondary (driver) repo is cloned next to the darbotdb repo
