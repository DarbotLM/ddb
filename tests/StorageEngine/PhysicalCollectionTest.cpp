...
class PhysicalCollectionTest
    : public ::testing::Test,
      darbotdb::tests::LogSuppressor<darbotdb::Logger::AUTHENTICATION,
                                     darbotdb::LogLevel::WARN> {
 protected:
  darbotdb::ArangodServer server;
  StorageEngineMock engine;
  std::vector<std::reference_wrapper<
      darbotdb::application_features::ApplicationFeature>>
      features;
  ...
  PhysicalCollectionTest() : server(nullptr, nullptr), engine(server) {
    ...
    features.emplace_back(
        server.addFeature<
            darbotdb::AuthenticationFeature>());  // required for VocbaseContext
...
TEST_F(PhysicalCollectionTest, test_new_object_for_insert) {
  ...
  auto json = darbotdb::velocypack::Parser::fromJson("{ \"name\": \"test\" }");
...
class MockIndex : public Index {
 public:
  MockIndex(
      Index::IndexType type, bool needsReversal, darbotdb::IndexId id,
...
TEST_F(PhysicalCollectionTest, test_index_ordeing) {
  ...
  auto json = darbotdb::velocypack::Parser::fromJson("{ \"name\": \"test\" }");
  auto collection = vocbase.createCollection(json->slice());
  std::vector<std::vector<darbotdb::basics::AttributeName>> dummyFields;
