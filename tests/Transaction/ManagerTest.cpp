...
class TransactionManagerTest : public ::testing::Test {
 protected:
  darbotdb::tests::mocks::TransactionManagerSetup setup;
...
TEST_F(TransactionManagerTest, parsing_errors) {
  auto json = darbotdb::velocypack::Parser::fromJson("{ \"write\": [33] }");
  Result res =
  ...
  json = darbotdb::velocypack::Parser::fromJson(
...
TEST_F(TransactionManagerTest, collection_not_found) {
  darbotdb::ExecContextSuperuserScope exeScope;
  ...
  Result res =
      mgr->ensureManagedTrx(vocbase, tid, json->slice(),
                            transaction::OperationOriginTestCase{}, false)
          .waitAndGet();
  EXPECT_EQ(res.errorNumber(), TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND);
...
TEST_F(TransactionManagerTest, simple_transaction_and_abort) {
  ...
  auto doc = darbotdb::velocypack::Parser::fromJson("{ \"_key\": \"1\"}");
  {
    auto ctx =
...
TEST_F(TransactionManagerTest, simple_transaction_and_commit) {
...

  {
    ...
    auto doc = darbotdb::velocypack::Parser::fromJson("{ \"abc\": 1}");
...
TEST_F(TransactionManagerTest, aql_standalone_transaction) {
  ...
  auto ctx = std::make_shared<transaction::AQLStandaloneContext>(
      vocbase, tid,
      darbotdb::transaction::OperationOriginAQL{"running AQL query"});
  auto origin = ctx->operationOrigin();
  ...
  darbotdb::aql::QueryResult qres = executeQuery(vocbase, qq, ctx);
  ASSERT_TRUE(qres.ok());
...
TEST_F(TransactionManagerTest, permission_denied_readonly) {
  ...
  struct ExecContext : public darbotdb::ExecContext {
    ExecContext()
        : darbotdb::ExecContext(darbotdb::ExecContext::ConstructorToken{},
                                darbotdb::ExecContext::Type::Internal, "dummy",
                                "testVocbase", darbotdb::auth::Level::RO,
  ...
  res = mgr->ensureManagedTrx(vocbase, tid, json->slice(),
                              transaction::OperationOriginTestCase{}, false)
            .waitAndGet();
  ASSERT_EQ(res.errorNumber(), TRI_ERROR_DDB_READ_ONLY);
}
...
TEST_F(TransactionManagerTest, lock_usage_of_expired_transaction) {
  ...
  auto json2 = darbotdb::velocypack::Parser::fromJson(
      "{ \"collections\":{\"write\": [\"testCollection\"]}}");
