...
class SpliceSubqueryNodeOptimizerRuleTest : public ::testing::Test {
 protected:
  ...
  void verifySubquerySplicing(std::string const& querystring,
    ...
    auto splicedQuery = darbotdb::aql::Query::create(
        std::move(ctx), darbotdb::aql::QueryString(querystring), bindParamVpack,
        darbotdb::aql::QueryOptions(ruleOptions(additionalOptions)->slice()));
    waitForAsync(splicedQuery->prepareQuery());
    ASSERT_EQ(queryRegistry->numberRegisteredQueries(), 0)
        << "query string: " << querystring;

    auto splicedPlan =
        const_cast<darbotdb::aql::ExecutionPlan*>(splicedQuery->plan());
...
TEST_F(SpliceSubqueryNodeOptimizerRuleTest, splice_subquery_no_subquery_plan) {
  ...
  auto expected = darbotdb::velocypack::Parser::fromJson(R"([15])");
...
TEST_F(SpliceSubqueryNodeOptimizerRuleTest, splice_subquery_single_input) {
  ...
  auto expected = darbotdb::velocypack::Parser::fromJson(R"([[1]])");
  verifyQueryResult(query, expected->slice());
}
...
TEST_F(SpliceSubqueryNodeOptimizerRuleTest, splice_subquery_plan) {
  ...
  auto expected = darbotdb::velocypack::Parser::fromJson(R"([[1],[2]])");
  verifyQueryResult(query, expected->slice());
}
...
TEST_F(SpliceSubqueryNodeOptimizerRuleTest, splice_subquery_in_subquery_plan) {
  ...
  auto expected = darbotdb::velocypack::Parser::fromJson(R"([
    [[1, [1,2]]],
    [[2, [1,2]]]
  ])");
  verifyQueryResult(query, expected->slice());
...
TEST_F(SpliceSubqueryNodeOptimizerRuleTest,
  ...
  auto expected =
      darbotdb::velocypack::Parser::fromJson(R"([[[1],[2]],[[2],[1]]])");
  verifyQueryResult(query, expected->slice());
}
...
TEST_F(SpliceSubqueryNodeOptimizerRuleTest, splice_subquery_with_sort) {
  ...
  auto expected = darbotdb::velocypack::Parser::fromJson(
...
TEST_F(SpliceSubqueryNodeOptimizerRuleTest,
  ...
  auto expected = darbotdb::velocypack::Parser::fromJson(expectedString);
...
TEST_F(SpliceSubqueryNodeOptimizerRuleTest,
  ...
  auto expected = darbotdb::velocypack::Parser::fromJson(R"([2, 3, 4])");
  verifyQueryResult(query, expected->slice());
}

// Regression test for https://github.com/darbotdb/darbotdb/issues/10852
...
// Regression test for https://github.com/darbotdb/darbotdb/issues/10896
...
TEST_F(SpliceSubqueryNodeOptimizerRuleTest,
  ...
  auto expected =
      darbotdb::velocypack::Parser::fromJson(R"([[1, []], [2, []]])");
  verifyQueryResult(query, expected->slice());
}
