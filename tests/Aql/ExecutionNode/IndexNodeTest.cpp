 ...
 protected:
  ...
  IndexNodeTest() : server(false) {
    ...
    darbotdb::ServerState::instance()->setRole(
        darbotdb::ServerState::ROLE_SINGLE);
...
darbotdb::CreateDatabaseInfo createInfo(darbotdb::ArangodServer& server) {
  darbotdb::CreateDatabaseInfo info(server, darbotdb::ExecContext::current());
...
darbotdb::aql::QueryResult executeQuery(
    const std::shared_ptr<darbotdb::transaction::Context>& ctx,
...
TEST_F(IndexNodeTest, objectQuery) {
  ...
  auto indexJson = darbotdb::velocypack::Parser::fromJson(
  ...
  darbotdb::transaction::Methods trx(
      ...
      EMPTY, {collection->name()}, EMPTY, darbotdb::transaction::Options());
  EXPECT_TRUE(trx.begin().ok());

  darbotdb::OperationOptions opt;
  auto jsonDocument = darbotdb::velocypack::Parser::fromJson(
...
TEST_F(IndexNodeTest, expansionQuery) {
  ...
  darbotdb::OperationOptions opt;
  auto jsonDocument0 = darbotdb::velocypack::Parser::fromJson(
      "{\"_key\": \"doc_0\", \"tags\": {\"hop\": [{\"foo\": {\"fo\": "
      "\"foo_val\"}, \"bar\": {\"br\": \"bar_val\"}, \"baz\": {\"bz\": "
      "\"baz_val_0\"}}]}}");
  auto jsonDocument1 = darbotdb::velocypack::Parser::fromJson(
...
TEST_F(IndexNodeTest, constructIndexNode) {
  ...
  ASSERT_TRUE(createdIndex);
  ASSERT_FALSE(!index);
  // auto jsonDocument = darbotdb::velocypack::Parser::fromJson("{\"obj\":
...

  {
    // short path for a test
    {
      ...
      for (auto const& v : {std::make_unique<darbotdb::aql::Variable>(
                                "d", 3, false, resourceMonitor),
                            std::make_unique<darbotdb::aql::Variable>(
    ...
    // deserialization
    darbotdb::aql::IndexNode indNode(
        const_cast<darbotdb::aql::ExecutionPlan*>(query->plan()),
        createJson->slice());
    ...
    // serialization and deserialization
    {
      ...
      static_cast<darbotdb::aql::ExecutionNode&>(indNode).toVelocyPack(
          builder, darbotdb::aql::ExecutionNode::SERIALIZE_DETAILS);

      darbotdb::aql::IndexNode indNodeDeserialized(
          const_cast<darbotdb::aql::ExecutionPlan*>(query->plan()),
          builder.slice());
    ...
    {  // without properties
      {
        auto indNodeClone =
            dynamic_cast<darbotdb::aql::IndexNode*>(indNode.clone(
                const_cast<darbotdb::aql::ExecutionPlan*>(query->plan()),
                true));
    ...
    // not materialized
    {
      indNode.setLateMaterialized(nullptr, darbotdb::IndexId::primary(),
                                  darbotdb::aql::IndexNode::IndexVarsInfo());
...
TEST_F(IndexNodeTest, invalidLateMaterializedJSON) {
  ...
  auto const& v =
      std::make_unique<darbotdb::aql::Variable>("5", 6, false, resourceMonitor);
  if (vars->getVariable(v->id) == nullptr) {
    vars->createVariable(v.get());
  }
