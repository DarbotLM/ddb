...
TEST_F(InsertExecutorTest, insert_but_not_rev) {
  ...
  auto doc = slice.at(0);
  ASSERT_NE(darbotdb::basics::VelocyPackHelper::getStringValue(doc, "_rev", ""),
            invalidRev);
}
...
TEST_P(InsertExecutorTestCount, insert_with_key_with_return) {
  ...
  auto result = darbotdb::tests::executeQuery(vocbase, query, bindParameters);
...
TEST_F(InsertExecutorTest, insert_return_old) {
  ...
  AssertQueryFailsWith(vocbase, query, TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND);
}
