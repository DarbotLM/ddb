...
struct AsyncExecutorTest : AqlExecutorTestCase<false> {
 ...
 protected:
  MutexTestSetup create_mutex_consumers(
      darbotdb::aql::AqlItemBlockManager& blockManager, size_t nrOfInputRows,
    ...
    std::deque<darbotdb::aql::SharedAqlItemBlockPtr> blockDeque;
...
// Regression test for https://darbotdb.atlassian.net/browse/BTS-1325.
// See https://github.com/darbotdb/darbotdb/pull/18729 for details.
...
TEST_F(AsyncExecutorTest, AsyncNode_does_not_return_stored_WAITING) {
  ...
  darbotdb::ResourceMonitor monitor(globalMonitor);
  darbotdb::aql::AqlItemBlockManager blockManager(monitor);
