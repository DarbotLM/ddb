...
auto AqlExecutorTestCase<enableQueryTrace>::generateNodeDummy(
  ...
  auto dummy = std::make_unique<MockTypedNode>(
      const_cast<darbotdb::aql::ExecutionPlan*>(fakedQuery->plan()),
...
auto AqlExecutorTestCase<enableQueryTrace>::query() const
    -> std::shared_ptr<darbotdb::aql::Query> {
  return fakedQuery;
}

template class ::darbotdb::tests::aql::AqlExecutorTestCase<true>;
template class ::darbotdb::tests::aql::AqlExecutorTestCase<false>;
