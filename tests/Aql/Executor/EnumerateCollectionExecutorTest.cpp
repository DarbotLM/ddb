...
using CursorType = darbotdb::transaction::Methods::CursorType;
...
class EnumerateCollectionExecutorTest : public AqlExecutorTestCase<false> {
 protected:
  ...
  darbotdb::aql::Projections const projections;
  bool random;
  bool count;
  ...
  EnumerateCollectionExecutorTest()
      : AqlExecutorTestCase(),
        ...
        aqlCollection("UnitTestCollection", &vocbase,
                      ...
                      darbotdb::aql::Collection::Hint::Collection),
        random(false),
        count(false),
...
class EnumerateCollectionExecutorTestProduce
 ...
 protected:
  ...
  VPackOptions const* insertDocuments(size_t amount,
    ...
    SCOPED_TRACE(insertQuery);
    auto queryRes = darbotdb::tests::executeQuery(vocbase, insertQuery, {});
    ...
    for (size_t i = 2; i <= amount; i++) {
      ...
      auto queryResInner =
          darbotdb::tests::executeQuery(vocbase, finalQuery, {});
