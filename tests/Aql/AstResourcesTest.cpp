...
TEST(ShortStringStorageTest, testEmpty) {
  ...
  darbotdb::aql::ShortStringStorage sss(resourceMonitor, /*block size*/ 4096);
...
TEST(ShortStringStorageTest, testFillAndClear) {
  ...
  darbotdb::GlobalResourceMonitor global;
  darbotdb::ResourceMonitor resourceMonitor(global);
  darbotdb::aql::ShortStringStorage sss(resourceMonitor,
...
TEST(AstResourcesTest, testLongStringsFillAndClear) {
  darbotdb::GlobalResourceMonitor global;
  darbotdb::ResourceMonitor resourceMonitor(global);
  darbotdb::aql::AstResources resources(resourceMonitor);

  constexpr size_t overheadPerString =
      darbotdb::aql::AstResources::memoryUsageForStringBlock();
  ...
  ASSERT_GT(payload.size(), darbotdb::aql::ShortStringStorage::maxStringLength);
  ...
  for (size_t i = 0; i < 10; ++i) {
    if (i + 1 > capacity) {
      size_t newCapacity =
          std::max(darbotdb::aql::AstResources::kMinCapacityForLongStrings,
                   capacity * 2);
