...
void darbotdb::tests::aql::AssertQueryResultToSlice(QueryResult const& result,
                                                    VPackSlice expected) {
...
void darbotdb::tests::aql::AssertQueryHasResult(TRI_vocbase_t& database,
                                                std::string const& query,
                                                VPackSlice expected) {
  auto const bindParameters = VPackParser::fromJson("{ }");
  SCOPED_TRACE("Query: " + query);
  auto queryResult =
      darbotdb::tests::executeQuery(database, query, bindParameters);
  AssertQueryResultToSlice(queryResult, expected);
}

void darbotdb::tests::aql::AssertQueryFailsWith(TRI_vocbase_t& database,
                                                std::string const& query,
