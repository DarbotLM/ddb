...
class SortLimitTest
 ...
 protected:
  ...
  std::vector<darbotdb::velocypack::Builder> insertedDocs;
  ...
  auto buildOptions(std::string rules)
      -> std::shared_ptr<darbotdb::velocypack::Builder> {
    if (doFullCount()) {
      return darbotdb::velocypack::Parser::fromJson(
  ...
  void verifyExpectedResults(TRI_vocbase_t& vocbase,
    ...
    for (darbotdb::velocypack::ArrayIterator itr(slice); itr.valid(); ++itr) {
      ...
      EXPECT_EQ(0, darbotdb::basics::VelocyPackHelper::compare(
    ...
    auto actualFullCount =
        darbotdb::basics::VelocyPackHelper::getNumericValue<size_t>(
  ...
  void CreateCollection() {
    ...
    for (size_t i = 0; i < total; i++) {
      docs.emplace_back(darbotdb::velocypack::Parser::fromJson(
