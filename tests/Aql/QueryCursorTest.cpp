...
namespace {
darbotdb::velocypack::SharedSlice operator"" _vpack(const char* json,
                                                    size_t len) {
...
TEST_F(QueryCursorTest, resultCursorResultArrayIndexSingleBatch) {
  ...
  fakeRequest->setRequestType(darbotdb::rest::RequestType::POST);
  ...
  auto* registry = darbotdb::QueryRegistryFeature::registry();

  auto testee = std::make_shared<darbotdb::RestCursorHandler>(
...
TEST_F(QueryCursorTest, streamingCursorResultArrayIndexTwoBatches) {
  ...
  {  // release the query, so the AQL feature doesn't wait on it during shutdown
    ...
    auto fakeRequest = std::make_unique<GeneralRequestMock>(vocbase);
    fakeRequest->setRequestType(darbotdb::rest::RequestType::DELETE_REQ);
    fakeRequest->addSuffix(idSlice.copyString());
    auto fakeResponse = std::make_unique<GeneralResponseMock>();
    auto restHandler = std::make_shared<darbotdb::RestCursorHandler>(
        server->server(), fakeRequest.release(), fakeResponse.release(),
        registry);
