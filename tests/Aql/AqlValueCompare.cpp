...
using AqlValue = darbotdb::aql::AqlValue;

template<typename T>
AqlValue makeAQLValue(T t) {
  return AqlValue(darbotdb::aql::AqlValueHintNull());
}

AqlValue makeAQLValue(int64_t x) {
  return AqlValue(darbotdb::aql::AqlValueHintInt(x));
}

AqlValue makeAQLValue(uint64_t x) {
  return AqlValue(darbotdb::aql::AqlValueHintUInt(x));
}
...
static inline int AqlValueComp(AqlValue& a, AqlValue& b) {
  darbotdb::velocypack::Options opt;
