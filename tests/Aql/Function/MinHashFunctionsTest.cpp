...
std::vector<AqlValue> buildArgs(char const* args) {
  ...
  auto const argsJson = darbotdb::velocypack::Parser::fromJson(args);
...
void assertFuncThrow(char const* args, darbotdb::aql::Function const& f) {
  ASSERT_NE(nullptr, args);

  auto params = buildArgs(args);

  auto cleanup = darbotdb::scopeGuard([&params]() noexcept {
    for (auto& p : params) {
      p.destroy();
    }
  });
...
void assertMinHashErrorThrow(char const* args) {
  darbotdb::aql::Function f("MINHASH_ERROR", &functions::MinHashError);
  assertFuncThrow(args, f);
}

void assertMinHashCountThrow(char const* args) {
  darbotdb::aql::Function f("MINHASH_COUNT", &functions::MinHashCount);
  assertFuncThrow(args, f);
}

void assertMinHashThrow(char const* args) {
  darbotdb::aql::Function f("MINHASH", &functions::MinHash);
  assertFuncThrow(args, f);
}

void assertMinHashMatchThrow(char const* args) {
  darbotdb::aql::Function f("MINHASH_MATCH", &functions::MinHash);
  assertFuncThrow(args, f);
}
