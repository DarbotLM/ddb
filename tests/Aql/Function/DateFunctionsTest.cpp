...
struct TestDate {
 public:
  TestDate(std::string const json, bool v) : _date(nullptr), _isValid(v) {
    ...
    _date = darbotdb::velocypack::Parser::fromJson(json);
  }
 ...
 private:
  std::shared_ptr<darbotdb::velocypack::Builder> _date;
  bool _isValid;
};
...
#include "IS_DATESTRING.testcases"
  ...
  darbotdb::aql::Function fun("IS_DATESTRING", &functions::IsDatestring);
...
struct TestDate {
 ...
 private:
  darbotdb::velocypack::Builder _argBuilder;
  bool _isValid;
};
...
#include "DATE_COMPARE.testcases"
  ...
  darbotdb::aql::Function fun("DATE_COMPARE", &functions::DateCompare);
  darbotdb::aql::AstNode node(NODE_TYPE_FCALL);
  node.setData(static_cast<void const*>(&fun));
...
class DateFunctionsTestDateDiff : public ::testing::Test {
 protected:
  ...
  void testCombinations(std::string const& f, double expected) {
    darbotdb::aql::Function fun("DATE_DIFF", &functions::DateDiff);
    darbotdb::aql::AstNode node(NODE_TYPE_FCALL);
    node.setData(static_cast<void const*>(&fun));
...
struct TestDate {
 public:
  TestDate(std::string const& json, std::string const& v)
    ...
    // Make sure to only insert valid JSON.
    // We are not testing the parser here.
    _input = darbotdb::velocypack::Parser::fromJson(json);
  }
 ...
 private:
  std::shared_ptr<darbotdb::velocypack::Builder> _input;
...
#include "DATE_SUBTRACT.testcases"
  ...
  darbotdb::aql::Function fun("DATE_SUBTRACT", &functions::DateSubtract);
  darbotdb::aql::AstNode node(NODE_TYPE_FCALL);
  node.setData(static_cast<void const*>(&fun));
