...
AqlValue evaluate(AqlValue const& lhs, AqlValue const& rhs) {
  ...
  darbotdb::aql::Function f("JACCARD", &functions::Jaccard);
...
AqlValue evaluate(char const* lhs, char const* rhs) {
  auto const lhsJson = darbotdb::velocypack::Parser::fromJson(lhs);
  auto const rhsJson = darbotdb::velocypack::Parser::fromJson(rhs);
