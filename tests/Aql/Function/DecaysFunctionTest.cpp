...
TEST(GaussDecayFunctionTest, test) {
  ...
  darbotdb::aql::Function f("DECAY_GAUSS", &functions::DecayGauss);
...
TEST(ExpDecayFunctionTest, test) {
  // preparing
  darbotdb::aql::AstNode node(NODE_TYPE_FCALL);
  darbotdb::aql::Function f("DECAY_EXP", &functions::DecayExp);
  node.setData(static_cast<void const*>(&f));
...
TEST(LinDecayFunctionTest, test) {
  // preparing
  darbotdb::aql::AstNode node(NODE_TYPE_FCALL);
  darbotdb::aql::Function f("DECAY_LINEAR", &functions::DecayLinear);
  node.setData(static_cast<void const*>(&f));
