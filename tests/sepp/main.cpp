...
void addConfig(darbotdb::velocypack::Builder& config, std::string_view param) {
  auto [path, v] = splitKeyValue(param);
  auto value = darbotdb::velocypack::Parser::fromJson(v.data(), v.size());
  ...
  darbotdb::velocypack::SliceContainer configSlice(config.slice());
  config.clear();
  darbotdb::velocypack::Collection::merge(config, configSlice.slice(),
                                          builder.slice(), true);
}
...
auto parseConfig(Options const& opts) {
  ...
  auto config = darbotdb::velocypack::Parser::fromJson(buffer.str());
...
int main(int argc, char const* argv[]) {
  ...
  try {
    ...
    darbotdb::sepp::Runner runner(argv[0], options.report, config->slice());
    runner.run();
