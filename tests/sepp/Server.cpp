...
struct Server::Impl {
  Impl(darbotdb::RocksDBOptionsProvider const& optionsProvider,
       darbotdb::CacheOptionsProvider const& cacheOptionsProvider,
 ...
 private:
  ...
  std::shared_ptr<darbotdb::options::ProgramOptions> _options;
...
Server::Impl::Impl(RocksDBOptionsProvider const& optionsProvider,
                   ...
                   std::string databaseDirectory)
    : _options(std::make_shared<darbotdb::options::ProgramOptions>("sepp", "",
                                                                   "", "")),
...
void Server::Impl::start(char const* exectuable) {
  ...
  using darbotdb::application_features::ApplicationServer;
...
void Server::Impl::runServer(char const* exectuable) {
  ...
  try {
    ...
    LOG_TOPIC("5d508", ERR, darbotdb::Logger::FIXME)
        << "sepp ArangodServer terminated because of an exception: "
        << ex.what();
  } catch (...) {
    LOG_TOPIC("3c63a", ERR, darbotdb::Logger::FIXME)
        << "sepp ArangodServer terminated because of an exception of "
           "unknown type";
  }
  darbotdb::Logger::flush();
}

Server::Server(darbotdb::RocksDBOptionsProvider const& optionsProvider,
               darbotdb::CacheOptionsProvider const& cacheOptionsProvider,
               std::string databaseDirectory)
