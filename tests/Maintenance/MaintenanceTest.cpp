...
using namespace darbotdb::maintenance;
...
class MaintenanceTestActionPhaseOne : public SharedMaintenanceTest {
 protected:
  int _dummy;
  std::shared_ptr<darbotdb::options::ProgramOptions> po;
  darbotdb::ArangodServer as;
  containers::FlatHashSet<DatabaseID> makeDirty;
  MaintenanceFeature::errors_t errors;

  std::map<std::string, NodePtr> localNodes;

  std::unique_ptr<darbotdb::RocksDBEngine>
  ...
  MaintenanceTestActionPhaseOne()
      : SharedMaintenanceTest(),
        ...
        po(std::make_shared<darbotdb::options::ProgramOptions>(
    ...
    as.addFeature<darbotdb::RocksDBOptionFeature>();
    as.addFeature<darbotdb::application_features::GreetingsFeaturePhase>(
        std::false_type{});
    auto& selector = as.addFeature<darbotdb::EngineSelectorFeature>();
    auto& metrics = as.addFeature<darbotdb::metrics::MetricsFeature>(
    ...
    // server
    engine = std::make_unique<darbotdb::RocksDBEngine>(
        as, as.template getFeature<darbotdb::RocksDBOptionFeature>(), metrics);
    selector.setEngineTesting(engine.get());
  }

  ~MaintenanceTestActionPhaseOne() {
    as.getFeature<darbotdb::EngineSelectorFeature>().setEngineTesting(nullptr);
  }
  ...
  auto changeInternalCollectionTypePlan(
    ...
    auto path = darbotdb::cluster::paths::aliases::plan()
                    ->collections()
                    ->database(dbName)
                    ->collection(planId);
    auto vec = path->vec(paths::SkipComponents(2));
  ...
  for (auto& node : localNodes) {
    ...
    darbotdb::maintenance::diffPlanLocal(
        *engine, planToChangeset(plan), 0, {}, 0, dirty,
        ...
        callNotify, actions, darbotdb::MaintenanceFeature::ShardActionMap{},
        ReplicatedLogStatusMapByDatabase{}, ShardIdToLogIdMapByDatabase{});
...
TEST_F(
  ...
  for (auto node : localNodes) {
    ...
    node.second = node.second->placeAt(
        "db3", darbotdb::velocypack::Slice::emptyObjectSlice());
  ...
  for (auto node : localNodes) {
    ...
    darbotdb::maintenance::diffPlanLocal(
        *engine, planToChangeset(plan), 0, {}, 0, dirty,
        ...
        actions, darbotdb::MaintenanceFeature::ShardActionMap{},
        ReplicatedLogStatusMapByDatabase{}, ShardIdToLogIdMapByDatabase{});
...
TEST_F(MaintenanceTestActionPhaseOne, remove_an_index_from_plan) {
  ...
  plan = plan->perform<POP>({"Collections", dbname, cid, indexes},
                            darbotdb::velocypack::Slice::emptyObjectSlice())
             .get();
