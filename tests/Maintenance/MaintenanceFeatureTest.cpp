...
/// @author Copyright 2018, darbotdb GmbH, Cologne, Germany
////////////////////////////////////////////////////////////////////////////////
...
static darbotdb::ServerID const DBSERVER_ID;
...
class TestActionBasic : public ActionBase {
 public:
  TestActionBasic(darbotdb::MaintenanceFeature& feature,
    ...
    if (description.get("preaction_result_code", value).ok()) {
      ...
      _preAction = std::make_shared<ActionDescription>(
          std::move(pred), darbotdb::maintenance::NORMAL_PRIORITY, false);
    }  // if
    ...
    if (description.get("postaction_result_code", value).ok()) {
      ...
      _postAction = std::make_shared<ActionDescription>(
          std::move(postd), darbotdb::maintenance::NORMAL_PRIORITY, false);
    }  // if
...
struct MaintenanceFeatureTestThreaded : ::testing::Test {
 ...
 public:
  ...
  darbotdb::ArangodServer as{po, nullptr};
...
TEST_F(MaintenanceFeatureTestThreaded, iterate_action_0_times_ok) {
  TestMaintenanceFeature& tf =
      as.addFeature<darbotdb::MaintenanceFeature, TestMaintenanceFeature>();
  tf.setSecondsActionsBlock(0);  // disable retry wait for now
                                 //
  std::thread th(&darbotdb::ArangodServer::run, &as, 0, nullptr);

  auto threadGuard = darbotdb::scopeGuard([&]() noexcept {
    as.beginShutdown();
    th.join();
  });

  tf.setMaintenanceThreadsMax(darbotdb::MaintenanceFeature::minThreadLimit);
  ...
  action_base_ptr.reset(new TestActionBasic(
      tf, ActionDescription(
              std::map<std::string, std::string>{{"name", "TestActionBasic"},
                                                 {"iterate_count", "0"}},
              darbotdb::maintenance::NORMAL_PRIORITY, false)));
  darbotdb::Result result =
...
TEST_F(MaintenanceFeatureTestThreaded,
  ...
  tf.setMaintenanceThreadsMax(darbotdb::MaintenanceFeature::minThreadLimit - 1);
...
TEST_F(MaintenanceFeatureTestThreaded,
  ...
  auto factory = [](darbotdb::MaintenanceFeature& f,
