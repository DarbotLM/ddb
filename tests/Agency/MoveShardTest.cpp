...
/// @author Copyright 2017, darbotdb GmbH, Cologne, Germany
////////////////////////////////////////////////////////////////////////////////
      ...
      createTestStructure = [&](velocypack::Slice s, std::string const& path) {
        ...
        if (s.isObject()) {
          ...
          if (path == "/arango/Target/ToDo") {
            builder->add(jobId,
                         createJob(COLLECTION, SHARD_LEADER, "unfug").slice());
          }
          builder->close();
        } else {
          builder->add(s);
  ...
  std::function<std::unique_ptr<VPackBuilder>(velocypack::Slice,
      ...
      createTestStructure = [&](velocypack::Slice s, std::string const& path) {
        ...
        if (s.isObject()) {
          ...
          if (path == "/arango/Target/ToDo") {
            builder->add(jobId,
                         createJob(COLLECTION, "unfug", FREE_SERVER).slice());
          }
          builder->close();
        } else {
          builder->add(s);
      ...
      createTestStructure = [&](velocypack::Slice s, std::string const& path) {
        ...
        if (s.isObject()) {
          ...
          if (path == "/arango/Target/ToDo") {
            builder->add(
                jobId,
                createJob(COLLECTION, FREE_SERVER, FREE_SERVER2).slice());
          }
          builder->close();
        } else {
          builder->add(s);
...
TEST_F(
  ...
  When(Method(mockAgent, write))
      .AlwaysDo([&](velocypack::Slice q,
        ...
        EXPECT_TRUE(writes
                        .get("/arango/Plan/Collections/" + DATABASE +
                             "/unrelatedcollection/shards/unrelatedshard")
                        .isNone());
        EXPECT_TRUE(
            writes.get("/arango/Supervision/Shards/" + SHARD).copyString() ==
            "1");
        EXPECT_TRUE(
            writes.get("/arango/Supervision/Shards/unrelatedshard").isNone());
        ...
        EXPECT_TRUE(
            std::string(writes
                            .get("/arango/Plan/Collections/" + DATABASE + "/" +
                                 COLLECTION + "/shards/" + SHARD)
                            .typeName()) == "array");
        // apparently we are not cleaning up our mess. this is done somewhere
        // else :S (>=2)
        EXPECT_TRUE(writes
                        .get("/arango/Plan/Collections/" + DATABASE + "/" +
                             COLLECTION + "/shards/" + SHARD)
  ...
  When(Method(mockAgent, write))
      .Do([&](velocypack::Slice q,
        ...
        EXPECT_TRUE(
            ...
            "object");
        EXPECT_EQ(
            writes.get("/arango/Target/ToDo/1").get("database").copyString(),
            DATABASE);
        EXPECT_EQ(
            writes.get("/arango/Target/ToDo/1").get("collection").copyString(),
            COLLECTION);
        EXPECT_EQ(writes.get("/arango/Target/ToDo/1").get("shard").copyString(),
                  SHARD);
        EXPECT_EQ(
            writes.get("/arango/Target/ToDo/1").get("fromServer").copyString(),
            SHARD_LEADER);
        EXPECT_EQ(
            writes.get("/arango/Target/ToDo/1").get("toServer").copyString(),
            SHARD_FOLLOWER1);
        EXPECT_TRUE(std::string(writes.get("/arango/Target/ToDo/1")
                                    .get("timeCreated")
                                    .typeName()) == "string");
...
TEST_F(MoveShardTest,
       it_should_be_possible_to_create_a_new_moveshard_job_within_an_envelope) {
  Mock<AgentInterface> mockAgent;
  AgentInterface& agent = mockAgent.get();
...
TEST_F(
    MoveShardTest,
    whenever_someone_tries_to_create_a_useless_job_it_should_be_created_in_failed) {
  Mock<AgentInterface> mockAgent;
  AgentInterface& agent = mockAgent.get();
  ...
  ASSERT_TRUE(
      std::string(builder->slice().get("/Target/Failed/1").typeName()) ==
      "object");
}
