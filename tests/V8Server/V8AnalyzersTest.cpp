...
class V8AnalyzerTest
 ...
 protected:
  ...
  V8AnalyzerTest() {
    darbotdb::tests::v8Init();  // one-time initialize V8
...
TEST_F(V8AnalyzerTest, test_instance_accessors) {
  ...
  darbotdb::iresearch::TRI_InitV8Analyzers(*v8g, isolate);
  ...
  // test name (authorised)
  {
    ...
    user.grantDatabase(
        vocbase.name(),
        darbotdb::auth::Level::RO);  // for system collections
  ...
  // test name (not authorised)
  {
    ...
    user.grantDatabase(
        vocbase.name(),
        darbotdb::auth::Level::NONE);  // for system collections
                                       // User::collectionAuthLevel(...) returns
                                       // database auth::Level
    userManager->setAuthInfo(userMap);  // set user map to avoid loading
                                        // configuration from system database
...
TEST_F(V8AnalyzerTest, test_manager_create) {
  ...
  {
    std::vector<v8::Local<v8::Value>> args = {
        TRI_V8_STD_STRING(isolate, darbotdb::StaticStrings::SystemDatabase +
                                       "::testAnalyzer2"),
        v8::True(isolate),
    };
    ...
    user.grantDatabase(
        vocbase.name(),
        darbotdb::auth::Level::RW);  // for system collections
                                     // User::collectionAuthLevel(...) returns
                                     // database auth::Level
    userManager->setAuthInfo(userMap);  // set user map to avoid loading
                                        // configuration from system database
  ...
  // successful creation
  {
    ...
    auto* v8AnalyzerWeak = TRI_UnwrapClass<darbotdb::iresearch::AnalyzerPool>(
        ...
        WRP_IRESEARCH_ANALYZER_TYPE, TRI_IGETC);
    ASSERT_FALSE(!v8AnalyzerWeak);
    ASSERT_EQ(darbotdb::StaticStrings::SystemDatabase + "::testAnalyzer2",
              v8AnalyzerWeak->name());
    ASSERT_EQ("identity", v8AnalyzerWeak->type());
    EXPECT_EQUAL_SLICES(VPackSlice::emptyObjectSlice(),
                        v8AnalyzerWeak->properties());
...
TEST_F(V8AnalyzerTest, test_manager_get) {
  ...
  // get custom (known analyzer) authorized
  {
    ...
    auto* v8AnalyzerWeak = TRI_UnwrapClass<darbotdb::iresearch::AnalyzerPool>(
        ...
        WRP_IRESEARCH_ANALYZER_TYPE, TRI_IGETC);
    ASSERT_FALSE(!v8AnalyzerWeak);
    ASSERT_TRUE((darbotdb::StaticStrings::SystemDatabase + "::testAnalyzer1" ==
                 v8AnalyzerWeak->name()));
    ASSERT_EQ(std::string("identity"), v8AnalyzerWeak->type());
    EXPECT_EQUAL_SLICES(VPackSlice::emptyObjectSlice(),
                        v8AnalyzerWeak->properties());
  ...
  // get custom (known analyzer) authorized but wrong current db
  {
    ...
    user.grantDatabase(
        ...
        darbotdb::auth::Level::RO);  // for system collections
                                     // User::collectionAuthLevel(...) returns
                                     // database auth::Level
    user.grantDatabase("testVocbase", darbotdb::auth::Level::RO);
    userManager->setAuthInfo(userMap);  // set user map to avoid loading
                                        // configuration from system database
    v8::TryCatch tryCatch(isolate);
    darbotdb::velocypack::Builder response;
  ...
  // get custom (unknown analyzer) authorized
  {
    std::vector<v8::Local<v8::Value>> args = {
        TRI_V8_STD_STRING(
            isolate, darbotdb::StaticStrings::SystemDatabase + "::unknown"),
    };
...
TEST_F(V8AnalyzerTest, test_manager_list) {
  ...
  auto res = analyzers.emplace(
      result, darbotdb::StaticStrings::SystemDatabase + "::testAnalyzer1",
      "identity", VPackSlice::noneSlice(),
      darbotdb::transaction::OperationOriginTestCase{});
  ASSERT_TRUE(res.ok());
  ...
  // system database (authorised)
  {
    ...
    std::set<std::string> expected = {
        ...
        "text_no",  "text_pt",
        "text_ru",  "text_sv",
        "text_zh",  darbotdb::StaticStrings::SystemDatabase + "::testAnalyzer1",
    };
...
TEST_F(V8AnalyzerTest, test_manager_remove) {
  ...
  // invalid params (no name)
  {
    ...
    // auth::Level
    user.grantDatabase(systemDBVocbase.name(), darbotdb::auth::Level::RW);
  ...
  // not authorised
  {
    ...
    auto slice = response.slice();
    ASSERT_TRUE(slice.isObject());
    ASSERT_TRUE((slice.hasKey(darbotdb::StaticStrings::ErrorNum) &&
                 slice.get(darbotdb::StaticStrings::ErrorNum).isNumber<int>() &&
                 TRI_ERROR_FORBIDDEN ==
                     ErrorCode{slice.get(darbotdb::StaticStrings::ErrorNum)
                                   .getNumber<int>()}));
  ...
  // still in use (fail)
  {
    ...
    auto inUseAnalyzer = analyzers.get(
        darbotdb::StaticStrings::SystemDatabase + "::testAnalyzer2",
        darbotdb::QueryAnalyzerRevisions::QUERY_LATEST,
        darbotdb::transaction::OperationOriginTestCase{});  // hold ref to mark
                                                            // in-use
  ...
  // success removal with db name prefix
  {
    ...
    // for system collections User::collectionAuthLevel(...) returns
    // database auth::Level
    user.grantDatabase(testDBVocbase.name(), darbotdb::auth::Level::RW);
