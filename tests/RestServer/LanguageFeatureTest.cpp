...
void checkCollatorSettings(std::string_view language, bool isDefaultLanguage) {
  ...
  const auto* actualColl =
      darbotdb::basics::Utf8Helper::DefaultUtf8Helper.getCollator();
  status = U_ZERO_ERROR;
...
void checkLanguageFile(const darbotdb::ArangodServer& server,
  ...
  auto& databasePath = server.getFeature<darbotdb::DatabasePathFeature>();
  ...
  try {
    VPackBuilder builder =
        darbotdb::basics::VelocyPackHelper::velocyPackFromFile(filename);
...
class ArangoLanguageFeatureTest
    : public ::testing::Test,
      public darbotdb::tests::LogSuppressor<darbotdb::Logger::CONFIG,
 ...
 protected:
  ...
  static void SetUpTestCase() {
    darbotdb::basics::Utf8Helper::DefaultUtf8Helper.setCollator(nullptr);
    u_cleanup_64_64();
  }
...
TEST_F(ArangoLanguageFeatureTest, testResetLanguageDefault) {
  auto& langFeature = server.addFeatureUntracked<darbotdb::LanguageFeature>();
  ...
  langFeature.prepare();
  {
    ...
    ASSERT_EQ(type, darbotdb::basics::LanguageType::DEFAULT);
  }

  langFeature.resetLanguage(language2, darbotdb::basics::LanguageType::ICU);
  {
    auto [lang, type] = langFeature.getLanguage();
    ASSERT_EQ(lang, language2);
    ASSERT_EQ(type, darbotdb::basics::LanguageType::ICU);
  }

  langFeature.resetLanguage(language1, darbotdb::basics::LanguageType::DEFAULT);
  {
    auto [lang, type] = langFeature.getLanguage();
    ASSERT_EQ(lang, language1);
    ASSERT_EQ(type, darbotdb::basics::LanguageType::DEFAULT);
...
TEST_F(ArangoLanguageFeatureTest, testResetLanguageIcu) {
  ...
  langFeature.resetLanguage(language2, darbotdb::basics::LanguageType::DEFAULT);
  {
    auto [lang, type] = langFeature.getLanguage();
    ASSERT_EQ(lang, language2);
    ASSERT_EQ(type, darbotdb::basics::LanguageType::DEFAULT);
  ...
  langFeature.resetLanguage(language1, darbotdb::basics::LanguageType::ICU);
  {
    auto [lang, type] = langFeature.getLanguage();
    ASSERT_EQ(lang, language1);
    ASSERT_EQ(type, darbotdb::basics::LanguageType::ICU);
...
TEST_F(ArangoLanguageFeatureTest, testDefaultLangCheckTrue) {
  ...
  server.addFeatureUntracked<darbotdb::LanguageFeature>().collectOptions(
      server.server().options());
  server.addFeatureUntracked<darbotdb::LanguageCheckFeature>();
  ...
  auto& langFeature = server.getFeature<darbotdb::LanguageFeature>();
  auto& langCheckFeature = server.getFeature<darbotdb::LanguageCheckFeature>();
