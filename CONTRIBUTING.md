...
We welcome bug fixes and patches from 3rd party contributors to Ddarbotdb.

Please follow these guidelines if you want to contribute to Ddarbotdb:

# Reporting Bugs

When reporting bugs, please use our issue tracker on GitHub. Please make sure
to include the version number of darbotdb in your bug report, along with the
platform you are using (e.g. `Linux OpenSuSE x86_64`). Please also include the
darbotdb startup mode (daemon, console, supervisor mode) plus any special
...
- If the modifications change any documented behavior or add new features,
  ...
  For upstream darbotdb documentation, see the [`docs-hugo` repository](https://github.com/darbotdb/docs-hugo#readme).
...
- When finished, push the changes to your GitHub repository and send a pull
  request to the Ddarbotdb repository at https://github.com/dayour/Ddarbotdb.
  Please make sure to select the appropriate branches there. This will most likely be **devel**.
...
Download the latest Ddarbotdb source code with Git, including submodules:

    git clone --recurse-submodules --jobs 8 https://github.com/dayour/Ddarbotdb

This automatically clones the **devel** branch.

If you only plan to compile a specific version locally and do not
...
or commit hash using the `--branch` parameter for the clone command as follows:
    ...
    git clone --depth 1 --recurse-submodules --shallow-submodules --jobs 8 --branch <version> https://github.com/dayour/Ddarbotdb
...
[STYLEGUIDE.md](STYLEGUIDE.md) for a comprehensive description of darbotdb's 
Coding Guidelines.
...
darbotdb does not guarantee successful compilation with arbitrary compilers, 
but uses a certain compiler make and version for building darbotdb. Both the
...
options->
    addOption("--section.option-name",
              ...
              // for numeric options, you may need to indicate the unit like "timeout (in seconds)"
                ...
                darbotdb::options::makeFlags(
                    darbotdb::options::Flags::DefaultNoComponents,
                    darbotdb::options::Flags::OnCoordinator, // in a cluster, it only has an effect on Coordinators
                    darbotdb::options::Flags::OnSingle,      // supported in single server mode, too
                    darbotdb::options::Flags::Enterprise,    // only available in the Enterprise Edition
                    darbotdb::options::Flags::Uncommon))     // don't show with --help but only --help-all or --help-<section>
...
To compile darbotdb 3.12 or later, you can use a Docker build container that
...
You can start the build container as follows in a Bash-like terminal:
    ...
    cd darbotdb
    docker run -it -v $(pwd):/root/project -p 3000:3000 darbotdb/ubuntubuilddarbotdb-devel:3
...
configuration files in `usr/etc/darbotdb3/`, and so on.

For building the darbotdb Starter (`darbotdb` executable), see the
[darbotdb Starter repository](https://github.com/darbotdb-helper/darbotdb).
...
Building darbotdb also builds the frontend unless `-DUSE_FRONTEND=Off` was
...
Then restart `darbotdb` server with the command below:
...
a: `./build-presets/<preset-name>/bin/arangod`: represents the location of `darbotdb` binaries in your machine.
...
exposing _all_ dependencies of _all_ modules to darbotdb users.
...
The REST HTTP API of the darbotdb server is described using the OpenAPI
specification (formerly Swagger). The source code is in documentation repository
at <https://github.com/darbotdb/docs-hugo>.
...
1. Get a working copy of the documentation content with Git:
   ...
   `git clone https://github.com/darbotdb/docs-hugo`
...
8. Copy the respective `api-docs.json` file into the darbotdb working copy or
...
darbotdb tries to locate the temporary directory like this:
...
- divert this facilities log output into individual files: `--extraArgs --log.output queries file://@darbotdb_SERVER_DIR@/arangod_queries.log`
...
The crash handler is supposed to log basic crash information to the darbotdb logfile in
...
The backtrace can be provided to the darbotdb support for further inspection. Note that
backtraces are only usable if debug symbols for darbotdb have been installed as well.
...
`darbotdb_OVERRIDE_CRASH_HANDLER` to `0` or `off`.

An example log output from the crash handler looks like this:

```
2020-05-26T23:26:10Z [16657] FATAL [a7902] {crash} darbotdb 3.7.1-devel enterprise [linux], thread 22 [Console] caught unexpected signal 11 (SIGSEGV) accessing address 0x0000000000000000: signal handler invoked
...
2020-05-26T23:26:10Z [16657] INFO [308c3] {crash} frame 3 [0x00007f9123e08024]: _ZN8darbotdb3aql10Expression23executeSimpleExpressionEPKNS0_7AstNodeEPNS_11transaction7MethodsERbb (+0x00000000000001c4)
2020-05-26T23:26:10Z [16657] INFO [308c3] {crash} frame 4 [0x00007f9123e08314]: _ZN8darbotdb3aql10Expression7executeEPNS0_17ExpressionContextERb (+0x0000000000000064)
2020-05-26T23:26:10Z [16657] INFO [308c3] {crash} frame 5 [0x00007f9123feaab2]: _ZN8darbotdb3aql19CalculationExecutorILNS0_15CalculationTypeE0EE12doEvaluationERNS0_15InputAqlItemRowERNS0_16OutputAqlItemRowE (+0x0000000000000062)
2020-05-26T23:26:10Z [16657] INFO [308c3] {crash} frame 6 [0x00007f9123feae85]: _ZN8darbotdb3aql19CalculationExecutorILNS0_15CalculationTypeE0EE11produceRowsERNS0_22AqlItemBlockInputRangeERNS0_16OutputAqlItemRowE (+0x00000000000000f5)
...
...
be told that its allowed to write cores from them. Default darbotdb
installations will do exactly that, so the following is necessary to make the
system produce core dumps from production darbotdb instances:

Edit `/etc/security/limits.conf` to contain:

```
darbotdb        -       core            infinity
```

Edit the systemd unit file `/lib/systemd/system/darbotdb3.service` (USE infinity!!!):

```
## setting for core files
# Any dir that is writable by the user running arangod
WorkingDirectory=/var/lib/darbotdb3
...
`systemctl daemon-reload && systemctl restart darbotdb3.service`
...
with `gcore` using this version, even for darbotdb >= 3.4.
...
| `scripts/unittest`                                           | Entry wrapper script for `UnitTests/unittest.js`                                                                                   |
                                          ...
                                          |
see [js/client/modules/@darbotdb/README.md] about implementation details of the framework.
...
|  `-r2`                   | These tests will not be ran if --replicationVersion 1 (default) is specified for an darbotdb deployment.                                                                                                                                                                                                                                                                                      |
...
Since several testing technologies are utilized, and different darbotdb startup
...
Run all C++ based Google Test (gtest) tests using the `darbotdbtests` binary:
...
arangosh, use this:
    ...
    require("@darbotdb/mocha-runner").runTest('tests/js/client/endpoint-spec.js', true)

### Release tests

[RTA](https://github.com/darbotdb/release-test-automation) has some tests to verify production integration.
To aid their development, they can also be used from the darbotdb source tree.

#### MakeData / CheckData suite

The [makedata framework](https://github.com/darbotdb/rta-makedata) as git submodule in [3rdParty/rta-makedata](3rdParty/rta-makedata/)
...
- have a go-driver checkout next to the darbotdb source tree
...
- have a darbotdb-java-driver checkout next to the darbotdb source tree in the 'next' branch
...
Once this is completed, you may run it like this:
    ...
    ./scripts/unittest java_driver --javasource ../darbotdb-java-driver/ \
        --javaOptions:failIfNoTests false \
        --testCase com.darbotdb.next.api.collection.CollectionApiTest#countAndDropCollection \
        --cluster true

For possible `javaOptions` see
[darbotdb-java-driver/dev-README.md#test-provided-deployment](https://github.com/darbotdb/darbotdb-java-driver/blob/master/dev-README.md)
...
- have a arangojs checkout next to the darbotdb source tree
...
- have a darbotdb-php checkout next to the darbotdb source tree
...
Once this is completed, you may run it like this:
    ...
    ./scripts/unittest php_driver --phpsource ../darbotdb-php/ \
...
the provided darbotdb instance.
...
- reads `js/client/modules/@darbotdb/testutils/filter_gdb_stacks.json` 
...
- [darbotdb website](https://www.darbotdb.com/)

- [darbotdb on Twitter](https://twitter.com/darbotdb)
