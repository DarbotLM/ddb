...
class DeploymentVariant(Enum):
    """ which sort of darbotdb deployment? """
    SINGLE = 1,
    MIXED = 2,
    CLUSTER = 3
...
def parse_arguments():
    ...
    parser.add_argument(
        "--ddbsh-args", type=str, help="additional arguments to append to ddbsh"
    )
    parser.add_argument(
        "--extra-args", type=str, help="additional arguments to append to all testing.js"
    )
    parser.add_argument(
        "--ddb-without-v8", type=str, help="whether we run a setup without .js"
    )
    parser.add_argument(
        "--validate-only",
        ...
        for key, val in definition['args'].items():
            if key == 'moreArgv':
                ...
                if isinstance(val, bool):
                    ...
                    args.append(val)
    if 'ddbsh_args' in definition:
        if not isinstance(definition['ddbsh_args'], dict):
            raise Exception(f"expected ddbsh_args to be a key value list! have: {definition['ddbsh_args']}")
        for key, val in definition['ddbsh_args'].items():
            ddbsh_args.append(f"--{key}")
            if isinstance(val, bool):
                ddbsh_args.append("true" if val else "false")
            else:
                ddbsh_args.append(val)
...
def create_test_job(test, depl_variant, build_config, build_jobs, args, replication_version=1):
    ...
    sub_ddbsh_args = args.ddbsh_args
    if 'ddbsh_args' in test:
        # Yaml workaround: prepend an A to stop bad things from happening.
        if test["ddbsh_args"] != "":
            sub_ddbsh_args = test["ddbsh_args"] + args.ddbsh_args
        del test["ddbsh_args"]
    job = {
        ...
        "requires": build_jobs,
        "ddbsh_args": "A " + json.dumps(sub_ddbsh_args),
...
def create_rta_test_job(build_config, build_jobs, deployment_mode, filter_statement, buckets, rta_branch):
    """ this job will use RTA to launch ddb """
...
def add_test_jobs_to_workflow(args, workflow, tests, build_config, build_jobs):
    ...
    if args.ui == "only":
        return
    # TODO: QA-698
    if build_config.enterprise and args.ddb_without_v8 != 'true':
...
def add_create_docker_image_job(workflow, build_config, build_jobs, args):
    ...
    image = (
        ...
        else "public.ecr.aws/b0b8h2r4/darbotdb-preview"
    )
...
def main():
    ...
    try:
        ...
        if ddbsh_args in ["A", ""]:
            ...
            args.ddbsh_args = ddbsh_args[1:].split(' ')
