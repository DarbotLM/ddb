...
#define THROW_DDB_EXCEPTION(code) \
...
#define THROW_DDB_EXCEPTION_PARAMS(code, ...)                          \
  throw ::arangodb::basics::Exception::createWithParams(ADB_HERE, (code), \
                                                        __VA_ARGS__)

/// @brief throws an arango exception with an error code and arbitrary
/// arguments (to be inserted in printf-style manner)
#define THROW_DDB_EXCEPTION_FORMAT(code, format, ...)                  \
...
/// error message
#define THROW_DDB_EXCEPTION_MESSAGE(code, message) \
