...
class BenchmarkThread : public darbotdb::Thread {
 public:
 ...
 protected:
  void run() override {
    try {
      ...
      LOG_TOPIC("b69d7", FATAL, darbotdb::Logger::BENCH)
    ...
    auto check = darbotdb::HttpResponseChecker::check(
        _httpClient->getErrorMessage(), result.get());
    if (check.fail()) {
      LOG_TOPIC("5cda7", FATAL, darbotdb::Logger::BENCH)
          << check.errorMessage();
      FATAL_ERROR_EXIT();
    }
    ...
    if (_threadNumber == 0) {
      if (!_operation->setUp(_httpClient.get())) {
        LOG_TOPIC("528b6", FATAL, darbotdb::Logger::BENCH)
    ...
    while (!isStopping()) {
      ...
      try {
        ...
        LOG_TOPIC("29451", FATAL, darbotdb::Logger::BENCH)
            << "Caught OOM exception during test execution!";
        FATAL_ERROR_EXIT();
      } catch (std::exception const& ex) {
        LOG_TOPIC("793e3", FATAL, darbotdb::Logger::BENCH)
            << "Caught STD exception during test execution: " << ex.what();
        FATAL_ERROR_EXIT();
      } catch (...) {
        LOG_TOPIC("c1d6d", FATAL, darbotdb::Logger::BENCH)
 ...
 private:
  ...
  void processResponse(httpclient::SimpleHttpResult const* result) {
    ...
    if (++_warningCount < maxWarnings) {
      if (check.fail()) {
        LOG_TOPIC("fb835", WARN, darbotdb::Logger::BENCH)
            << "Request for URL '" << _requestData.url
            << "': " << check.errorMessage();
      }
    } else if (_warningCount == maxWarnings) {
      LOG_TOPIC("6daf1", WARN, darbotdb::Logger::BENCH)
          << "...more warnings...";
    }
  }
 ...
 private:
  ...
  std::unique_ptr<darbotdb::httpclient::SimpleHttpClient> _httpClient;
