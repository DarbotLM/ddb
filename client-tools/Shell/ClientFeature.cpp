...
using namespace darbotdb::httpclient;
...
void ClientFeature::collectOptions(std::shared_ptr<ProgramOptions> options) {
  ...
  auto& opt = options->addOption(
      ...
      darbotdb::options::makeFlags(Flags::FlushOnFirst, Flags::Default));
  if (isArangosh) {
...
arangosh without connecting to a server.)");
  ...
  if (isArangosh) {
    ...
    options->addOption("--server.force-json",
                       ...
                       darbotdb::options::makeDefaultFlags(
  ...
  options->addOption(
      ...
      darbotdb::options::makeDefaultFlags(darbotdb::options::Flags::Uncommon));
...
received by an darbotdb server.)");
...
void ClientFeature::validateOptions(std::shared_ptr<ProgramOptions> options) {
  if (_sslProtocol == SslProtocol::SSL_V2) {
    LOG_TOPIC("64f4f", FATAL, darbotdb::Logger::SSL)
  ...
  if (_connectionTimeout < 0.0) {
    LOG_TOPIC("81598", FATAL, darbotdb::Logger::FIXME)
  ...
  if (_requestTimeout < 0.0) {
    LOG_TOPIC("fb847", FATAL, darbotdb::Logger::FIXME)
  ...
  if (_maxPacketSize < 1 * 1024 * 1024) {
    LOG_TOPIC("f7793", FATAL, darbotdb::Logger::FIXME)
  ...
  if (_username.empty()) {
    LOG_TOPIC("fa58c", FATAL, darbotdb::Logger::FIXME)
  ...
  if ((_askJwtSecret || hasJwtSecretFile) &&
    ...
    LOG_TOPIC("65475", FATAL, darbotdb::Logger::FIXME)
        ...
        _endpoints.begin(), _endpoints.end(), [](auto const& endpoint) {
          if (!endpoint.empty() && (endpoint != "none") &&
            ...
            if (ep != nullptr && ep->isBroadcastBind()) {
              LOG_TOPIC("701fb", FATAL, darbotdb::Logger::FIXME)
  ...
  if (auto res = DatabaseNameValidator::validateName(true, true, _databaseName);
      res.fail()) {
    LOG_TOPIC("122a6", FATAL, darbotdb::Logger::FIXME) << res.errorMessage();
    FATAL_ERROR_EXIT();
  }
...
std::unique_ptr<httpclient::SimpleHttpClient> ClientFeature::createHttpClient(
  ...
  if (endpoint == nullptr) {
    if (definition != "none" && !suppressError) {
      LOG_TOPIC("2fac8", ERR, darbotdb::Logger::FIXME)
...
std::string ClientFeature::buildConnectedMessage(
  ...
  return absl::StrCat(
      "Connected to darbotdb '", endpointSpecification,
...
int ClientFeature::runMain(
  ...
  try {
    ...
    LOG_TOPIC("5b00f", ERR, darbotdb::Logger::FIXME)
        << argv[0]
        << " terminated because of an unhandled exception: " << ex.what();
    return EXIT_FAILURE;
  } catch (...) {
    LOG_TOPIC("98466", ERR, darbotdb::Logger::FIXME)
        << argv[0]
