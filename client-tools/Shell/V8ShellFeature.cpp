...
void V8ShellFeature::validateOptions(
  ...
  if (_startupDirectory.empty()) {
    LOG_TOPIC("6380f", FATAL, darbotdb::Logger::FIXME)
...
void V8ShellFeature::start() {
  ...
  v8g->_securityContext =
      darbotdb::JavaScriptSecurityContext::createAdminScriptContext();
  ...
  if (context.IsEmpty()) {
    LOG_TOPIC("5f5dd", FATAL, darbotdb::Logger::FIXME)
...
void V8ShellFeature::unprepare() {
...

  {
    ...
    TRI_v8_global_t* v8g = static_cast<TRI_v8_global_t*>(
        _isolate->GetData(darbotdb::V8PlatformFeature::V8_DATA_SLOT));
    _isolate->SetData(darbotdb::V8PlatformFeature::V8_DATA_SLOT, nullptr);
...
bool V8ShellFeature::runScript(std::vector<std::string> const& files,
  ...
  for (auto const& file : files) {
    if (!FileUtils::exists(file)) {
      LOG_TOPIC("4beec", ERR, darbotdb::Logger::FIXME)
    ...
    if (execute) {
      ...
      if (tryCatch.HasCaught()) {
        ...
        LOG_TOPIC("c254f", ERR, darbotdb::Logger::FIXME) << exception;
        ok = false;
      }
    } else {
...
bool V8ShellFeature::runString(std::vector<std::string> const& strings,
  ...
  for (auto const& script : strings) {
    ...
    if (tryCatch.HasCaught()) {
      std::string exception(TRI_StringifyV8Exception(_isolate, &tryCatch));
      LOG_TOPIC("979b9", ERR, darbotdb::Logger::FIXME) << exception;
      ok = false;
    } else {
...
bool V8ShellFeature::runUnitTests(std::vector<std::string> const& files,
  ...
  for (auto const& file : files) {
    if (!FileUtils::exists(file)) {
      LOG_TOPIC("51bdb", ERR, darbotdb::Logger::FIXME)
          << "error: JavaScript file not found: '" << file << "'";
      ok = false;
      continue;
    }
  ...
  TRI_ExecuteJavaScriptString(
      _isolate, "require(\"@darbotdb/testrunner\").runCommandLineTests();",
      TRI_V8_SHELL_COMMAND_NAME, true);

  if (tryCatch.HasCaught()) {
    std::string exception(TRI_StringifyV8Exception(_isolate, &tryCatch));
    LOG_TOPIC("59589", ERR, darbotdb::Logger::FIXME) << exception;
    ok = false;
  } else {
...
static void JS_VersionClient(v8::FunctionCallbackInfo<v8::Value> const& args) {
  ...
  if (!details) {
    ...
    TRI_V8_RETURN(TRI_V8_ASCII_STRING(isolate, darbotdb_VERSION));
  }
...
void V8ShellFeature::initGlobals() {
  ...
  TRI_AddGlobalVariableVocbase(
      _isolate, TRI_V8_ASCII_STRING(_isolate, "darbotdb_CLIENT_VERSION"),
  ...
  if (ctx == nullptr) {
    LOG_TOPIC("b754a", FATAL, darbotdb::Logger::FIXME)
  ...
  for (auto const& file : files) {
    switch (loader.loadScript(_isolate, file, nullptr)) {
      case JSLoader::eSuccess:
        LOG_TOPIC("edc8d", TRACE, darbotdb::Logger::FIXME)
            << "loaded JavaScript file '" << file << "'";
        break;
      case JSLoader::eFailLoad:
        LOG_TOPIC("022a8", FATAL, darbotdb::Logger::FIXME)
            << "cannot load JavaScript file '" << file << "'";
        FATAL_ERROR_EXIT();
        break;
      case JSLoader::eFailExecute:
        LOG_TOPIC("22385", FATAL, darbotdb::Logger::FIXME)
