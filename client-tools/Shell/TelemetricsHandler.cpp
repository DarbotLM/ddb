...
std::string const kTelemetricsGatheringUrl =
    "https://telemetrics.darbotdb.com/v1/collect";
}
...
void TelemetricsHandler::fetchTelemetricsFromServer() {
  ...
  std::unordered_map<std::string, std::string> headers = {
      {StaticStrings::UserAgent, std::string("arangosh/") + darbotdb_VERSION},
  ...
  while (!_server.isStopping()) {
    try {
      ...
      if (_httpClient != nullptr && _httpClient->isConnected()) {
        ...
        if (result.fail()) {
          LOG_TOPIC("a3146", WARN, darbotdb::Logger::FIXME)
              << "Error: could not detect darbotdb instance type: "
              << result.errorMessage();
        ...
        if (_telemetricsFetchResponse.ok() ||
          ...
          if (!deploymentSlice.isNone()) {
            ...
            if (deploymentType == "active_failover") {
              ...
              // connected to servers running previous versions of darbotdb
      ...
      LOG_TOPIC("e1b5b", WARN, darbotdb::Logger::FIXME)
...
velocypack::Builder TelemetricsHandler::sendTelemetricsToEndpoint(
  ...
  headers.emplace("darbotdb-request-type", "telemetrics");
  ...
  while (!_server.isStopping()) {
    try {
      ...
      LOG_TOPIC("09acf", WARN, darbotdb::Logger::FIXME)
