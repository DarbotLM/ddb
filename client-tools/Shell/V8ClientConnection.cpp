...
using namespace darbotdb::import;

namespace fu = darbotdb::fuerte;
...
std::string connectionIdentifier(fuerte::ConnectionBuilder& builder) {
  ...
  // create md5
  char hash[16];
  darbotdb::rest::SslInterface::sslMD5(raw.c_str(), raw.length(), &hash[0]);

  // as hex
  char hex[32];
  darbotdb::rest::SslInterface::sslHEX(hash, 16, &hex[0]);
  ...
  while (retryCount > 0) {
    ...
    try {
      ...
      if (!res) {
        ...
        LOG_TOPIC("9daaa", DEBUG, darbotdb::Logger::HTTPCLIENT)
      ...
      if (_lastHttpReturnCode >= 400) {
        ...
        if (it != headers.end()) {
          ...
          LOG_TOPIC("9daab", DEBUG, darbotdb::Logger::HTTPCLIENT)
              << "Connection attempt to endpoint '" << _client.endpoint()
              << "' failed: " << errorMessage
      ...
      if (!body.isObject()) {
        ...
        LOG_TOPIC("9daac", DEBUG, darbotdb::Logger::HTTPCLIENT)
            << "Connection attempt to endpoint '" << _client.endpoint()
            << "' failed: " << msg
            << " from: " << newConnection->localEndpoint();
        return nullptr;
      ...
      if (retryCount <= 0) {
        ...
        LOG_TOPIC("9daad", DEBUG, darbotdb::Logger::HTTPCLIENT)
            << "Connection attempt to endpoint '" << _client.endpoint()
            << "' failed: " << msg
            << " from: " << newConnection->localEndpoint();
        return nullptr;
...
static void ClientConnection_ConstructorCallback(
  ...
  if (v8connection->isConnected() &&
    ...
    LOG_TOPIC("9c8b4", INFO, darbotdb::Logger::FIXME)
        << ClientFeature::buildConnectedMessage(
...
static void ClientConnection_httpFuzzRequests(
  ...
  if (!seed.has_value()) {
    ...
    // during testing.
    LOG_TOPIC("39e50", WARN, darbotdb::Logger::HTTPCLIENT)
...
ErrorCode uncompressResponse(fu::Response& response) {
  ...
  if (response.contentEncoding() == fuerte::ContentEncoding::Deflate ||
    ...
    if (response.contentEncoding() == fuerte::ContentEncoding::Deflate) {
      code = darbotdb::encoding::zlibInflate(
      ...
      code = darbotdb::encoding::gzipUncompress(
          reinterpret_cast<uint8_t const*>(responseBody.data()),
          responseBody.size(), inflateBuf);
    }
...
uint32_t V8ClientConnection::sendFuzzRequest(fuzzer::RequestFuzzer& fuzzer) {
  ...
  try {
    ...
    if (rc != fu::Error::NoError) {
      LOG_TOPIC("39e53", INFO, darbotdb::Logger::HTTPCLIENT)
          << "rc: " << static_cast<uint32_t>(rc)
          << " from: " << getLocalEndpoint();
    }
  }
  if (!connection || connection->state() == fu::Connection::State::Closed) {
    LOG_TOPIC("39e51", INFO, darbotdb::Logger::HTTPCLIENT)
        << "connection closed after " << fuerte::v1::to_string(req_copy)
        << " state: " << to_string(connection->state())
        << " from: " << localEndpoint;
  }
  if (response) {
    LOG_TOPIC("39e52", INFO, darbotdb::Logger::HTTPCLIENT)
        << " state: " << to_string(connection->state())
