...
Result DatabaseFeature::createDatabase(CreateDatabaseInfo&& info,
...

  {
    ...
    std::lock_guard lockCreate{_databaseCreateLock};
    {
      ...
      if (it != databases->end()) {
        // name already in use
        return Result(TRI_ERROR_DDB_DUPLICATE_NAME,
...
ErrorCode DatabaseFeature::iterateDatabases(velocypack::Slice databases) {
  ...
  for (velocypack::Slice it : velocypack::ArrayIterator(databases)) {
    ...
    if (res.fail()) {
      std::string errorMsg;
      // note: TRI_ERROR_DDB_DATABASE_NAME_INVALID should not be
      // used anymore in 3.11 and higher.
      bool isNameError = res.is(TRI_ERROR_DDB_DATABASE_NAME_INVALID) ||
                         res.is(TRI_ERROR_DDB_ILLEGAL_NAME);
      if (isNameError) {
