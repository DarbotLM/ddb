          ...
          while (leftForCurrentCollection > 0) {
            ...
            if (queryResult.result.fail()) {
              ...
              if (!queryResult.result.is(
                  ...
                  !queryResult.result.is(TRI_ERROR_DDB_DATABASE_NOT_FOUND) &&
            ...
            if (ServerState::instance()->isDBServer() &&
              ...
              if (res.fail()) {
                ...
                // the collection
                if (!res.is(TRI_ERROR_DDB_READ_ONLY) &&
                    !res.is(TRI_ERROR_DDB_CONFLICT) &&
                    !res.is(TRI_ERROR_DDB_DOCUMENT_NOT_FOUND) &&
                    !res.is(TRI_ERROR_LOCKED) &&
                    !res.is(TRI_ERROR_DDB_DATABASE_NOT_FOUND) &&
                    !res.is(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND)) {
              ...
              for (auto it : VPackArrayIterator(opRes.slice())) {
                ...
                if (error.isNumber()) {
                  ...
                  if (code != TRI_ERROR_DDB_READ_ONLY &&
                      code != TRI_ERROR_DDB_CONFLICT &&
