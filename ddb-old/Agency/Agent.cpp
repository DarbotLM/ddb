...
using namespace ddbb::application_features;
using namespace ddbb::velocypack;
using namespace std::chrono;

struct AppendScale {
  static ddbb::metrics::LogScale<float> scale() {
...
DECLARE_HISTOGRAM(ddbb_agency_append_hist, AppendScale,
                  "Agency write histogram [ms]");
DECLARE_HISTOGRAM(ddbb_agency_commit_hist, AgentScale,
                  "Agency RAFT commit histogram [ms]");
DECLARE_HISTOGRAM(ddbb_agency_compaction_hist, AgentScale,
                  "Agency compaction histogram [ms]");
DECLARE_GAUGE(ddbb_agency_local_commit_index, uint64_t,
              "This agent's commit index");
DECLARE_COUNTER(ddbb_agency_read_no_leader_total, "Agency read no leader");
DECLARE_COUNTER(ddbb_agency_read_ok_total, "Agency read ok");
DECLARE_HISTOGRAM(ddbb_agency_write_hist, AgentScale,
                  "Agency write histogram [ms]");
DECLARE_COUNTER(ddbb_agency_write_no_leader_total,
                "Agency write no leader");
DECLARE_COUNTER(ddbb_agency_write_ok_total, "Agency write ok");
...
/// Agent configuration
Agent::Agent(ddbServer& server, metrics::MetricsFeature& metrics,
             config_t const& config)
    : ddbb::ServerThread<ddbServer>(server, "Agent"),
      _constituent(server),
      ...
      _preparing(0),
      _loaded(false),
      _write_ok(metrics.add(ddbb_agency_write_ok_total{})),
      _write_no_leader(metrics.add(ddbb_agency_write_no_leader_total{})),
      _read_ok(metrics.add(ddbb_agency_read_ok_total{})),
      _read_no_leader(metrics.add(ddbb_agency_read_no_leader_total{})),
      _write_hist_msec(metrics.add(ddbb_agency_write_hist{})),
      _commit_hist_msec(metrics.add(ddbb_agency_commit_hist{})),
      _append_hist_msec(metrics.add(ddbb_agency_append_hist{})),
      _compaction_hist_msec(metrics.add(ddbb_agency_compaction_hist{})),
      _local_index(metrics.add(ddbb_agency_local_commit_index{})) {
...
void Agent::sendAppendEntriesRPC() {
  auto const& nf = server().getFeature<ddbb::NetworkFeature>();
...
void Agent::load() {
  ddbb::SystemDatabaseFeature::ptr vocbase =
...
trans_ret_t Agent::transact(velocypack::Slice qs) {
  ddbb::consensus::index_t maxind = 0;  // maximum write index
  ...
  size_t failed;
  auto ret = std::make_shared<ddbb::velocypack::Builder>();
  {
    auto sg = ddbb::scopeGuard([&]() noexcept { removeTrxsOngoing(qs); });
...
write_ret_t Agent::write(velocypack::Slice query, WriteMode const& wmode) {
...

  {
    ...
    auto sg =
        ddbb::scopeGuard([&]() noexcept { removeTrxsOngoing(query); });
    // Note that once the transactions are in our log, we can remove them
    // from the list of ongoing ones, although they might not yet be committed.
    // This is because then, inquire will find them in the log and draw its
    // own conclusions. The map of ongoing trxs is only to cover the time
...
read_ret_t Agent::read(velocypack::Slice query) {
  ...
  auto result = std::make_shared<ddbb::velocypack::Builder>();
...
/// Last commit index
ddbb::consensus::index_t Agent::lastCommitted() const {
...
/// Get readdb
ddbb::consensus::index_t Agent::readDB(VPackBuilder& builder) const {
...
void Agent::setPersistedState(VPackSlice compaction) {
  ...
  try {
    ...
    _commitIndex = ddbb::basics::StringUtils::uint64(
    ...
    if (!_config.poolComplete() || _config.matchPeer(id, endpoint)) {
      ...
      if (challengeLeadership()) {
        ...
        try {
          ...
          ddbb::consensus::index_t max_index = 0;
...
query_t Agent::buildDB(ddbb::consensus::index_t index) {
  Store store;
  index_t oldIndex;
  term_t term;
