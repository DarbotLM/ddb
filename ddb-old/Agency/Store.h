...
class Store {
 public:
  ...
  std::vector<apply_ret_t> applyTransactions(
      darbotdb::velocypack::Slice query,
  ...
  std::vector<bool> applyLogEntries(darbotdb::velocypack::Builder const& query,
                                    index_t index, term_t term);

  /// @brief Read multiple entries from store
  std::vector<bool> readMultiple(darbotdb::velocypack::Slice query,
                                 darbotdb::velocypack::Builder& result) const;

  /// @brief Read individual entry specified in slice into builder
  bool read(darbotdb::velocypack::Slice query,
            darbotdb::velocypack::Builder& result) const;
  ...
  void get(std::string const& path, darbotdb::velocypack::Builder& b,
 ...
 private:
  ...
  bool applies(darbotdb::velocypack::Slice const&);

  friend class consensus::Node;
  /// @brief Check precondition
  check_ret_t check(darbotdb::velocypack::Slice slice,
