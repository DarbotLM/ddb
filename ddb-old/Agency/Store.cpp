...
std::vector<bool> Store::applyLogEntries(
    darbotdb::velocypack::Builder const& queries, index_t index, term_t term) {
          ...
          if (!op.value.isArray()) {  // right hand side must be array will
            ...
            if (node->isArray()) {
              ...
              std::unordered_set<VPackSlice,
                                 darbotdb::velocypack::NormalizedCompare::Hash,
                                 darbotdb::velocypack::NormalizedCompare::Equal>
                  elems;
...
bool Store::applies(darbotdb::velocypack::Slice const& transaction) {
...
void Store::get(std::string const& path, darbotdb::velocypack::Builder& b,
  ...
  if (auto node = _node->get(path); node) {
    ...
    b.add(darbotdb::velocypack::Slice::emptyObjectSlice());
  }
}
