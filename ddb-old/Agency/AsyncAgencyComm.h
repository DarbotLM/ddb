...
struct AsyncAgencyCommResult {
  darbotdb::fuerte::Error error;
  std::unique_ptr<darbotdb::fuerte::Response> response;

  [[nodiscard]] bool ok() const noexcept {
    return darbotdb::fuerte::Error::NoError == this->error;
  }
  ...
  darbotdb::fuerte::StatusCode statusCode() const {
    TRI_ASSERT(response != nullptr);
    return response->statusCode();
  }

  [[nodiscard]] Result asResult() const {
    using namespace darbotdb::network;
    if (!ok()) {
...
struct AgencyReadResult : public AsyncAgencyCommResult {
  AgencyReadResult(
      ...
      std::shared_ptr<darbotdb::cluster::paths::Path const> valuePath)
 ...
 private:
  VPackSlice _value;
  std::shared_ptr<darbotdb::cluster::paths::Path const> _valuePath;
};
...
class AsyncAgencyComm final {
 public:
  using FutureResult = darbotdb::futures::Future<AsyncAgencyCommResult>;
  using FutureReadResult = darbotdb::futures::Future<AgencyReadResult>;

  [[nodiscard]] FutureResult getValues(
      std::string const& path,
      std::optional<network::Timeout> timeout = {}) const;
  [[nodiscard]] FutureReadResult getValues(
      std::shared_ptr<darbotdb::cluster::paths::Path const> const& path,
      std::optional<network::Timeout> timeout = {}) const;
  ...
  [[nodiscard]] FutureResult deleteKey(
      network::Timeout timeout,
      std::shared_ptr<darbotdb::cluster::paths::Path const> const& path) const;
  ...
  [[nodiscard]] FutureResult setTransientValue(
      std::string const& key, darbotdb::velocypack::Slice const& slice,
  ...
  [[nodiscard]] FutureResult sendWithFailover(
      darbotdb::fuerte::RestVerb method, std::string_view url,
  ...
  [[nodiscard]] FutureResult sendWithFailover(darbotdb::fuerte::RestVerb method,
                                              std::string_view url,
                                              network::Timeout timeout,
                                              RequestType type,
                                              uint64_t index) const;
