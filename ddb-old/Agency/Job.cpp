...
std::string Job::agencyPrefix = "ddb";
...
std::vector<std::string> Job::healthyServers(
    darbotdb::consensus::Node const& snapshot) {
  std::vector<std::string> ret;
...
bool Job::isServerLeaderForState(const Node& snap, std::string const& db,
                                 darbotdb::replication2::LogId stateId,
...
std::optional<darbotdb::replication2::agency::LogTarget> Job::readStateTarget(
  ...
  auto target =
      velocypack::deserialize<darbotdb::replication2::agency::LogTarget>(
          targetNode->toBuilder().slice());
  return target;
}

std::optional<darbotdb::replication2::agency::LogPlanSpecification>
Job::readLogPlan(Node const& snap, std::string const& db,
  ...
  auto plan = velocypack::deserialize<
      darbotdb::replication2::agency::LogPlanSpecification>(
      planNode->toBuilder().slice());
  return plan;
}

std::optional<darbotdb::replication2::LogId> Job::getReplicatedStateId(
...
std::string Job::findOtherHealthyParticipant(
    Node const& snap, darbotdb::replication2::agency::LogTarget const& target,
    std::string const& serverToAvoid) {
  std::unordered_map<std::string, bool> good;
