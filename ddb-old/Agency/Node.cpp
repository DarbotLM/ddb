...
ResultT<NodePtr> Node::handle<INCREMENT>(Node const* target,
                                         VPackSlice const& slice) {
  auto inc = darbotdb::basics::VelocyPackHelper::getNumericValue<std::int64_t>(
      slice, "step", 1);
...
ResultT<NodePtr> Node::handle<DECREMENT>(Node const* target,
                                         VPackSlice const& slice) {
  auto dec = darbotdb::basics::VelocyPackHelper::getNumericValue<std::int64_t>(
      slice, "step", 1);
  auto pre = target ? target->getInt().value_or(0) : 0;
...
darbotdb::ResultT<NodePtr> Node::applyOp(Node const* target, VPackSlice slice) {
...
darbotdb::ResultT<NodePtr> Node::applyOp(std::string_view path,
  ...
  if (slice.isObject()) {
    ...
    for (auto const& [key, sub] : VPackObjectIterator(slice)) {
      ...
      if (keyStr.find("/") != std::string::npos) {
        THROW_DDB_EXCEPTION_FORMAT(TRI_ERROR_AGENCY_MALFORMED_TRANSACTION,
...
DECLARE_GAUGE(darbotdb_agency_node_memory_usage, uint64_t, nodeMetricsHelpText);

void Node::toPrometheus(std::string& result, std::string_view globals,
                        bool ensureWhitespace) {
  auto name = darbotdb_agency_node_memory_usage{}.name();
...
// AccountingAllocator
using AllocatorType =
    typename darbotdb::consensus::Node::allocator_type::rebind<uint8_t>::type;

namespace darbotdb::velocypack {
