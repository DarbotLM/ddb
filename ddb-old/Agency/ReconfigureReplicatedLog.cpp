...
auto inspect(Inspector& f, ReconfigureOperation& x) {
  return f.variant(x.value)
      .embedded("operation")
      .alternatives(
          darbotdb::inspection::type<ReconfigureOperation::SetLeader>(
              "set-leader"),
          darbotdb::inspection::type<ReconfigureOperation::AddParticipant>(
              "add-participant"),
          darbotdb::inspection::type<ReconfigureOperation::RemoveParticipant>(
              "remove-participant"));
}
template<typename Inspector>
...
darbotdb::Result ReconfigureReplicatedLog::abort(const std::string& reason) {
...
struct OperationApplier {
  ...
  darbotdb::replication2::agency::LogTarget& target;
};
}  // namespace
...
ReconfigureReplicatedLog::ReconfigureReplicatedLog(
    ...
    darbotdb::replication2::LogId logId, std::vector<ReconfigureOperation> ops,
