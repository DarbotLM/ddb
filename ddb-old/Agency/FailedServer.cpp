...
bool FailedServer::start(bool& aborts) {
  ...
  if (status && status.value() != Supervision::HEALTH_STATUS_FAILED) {
    ...
    std::stringstream reason;
    reason << "Server " << _server
           << " no longer in health. Already removed. Abort.";
...

  {
    ...
    // Preconditions
    {
      ...
      _jb->add(VPackValue(failedServersPrefix));
      {
        VPackObjectBuilder old(_jb.get());
        _jb->add("old",
                 _snapshot.hasAsBuilder(failedServersPrefix).value().slice());
      }
    }  // Preconditions
  }
...
JOB_STATUS FailedServer::status() {
  ...
  bool serverHealthy =
      serverHealth && *serverHealth == Supervision::HEALTH_STATUS_GOOD;
...
darbotdb::Result FailedServer::abort(std::string const& reason) {
  return Result{};
