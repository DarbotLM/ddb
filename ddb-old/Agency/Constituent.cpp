...
DECLARE_GAUGE(darbotdb_agency_term, uint64_t, "Agency's term");
...
bool Constituent::logUpToDate(darbotdb::consensus::index_t prevLogIndex,
...
bool Constituent::logMatches(darbotdb::consensus::index_t prevLogIndex,
                             term_t prevLogTerm) const {
...
void Constituent::callElection() {
  ...
  auto const& nf = _agent->server().getFeature<darbotdb::NetworkFeature>();
...
void Constituent::run() {
  ...
  // Most recent vote
  {
    ...
    auto query = darbotdb::aql::Query::create(
        ...
        darbotdb::aql::QueryString(aql), nullptr);
