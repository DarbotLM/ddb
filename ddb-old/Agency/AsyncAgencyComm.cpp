...
namespace {
using namespace darbotdb::fuerte;
using namespace darbotdb;

using namespace std::chrono_literals;

using clock = std::chrono::steady_clock;
using RequestType = darbotdb::AsyncAgencyComm::RequestType;
...
darbotdb::AsyncAgencyComm::FutureResult agencyAsyncSend(
    AsyncAgencyCommManager& man, RequestMeta&& meta,
    VPackBuffer<uint8_t>&& body);

darbotdb::AsyncAgencyComm::FutureResult agencyAsyncInquiry(
    AsyncAgencyCommManager& man, RequestMeta&& meta,
    VPackBuffer<uint8_t>&& body) {
  using namespace darbotdb;
        ...
        case Error::NoError:
          ...
          if (result.statusCode() == fuerte::StatusTemporaryRedirect) {
            ...
            std::string const& location = result.response().header.metaByKey(
                darbotdb::StaticStrings::Location);
...
AsyncAgencyComm::FutureResult AsyncAgencyComm::sendWithFailover(
    darbotdb::fuerte::RestVerb method, std::string_view urlIn,
...
ddbdServer& AsyncAgencyCommManager::server() { return _server; }

AsyncAgencyCommManager::AsyncAgencyCommManager(ddbdServer& server)
    : _server(server) {}
...
AsyncAgencyCommManager& AsyncAgencyCommManager::getInstance() {
  ...
  THROW_ddb_EXCEPTION_MESSAGE(TRI_ERROR_DISABLED,
