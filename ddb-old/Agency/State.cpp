...
DECLARE_GAUGE(darbotdb_agency_log_size_bytes, uint64_t,
              "Agency replicated log size [bytes]");
DECLARE_GAUGE(darbotdb_agency_client_lookup_table_size, uint64_t,
...
State::State(metrics::MetricsFeature& metrics)
    : _agent(nullptr),
      ...
      _lastCompactionAt(0),
      _cur(0),
      _log_size(metrics.add(darbotdb_agency_log_size_bytes{})),
      _clientIdLookupCount(
          metrics.add(darbotdb_agency_client_lookup_table_size{})) {}
...
bool State::persist(index_t index, term_t term, uint64_t millis,
                    darbotdb::velocypack::Slice const& entry,
...
bool State::persistConf(index_t index, term_t term, uint64_t millis,
  ...
  try {
    ...
    if (logResult.fail()) {
      THROW_DDB_EXCEPTION(logResult.result);
    }
...
std::vector<index_t> State::logLeaderMulti(
    darbotdb::velocypack::Slice transactions,
  ...
  if (transactions.length() != applicable.size()) {
    THROW_DDB_EXCEPTION_MESSAGE(TRI_ERROR_AGENCY_MALFORMED_TRANSACTION,
...
index_t State::logLeaderSingle(darbotdb::velocypack::Slice slice, term_t term,
    ...
    while (ndups < transactions.length()) {
      ...
      if (idx == _log.at(pos).index && trm != _log.at(pos).term) {
        ...
        if (queryResult.result.fail()) {
          THROW_DDB_EXCEPTION(queryResult.result);
        }
...
bool State::ensureCollection(std::string const& name, bool drop) {
  ...
  if (collection == nullptr) {
    THROW_DDB_EXCEPTION_MESSAGE(TRI_errno(), "cannot create collection");
  }
...
bool State::loadCollections(TRI_vocbase_t* vocbase, bool waitForSync) {
  ...
  if (loadPersisted()) {
    ...
    if (_log.empty()) {
      ...
      VPackSlice value = darbotdb::velocypack::Slice::emptyObjectSlice();
  ...
  if (result.isArray() && result.length() > 0) {
    ...
    for (auto const& ii : VPackArrayIterator(result)) {
      ...
      if (index == lastIndex + 1 || (index == lastIndex && _log.empty())) {
        // Real entries
        buffer_t tmp =
            std::make_shared<darbotdb::velocypack::Buffer<uint8_t>>();
...
bool State::persistCompactionSnapshot(index_t cind,
                                      darbotdb::consensus::term_t term,
                                      darbotdb::consensus::Store& snapshot) {
  ...
  if (checkCollection("compact")) {
    ...
    try {
      ...
      if (!result.ok()) {
        if (result.is(TRI_ERROR_DDB_UNIQUE_CONSTRAINT_VIOLATED)) {
...
void State::persistActiveAgents(query_t const& active, query_t const& pool) {
  ...
  if (result.fail()) {
    THROW_DDB_EXCEPTION(result.result);
  }
...
query_t State::allLogs() const {
  ...
  auto compq = darbotdb::aql::Query::create(
      transaction::StandaloneContext::create(*_vocbase, origin),
      aql::QueryString(comp), nullptr);
  auto logsq = darbotdb::aql::Query::create(
      transaction::StandaloneContext::create(*_vocbase, origin),
  ...
  if (compqResult.result.fail()) {
    THROW_DDB_EXCEPTION(compqResult.result);
  }

  aql::QueryResult logsqResult = logsq->executeSync();

  if (logsqResult.result.fail()) {
    THROW_DDB_EXCEPTION(logsqResult.result);
  }
...
std::shared_ptr<VPackBuilder> State::latestAgencyState(TRI_vocbase_t& vocbase,
  ...
  auto query2 = darbotdb::aql::Query::create(
      transaction::StandaloneContext::create(vocbase, origin),
      aql::QueryString(aql), nullptr);
  ...
  if (queryResult2.result.fail()) {
    THROW_DDB_EXCEPTION(queryResult2.result);
  }
...
uint64_t State::toVelocyPack(index_t lastIndex, VPackBuilder& builder) const {
  ...
  auto logQuery = darbotdb::aql::Query::create(
      transaction::StandaloneContext::create(*_vocbase, origin),
  ...
  if (logQueryResult.result.fail()) {
    THROW_DDB_EXCEPTION(logQueryResult.result);
  }
  ...
  if (n > 0) {
    ...
    auto compQuery = darbotdb::aql::Query::create(
        transaction::StandaloneContext::create(*_vocbase, origin),
    ...
    if (compQueryResult.result.fail()) {
      THROW_DDB_EXCEPTION(compQueryResult.result);
    }
...
/*static*/ index_t State::extractIndexFromKey(
    darbotdb::velocypack::Slice data) {
  ...
  return index_t(darbotdb::basics::StringUtils::uint64(key, keyLength));
}
