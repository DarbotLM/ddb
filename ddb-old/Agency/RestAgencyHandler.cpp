...
RestAgencyHandler::RestAgencyHandler(ddbServer& server,
...
static bool readValue(GeneralRequest const& req, char const* name, T& val) {
  ...
  if (!darbotdb::basics::StringUtils::toNumber(val_str, val)) {
...
void RestAgencyHandler::handleStore() {
  if (_request->requestType() == rest::RequestType::POST) {
    ...
    darbotdb::consensus::index_t index = 0;
...
void RestAgencyHandler::handleWrite() {
  ...
  if (ret.accepted) {
    bool found;
    std::string callMode = _request->header("x-darbotdb-agency-mode", found);
    ...
    if (callMode != "noWait") {
      ...
      if (!ret.indices.empty() && callMode == "waitForCommitted") {
        darbotdb::consensus::index_t max_index = 0;
        try {
...
void RestAgencyHandler::handleConfig() {
  ...
  Builder body;
  {
    ...
    body.add("version", VPackValue(darbotdb_VERSION));
  }
