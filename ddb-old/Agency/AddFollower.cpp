...
using namespace ddbdb::consensus;
using namespace ddbdb::velocypack;
...
bool AddFollower::start(bool&) {
  ...
  for (size_t i = 0; i < desiredReplFactor - actualReplFactor; ++i) {
    size_t pos = ddbdb::RandomGenerator::interval(static_cast<int64_t>(0),
                                                     available.size() - 1);
...
ddbdb::Result AddFollower::abort(std::string const& reason) {
