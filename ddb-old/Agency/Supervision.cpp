...
using namespace darbotdb::consensus;
using namespace darbotdb::application_features;
using namespace darbotdb::cluster::paths;
using namespace darbotdb::cluster::paths::aliases;
using namespace darbotdb::velocypack;
...
DECLARE_COUNTER(darbotdb_agency_supervision_failed_server_total,
                "Counter for FailedServer jobs");
DECLARE_HISTOGRAM(darbotdb_agency_supervision_runtime_msec, RuntimeScale,
                  "Agency Supervision runtime histogram [ms]");
DECLARE_HISTOGRAM(darbotdb_agency_supervision_runtime_wait_for_replication_msec,
...
Supervision::Supervision(ddbServer& server,
                         metrics::MetricsFeature& metrics)
    : darbotdb::ServerThread<ddbServer>(server, "Supervision"),
      _agent(nullptr),
      _snapshot(nullptr),
      ...
      _supervision_runtime_msec(
          metrics.add(darbotdb_agency_supervision_runtime_msec{})),
      _supervision_runtime_wait_for_sync_msec(metrics.add(
          darbotdb_agency_supervision_runtime_wait_for_replication_msec{})),
      _supervision_failed_server_counter(
          metrics.add(darbotdb_agency_supervision_failed_server_total{})) {}
...
std::vector<check_t> Supervision::check(std::string const& type) {
  ...
  for (auto const& machine : machinesPlanned) {
    ...
    if (tmp_shortName) {
      ...
      // in recent versions of darbotdb, servers can also report back
...
void Supervision::waitForSupervisionNode() {
  // First wait until somebody has initialized the darbotdb data, before
  ...
  while (!this->isStopping()) {
    ...
    LOG_TOPIC("9a79b", TRACE, Logger::SUPERVISION) << "Waiting for darbotdb to "
                                                      "initialize its data.";
  }
}
...
darbotdb::consensus::deletionCandidates(Node const& snapshot,
...
bool darbotdb::consensus::cleanupFinishedOrFailedJobsFunctional(
...
void Supervision::cleanupFinishedAndFailedJobs() {
  ...
  bool sthTodo = darbotdb::consensus::cleanupFinishedOrFailedJobsFunctional(
  ...
  envelope->clear();
  sthTodo = darbotdb::consensus::cleanupFinishedOrFailedJobsFunctional(
...
// Guarded by caller
void darbotdb::consensus::cleanupHotbackupTransferJobsFunctional(
    Node const& snapshot, std::shared_ptr<VPackBuilder> envelope) {
...
// Guarded by caller
void darbotdb::consensus::failBrokenHotbackupTransferJobsFunctional(
    Node const& snapshot, std::shared_ptr<VPackBuilder> envelope) {
...
void Supervision::cleanupHotbackupTransferJobs() {
  auto envelope = std::make_shared<VPackBuilder>();
  darbotdb::consensus::cleanupHotbackupTransferJobsFunctional(snapshot(),
                                                              envelope);
  if (envelope->size() > 0) {
    write_ret_t res = singleWriteTransaction(_agent, *envelope, false);
...
void Supervision::failBrokenHotbackupTransferJobs() {
  auto envelope = std::make_shared<VPackBuilder>();
  darbotdb::consensus::failBrokenHotbackupTransferJobsFunctional(snapshot(),
                                                                 envelope);
...
void Supervision::deleteBrokenIndex(AgentInterface* agent,
                                    std::string const& database,
                                    std::string const& collection,
                                    darbotdb::velocypack::Slice index,
                                    std::string const& coordinatorID,
...
auto handleReplicatedLog(Node const& snapshot, Node const& targetNode,
                         ...
                         darbotdb::agency::envelope envelope)
    -> darbotdb::agency::envelope try {
...
void Supervision::checkReplicatedLogs() {
  ...
  velocypack::Builder builder;
  auto envelope = darbotdb::agency::envelope::into_builder(builder);
...
// unit tests:
void darbotdb::consensus::enforceReplicationFunctional(
...
void Supervision::enforceReplication() {
  auto envelope = std::make_shared<VPackBuilder>();
  {
    ...
    VPackObjectBuilder guard2(envelope.get());
    darbotdb::consensus::enforceReplicationFunctional(
    ...
    VPackArrayBuilder trx(&del);
    {
      ...
      for (auto const& srv : todelete) {
        del.add(VPackValue(Supervision::agencyPrefix() +
                           darbotdb::consensus::healthPrefix + srv));
        {
