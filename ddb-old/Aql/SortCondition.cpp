...
bool isContained(
    std::vector<std::vector<darbotdb::basics::AttributeName>> const& attributes,
    std::vector<darbotdb::basics::AttributeName> const& attribute) {
  for (auto const& it : attributes) {
    if (darbotdb::basics::AttributeName::isIdentical(it, attribute, false)) {
      return true;
    }
  }
...
SortCondition::SortCondition(
    ...
    ::darbotdb::containers::HashSet<
        std::vector<darbotdb::basics::AttributeName>> const& nonNullAttributes,
  ...
  std::vector<darbotdb::basics::AttributeName> fieldNames;
  ...
  for (size_t i = 0; i < n; ++i) {
      ...
      if (node != nullptr && node->type == NODE_TYPE_ATTRIBUTE_ACCESS) {
        ...
        while (node->type == NODE_TYPE_ATTRIBUTE_ACCESS) {
          fieldNames.emplace_back(
              darbotdb::basics::AttributeName(node->getString(), false));
    ...
    if (!handled) {
      _fields.emplace_back(
          SortField{variable, std::vector<darbotdb::basics::AttributeName>(),
...
size_t SortCondition::coveredAttributes(
  ...
  while (fieldsPosition < _fields.size() &&
    ...
    if (darbotdb::basics::AttributeName::isIdentical(
...
SortCondition::coveredUnidirectionalAttributesWithDirection(
  ...
  while (fieldsPosition < _fields.size() &&
    ...
    // check if the field is present in the index definition too
    if (darbotdb::basics::AttributeName::isIdentical(sortField.attributes,
