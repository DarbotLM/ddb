...
async<void> ExecutionEngine::instantiateFromPlan(Query& query,
  ...
  if (darbotdb::ServerState::isSingleServerOrCoordinator(role)) {
  ...
  if (ServerState::isCoordinator(role)) {
    ...
    EngineId eId =
        darbotdb::ServerState::isDBServer(role) ? TRI_NewTickServer() : 0;
...
void darbotdb::aql::ExecutionEngine::setupEngineRoot(ExecutionBlock& root) {
...
void darbotdb::aql::ExecutionEngine::initFromPlanForCalculation(
    ExecutionPlan& plan) {
  plan.findVarUsage();
...
void darbotdb::aql::ExecutionEngine::reset() {
  _root = nullptr;
  _blocks.clear();
...
std::vector<darbotdb::cluster::CallbackGuard>&
