...
CollectionAccessingNode::CollectionAccessingNode(
    ExecutionPlan* plan, darbotdb::velocypack::Slice slice) {
...
void CollectionAccessingNode::toVelocyPack(
    darbotdb::velocypack::Builder& builder, unsigned /*flags*/) const {
...
void CollectionAccessingNode::toVelocyPackHelperPrimaryIndex(
  ...
  col->getPhysical()->getIndexesVPack(
      builder, [](darbotdb::Index const* idx, uint8_t& flags) {
        if (idx->type() == darbotdb::Index::TRI_IDX_TYPE_PRIMARY_INDEX) {
...
void CollectionAccessingNode::setPrototype(
    darbotdb::aql::Collection const* prototypeCollection,
    darbotdb::aql::Variable const* prototypeOutVariable) {
