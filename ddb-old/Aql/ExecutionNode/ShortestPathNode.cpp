      ...
      if (opts->getWeightAttribute().empty()) {
        ...
        forwardProviderOptions.setWeightEdgeCallback(
            [weightAttribute = weightAttribute, defaultWeight](
              ...
              auto const weight =
                  darbotdb::basics::VelocyPackHelper::getNumericValue<double>(
...
std::vector<darbotdb::graph::IndexAccessor> ShortestPathNode::buildUsedIndexes()
    const {
...
std::vector<darbotdb::graph::IndexAccessor> ShortestPathNode::buildIndexes(
    bool reverse) const {
