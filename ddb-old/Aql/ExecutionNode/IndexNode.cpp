...
IndexNode::IndexNode(ExecutionPlan* plan,
                     darbotdb::velocypack::Slice const& base)
...
void IndexNode::setProjections(darbotdb::aql::Projections projections) {
...
void IndexNode::doToVelocyPack(VPackBuilder& builder, unsigned flags) const {
  ...
  // this attribute is never read back by ddb, but it is used a lot
...
darbotdb::aql::AstNode* IndexNode::makeUnique(
    darbotdb::aql::AstNode* node) const {
  if (node->type != darbotdb::aql::NODE_TYPE_ARRAY || node->numMembers() >= 2) {
...
std::vector<Variable const*> IndexNode::getVariablesSetHere() const {
  ...
  std::vector<darbotdb::aql::Variable const*> vars;
