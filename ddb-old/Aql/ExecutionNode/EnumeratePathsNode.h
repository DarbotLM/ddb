...
class EnumeratePathsNode : public virtual GraphNode {
 ...
 public:
  ...
  darbotdb::graph::PathType::Type pathType() const { return _pathType; }
  ...
  std::vector<darbotdb::graph::IndexAccessor> buildUsedIndexes() const;

  std::vector<darbotdb::graph::IndexAccessor> buildReverseUsedIndexes() const;
 ...
 private:
  std::vector<darbotdb::graph::IndexAccessor> buildIndexes(bool reverse) const;

  void enumeratePathsCloneHelper(ExecutionPlan& plan,
                                 EnumeratePathsNode& c) const;

  /// @brief algorithm type (K_SHORTEST_PATHS or K_PATHS)
  darbotdb::graph::PathType::Type _pathType;
  ...
  std::unique_ptr<ExecutionBlock> _makeExecutionBlockImpl(
      ...
      darbotdb::graph::PathValidatorOptions validatorOptions,
