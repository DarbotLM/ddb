...
class PlanChecker
 ...
 public:
  ...
  bool before(ExecutionNode* node) override {
    ...
    if (!ok) {
      LOG_TOPIC("d45f8", ERR, darbotdb::Logger::AQL) << "Inconsistent plan:";
      _plan.show();
      LOG_TOPIC("c14a2", ERR, darbotdb::Logger::AQL)
          << "encountered the following error(s):";
      for (auto const& err : errors) {
        LOG_TOPIC("17a18", ERR, darbotdb::Logger::AQL) << err.str();
      }
    }
    TRI_ASSERT(ok);
...
#endif  // darbotdb_ENABLE_MAINTAINER_MODE
