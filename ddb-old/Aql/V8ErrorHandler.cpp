...
void handleV8Error(v8::TryCatch& tryCatch, v8::Handle<v8::Value>& result) {
  ...
  if (tryCatch.HasCaught()) {
    ...
    if (tryCatch.Exception()->IsObject()) {
      ...
      if (TRI_HasProperty(context, isolate, objValue, errorNum) &&
        ...
        // found something that looks like an ddbError
        if ((errorNumValue->IsNumber() || errorNumValue->IsNumberObject()) &&
          ...
          THROW_ddb_EXCEPTION_MESSAGE(errorCode, errorMessage);
        }
      }

      // exception is no ddbError
      ...
      THROW_ddb_EXCEPTION_MESSAGE(TRI_ERROR_QUERY_SCRIPT, details);
    }
  ...
  if (failed) {
    ...
    THROW_ddb_EXCEPTION_MESSAGE(TRI_ERROR_QUERY_SCRIPT,
