...
bool willUseV8(darbotdb::aql::ExecutionPlan const& plan) {
  struct V8Checker
      : darbotdb::aql::WalkerWorkerBase<darbotdb::aql::ExecutionNode> {
    bool before(darbotdb::aql::ExecutionNode* n) override {
      if (n->getType() == darbotdb::aql::ExecutionNode::CALCULATION &&
          static_cast<darbotdb::aql::CalculationNode*>(n)
              ->expression()
              ->willUseV8()) {
        result = true;
        return true;
...
bool accessesCollectionVariable(darbotdb::aql::ExecutionPlan const* plan,
                                darbotdb::aql::ExecutionNode const* node,
                                darbotdb::aql::VarSet& vars) {
  using EN = darbotdb::aql::ExecutionNode;

  if (node->getType() == EN::CALCULATION) {
    auto nn = EN::castTo<darbotdb::aql::CalculationNode const*>(node);
    vars.clear();
    darbotdb::aql::Ast::getReferencedVariables(nn->expression()->node(), vars);
  } else if (node->getType() == EN::SUBQUERY) {
    auto nn = EN::castTo<darbotdb::aql::SubqueryNode const*>(node);
    vars.clear();
...
std::optional<darbotdb::ShardID> getSingleShardId(
    darbotdb::aql::ExecutionPlan const* plan,
    darbotdb::aql::ExecutionNode const* node,
    darbotdb::aql::Collection const* collection,
    darbotdb::aql::Variable const* collectionVariable = nullptr);

darbotdb::aql::Variable const* getOutVariable(
    darbotdb::aql::ExecutionNode const* node) {
  using EN = darbotdb::aql::ExecutionNode;
  using darbotdb::aql::ExecutionNode;

  switch (node->getType()) {
    case EN::CALCULATION:
      return ExecutionNode::castTo<darbotdb::aql::CalculationNode const*>(node)
          ->outVariable();
    default: {
      auto const* n =
          dynamic_cast<darbotdb::aql::DocumentProducingNode const*>(node);
      if (n != nullptr) {
...
void replaceGatherNodeVariables(
    darbotdb::aql::ExecutionPlan* plan, darbotdb::aql::GatherNode* gatherNode,
    std::unordered_map<darbotdb::aql::Variable const*,
                       darbotdb::aql::Variable const*> const& replacements) {
  using EN = darbotdb::aql::ExecutionNode;
  ...
  darbotdb::aql::SortElementVector& elements = gatherNode->elements();
  for (auto& it : elements) {
    ...
    if (it2 != replacements.end()) {
      ...
      for (auto const& it3 : replacements) {
        ...
        auto* expr = darbotdb::aql::ExecutionNode::castTo<
                         darbotdb::aql::CalculationNode const*>(setter)
                         ->expression();
        buffer.clear();
...
void restrictToShard(darbotdb::aql::ExecutionNode* node,
                     darbotdb::ShardID const& shardId) {
  auto* n = dynamic_cast<darbotdb::aql::CollectionAccessingNode*>(node);
  if (n != nullptr) {
...
class CollectionVariableTracker final
    : public darbotdb::aql::WalkerWorker<
          darbotdb::aql::ExecutionNode,
          darbotdb::aql::WalkerUniqueness::NonUnique> {
  using DependencyPair = std::pair<darbotdb::aql::Variable const*,
                                   darbotdb::aql::Collection const*>;
  using DependencySet = std::unordered_set<DependencyPair, ::PairHash>;
  bool _stop;
  std::unordered_map<darbotdb::aql::Variable const*, DependencySet>
      _dependencies;
  std::unordered_map<darbotdb::aql::Collection const*, darbotdb::aql::VarSet>
      _collectionVariables;

 private:
  template<class NodeType>
  void processSetter(darbotdb::aql::ExecutionNode const* en,
                     darbotdb::aql::Variable const* outVariable) {
    auto node = darbotdb::aql::ExecutionNode::castTo<NodeType const*>(en);
    try {
      darbotdb::aql::VarSet inputVariables;
  ...
  template<class NodeType>
  void processModificationNode(darbotdb::aql::ExecutionNode const* en) {
    auto node = darbotdb::aql::ExecutionNode::castTo<NodeType const*>(en);
    auto collection = node->collection();
    std::vector<darbotdb::aql::Variable const*> outVariables{
 ...
 public:
  ...
  DependencySet const& getDependencies(darbotdb::aql::Variable const* var) {
    return _dependencies[var];
  }

  void after(darbotdb::aql::ExecutionNode* en) override final {
    using EN = darbotdb::aql::ExecutionNode;
    using darbotdb::aql::ExecutionNode;
    ...
    switch (en->getType()) {
      case EN::CALCULATION: {
        ...
        processSetter<darbotdb::aql::CalculationNode>(en, outVariable);
        break;
      }

      case EN::INDEX:
      case EN::ENUMERATE_COLLECTION: {
        auto collection = darbotdb::aql::utils::getCollection(en);
      ...
      case EN::UPDATE: {
        processModificationNode<darbotdb::aql::UpdateNode>(en);
        break;
      }

      case EN::UPSERT: {
        processModificationNode<darbotdb::aql::UpsertNode>(en);
        break;
      }

      case EN::INSERT: {
        processModificationNode<darbotdb::aql::InsertNode>(en);
        break;
      }

      case EN::REMOVE: {
        processModificationNode<darbotdb::aql::RemoveNode>(en);
        break;
      }

      case EN::REPLACE: {
        processModificationNode<darbotdb::aql::ReplaceNode>(en);
        break;
      }
...
class RestrictToSingleShardChecker final
  ...
  darbotdb::aql::ExecutionPlan* _plan;
  CollectionVariableTracker& _tracker;
  std::unordered_map<
      darbotdb::aql::Variable const*,
      std::variant<AllShards, std::unordered_set<darbotdb::ShardID>>>
      _shardsUsed;
  std::unordered_map<
      darbotdb::aql::Variable const*,
      std::variant<AllShards, std::unordered_set<darbotdb::ShardID>>>
      _shardsCleared;
  bool _stop;
  std::map<darbotdb::aql::Collection const*, bool> _unsafe;

 public:
  explicit RestrictToSingleShardChecker(darbotdb::aql::ExecutionPlan* plan,
  ...
  darbotdb::ShardID getShard(darbotdb::aql::Variable const* variable) const {
    auto const& it = _shardsCleared.find(variable);
    if (it == _shardsCleared.end()) {
      return darbotdb::ShardID::invalidShard();
    }
    ...
    if (std::holds_alternative<AllShards>(set)) {
      ...
      auto const& shardList =
          std::get<std::unordered_set<darbotdb::ShardID>>(set);
      return *shardList.begin();
    }
  }

  bool isSafeForOptimization(
      darbotdb::aql::Collection const* collection) const {
  ...
  bool isSafeForOptimization(darbotdb::aql::Variable const* variable) const {
    ...
    if (std::holds_alternative<AllShards>(it->second)) {
      ...
      return std::get<std::unordered_set<darbotdb::ShardID>>(it->second)
                 .size() == 1;
    }
  }

  bool enterSubquery(darbotdb::aql::ExecutionNode*,
                     darbotdb::aql::ExecutionNode*) override final {
    return true;
  }

  bool before(darbotdb::aql::ExecutionNode* en) override final {
    using EN = darbotdb::aql::ExecutionNode;
    using darbotdb::aql::ExecutionNode;
    ...
    switch (en->getType()) {
      ...
      case EN::FILTER: {
        auto node = ExecutionNode::castTo<darbotdb::aql::FilterNode const*>(en);
        darbotdb::aql::Variable const* inputVariable = node->inVariable();
      ...
      case EN::REMOVE: {
        auto node =
            ExecutionNode::castTo<darbotdb::aql::ModificationNode const*>(en);
 ...
 private:
  void handleShardOutput(std::optional<darbotdb::ShardID> shardId,
                         darbotdb::aql::Variable const* variable) {
    if (!shardId.has_value()) {
      ...
      if (it == _shardsUsed.end() ||
        ...
        _shardsUsed[variable] =
            std::unordered_set<darbotdb::ShardID>{std::move(shardId.value())};
      } else {
        std::get<std::unordered_set<darbotdb::ShardID>>(it->second)
            .emplace(std::move(shardId.value()));
      }
    }
  }

  void handleInputVariable(darbotdb::aql::ExecutionNode const* en,
                           darbotdb::aql::Variable const* inputVariable) {
  ...
  void handleDocumentNode(darbotdb::aql::ExecutionNode const* en) {
    TRI_ASSERT(en->getType() == darbotdb::aql::ExecutionNode::INDEX ||
               en->getType() ==
                   darbotdb::aql::ExecutionNode::ENUMERATE_COLLECTION);
    auto collection = darbotdb::aql::utils::getCollection(en);
    auto variable = ::getOutVariable(en);
    auto shardId = ::getSingleShardId(_plan, en, collection, variable);
    handleShardOutput(std::move(shardId), variable);
  ...
  void handleSourceNode(darbotdb::aql::ExecutionNode const* en) {
    auto variable = ::getOutVariable(en);
...
void findShardKeyInComparison(darbotdb::aql::AstNode const* root,
                              darbotdb::aql::Variable const* inputVariable,
                              std::unordered_set<std::string>& toFind,
                              darbotdb::velocypack::Builder& builder) {
  using darbotdb::aql::AstNode;
  using darbotdb::aql::Variable;
  TRI_ASSERT(root->type ==
             darbotdb::aql::AstNodeType::NODE_TYPE_OPERATOR_BINARY_EQ);

  AstNode const* value = nullptr;
  std::pair<Variable const*, std::vector<darbotdb::basics::AttributeName>> pair;
...
void findShardKeysInExpression(darbotdb::aql::AstNode const* root,
                               darbotdb::aql::Variable const* inputVariable,
                               std::unordered_set<std::string>& toFind,
                               darbotdb::velocypack::Builder& builder) {
  if (root == nullptr) {
  ...
  switch (root->type) {
    case darbotdb::aql::AstNodeType::NODE_TYPE_OPERATOR_NARY_OR: {
      ...
      if (root == nullptr ||
          root->type !=
              darbotdb::aql::AstNodeType::NODE_TYPE_OPERATOR_NARY_AND) {
        return;
      }
    }  // falls through
    case darbotdb::aql::AstNodeType::NODE_TYPE_OPERATOR_BINARY_AND:
    case darbotdb::aql::AstNodeType::NODE_TYPE_OPERATOR_NARY_AND: {
    ...
    case darbotdb::aql::AstNodeType::NODE_TYPE_OPERATOR_BINARY_EQ: {
...
static constexpr std::initializer_list<darbotdb::aql::ExecutionNode::NodeType>
    removeUnnecessaryCalculationsNodeTypes{
        darbotdb::aql::ExecutionNode::CALCULATION,
        darbotdb::aql::ExecutionNode::SUBQUERY};
static constexpr std::initializer_list<darbotdb::aql::ExecutionNode::NodeType>
    interchangeAdjacentEnumerationsNodeTypes{
        darbotdb::aql::ExecutionNode::ENUMERATE_COLLECTION,
        darbotdb::aql::ExecutionNode::ENUMERATE_LIST};
static constexpr std::initializer_list<darbotdb::aql::ExecutionNode::NodeType>
    scatterInClusterNodeTypes{
        darbotdb::aql::ExecutionNode::ENUMERATE_COLLECTION,
        darbotdb::aql::ExecutionNode::ENUMERATE_NEAR_VECTORS,
        darbotdb::aql::ExecutionNode::INDEX,
        darbotdb::aql::ExecutionNode::MATERIALIZE,
        darbotdb::aql::ExecutionNode::ENUMERATE_IRESEARCH_VIEW,
        darbotdb::aql::ExecutionNode::INSERT,
        darbotdb::aql::ExecutionNode::UPDATE,
        darbotdb::aql::ExecutionNode::REPLACE,
        darbotdb::aql::ExecutionNode::REMOVE,
        darbotdb::aql::ExecutionNode::UPSERT};
static constexpr std::initializer_list<darbotdb::aql::ExecutionNode::NodeType>
...
static constexpr std::initializer_list<
    darbotdb::aql::ExecutionNode::NodeType> const moveFilterIntoEnumerateTypes{
    darbotdb::aql::ExecutionNode::ENUMERATE_COLLECTION,
    darbotdb::aql::ExecutionNode::INDEX,
    darbotdb::aql::ExecutionNode::ENUMERATE_LIST};
static constexpr std::initializer_list<darbotdb::aql::ExecutionNode::NodeType>
    undistributeNodeTypes{darbotdb::aql::ExecutionNode::UPDATE,
                          darbotdb::aql::ExecutionNode::REPLACE,
                          darbotdb::aql::ExecutionNode::REMOVE};
...
std::optional<darbotdb::ShardID> getSingleShardId(
    ...
    darbotdb::aql::ExecutionNode const* node,
    darbotdb::aql::Collection const* collection,
    darbotdb::aql::Variable const* collectionVariable) {
  using EN = darbotdb::aql::ExecutionNode;
  using darbotdb::aql::ExecutionNode;
  ...
  darbotdb::aql::Variable const* inputVariable = nullptr;
  if (node->getType() == EN::INDEX ||
      node->getType() == EN::ENUMERATE_COLLECTION) {
    inputVariable =
        ExecutionNode::castTo<darbotdb::aql::DocumentProducingNode const*>(node)
            ->outVariable();
  } else if (node->getType() == EN::FILTER) {
    inputVariable =
        ExecutionNode::castTo<darbotdb::aql::FilterNode const*>(node)
            ->inVariable();
  } else if (node->getType() == EN::INSERT) {
    inputVariable =
        ExecutionNode::castTo<darbotdb::aql::InsertNode const*>(node)
            ->inVariable();
  } else if (node->getType() == EN::REMOVE) {
    inputVariable =
        ExecutionNode::castTo<darbotdb::aql::RemoveNode const*>(node)
            ->inVariable();
  } else if (node->getType() == EN::REPLACE || node->getType() == EN::UPDATE) {
    auto updateReplaceNode =
        ExecutionNode::castTo<darbotdb::aql::UpdateReplaceNode const*>(node);
    ...
    THROW_ddb_EXCEPTION_MESSAGE(TRI_ERROR_INTERNAL, "logic error");
  }
  ...
  auto sb = darbotdb::velocypack::SupervisedBuffer(
      plan->getAst()->query().resourceMonitor());
  VPackBuilder builder(sb);
  builder.openObject();

  if (setter->getType() == EN::CALCULATION) {
    darbotdb::aql::CalculationNode const* c =
        ExecutionNode::castTo<darbotdb::aql::CalculationNode const*>(setter);
    ...
    if (n->isStringValue()) {
      if (!n->isConstant() || toFind.size() != 1 ||
          toFind.find(darbotdb::StaticStrings::KeyString) == toFind.end()) {
        return std::nullopt;
      }

      // the lookup value is a string, and the only shard key is _key: so we
      // can use it
      builder.add(VPackValue(darbotdb::StaticStrings::KeyString));
      ...
      for (size_t i = 0; i < n->numMembers(); ++i) {
        ...
        if (sub->type != darbotdb::aql::AstNodeType::NODE_TYPE_OBJECT_ELEMENT) {
          continue;
        }
    ...
    auto const* c =
        ExecutionNode::castTo<darbotdb::aql::IndexNode const*>(setter);
    ...
    if (condition != nullptr) {
      darbotdb::aql::AstNode const* root = condition->root();
  ...
  if (!toFind.empty() && (node->getType() == EN::INDEX ||
                          node->getType() == EN::ENUMERATE_COLLECTION)) {
    auto en = dynamic_cast<darbotdb::aql::DocumentProducingNode const*>(node);
    TRI_ASSERT(en != nullptr);

    if (en->hasFilter()) {
      darbotdb::aql::AstNode const* root = en->filter()->node();
      ::findShardKeysInExpression(root, inputVariable, toFind, builder);
    }
  }
  ...
  if (node->getType() == EN::INSERT && collection->numberOfShards() != 1 &&
      (shardKeys.size() != 1 ||
       shardKeys[0] != darbotdb::StaticStrings::KeyString) &&
      builder.slice().get(darbotdb::StaticStrings::KeyString).isNone()) {
...
bool shouldApplyHeapOptimization(darbotdb::aql::SortNode& sortNode,
                                 darbotdb::aql::LimitNode& limitNode) {
...
bool applyGraphProjections(darbotdb::aql::TraversalNode* traversal) {
  auto* options =
      static_cast<darbotdb::traverser::TraverserOptions*>(traversal->options());
  darbotdb::containers::FlatHashSet<darbotdb::aql::AttributeNamePath>
      attributes;
  bool modified = false;
  ...
  if (traversal->vertexOutVariable() != nullptr) {
    useVertexProjections = darbotdb::aql::utils::findProjections(
  ...
  if (useVertexProjections && options->producePathsVertices() &&
    ...
    useVertexProjections = darbotdb::aql::utils::findProjections(
        traversal, pathOutVariable, darbotdb::StaticStrings::GraphQueryVertices,
        /*excludeStartNodeFilterCondition*/ false, attributes);
  }
  ...
  if (useVertexProjections && !attributes.empty() &&
    ...
    traversal->setVertexProjections(
        darbotdb::aql::Projections(std::move(attributes)));
    modified = true;
  }
  ...
  if (traversal->edgeOutVariable() != nullptr) {
    useEdgeProjections = darbotdb::aql::utils::findProjections(
  ...
  if (useEdgeProjections && options->producePathsEdges() &&
      pathOutVariable != nullptr) {
    useEdgeProjections = darbotdb::aql::utils::findProjections(
        traversal, pathOutVariable, darbotdb::StaticStrings::GraphQueryEdges,
        /*excludeStartNodeFilterCondition*/ false, attributes);
  }
  ...
  if (useEdgeProjections) {
    ...
    if (darbotdb::ServerState::instance()->isCoordinator() &&
      ...
      darbotdb::aql::AttributeNamePath idElement = {
          darbotdb::StaticStrings::IdString,
      ...
      if (traversal->options()->mode ==
          darbotdb::traverser::TraverserOptions::Order::WEIGHTED) {
        darbotdb::aql::AttributeNamePath weightElement = {
    ...
    darbotdb::aql::AttributeNamePath fromElement = {
        darbotdb::StaticStrings::FromString,
        traversal->plan()->getAst()->query().resourceMonitor()};
    attributes.emplace(std::move(fromElement));

    darbotdb::aql::AttributeNamePath toElement = {
        darbotdb::StaticStrings::ToString,
        traversal->plan()->getAst()->query().resourceMonitor()};
...
using namespace darbotdb::containers;
...
void darbotdb::aql::sortInValuesRule(Optimizer* opt,
  ...
  for (auto const& n : nodes) {
    ...
    if (setter->getType() == EN::CALCULATION) {
      ...
      if (testNode->type == NODE_TYPE_FCALL &&
        ...
        auto sg =
            darbotdb::scopeGuard([&]() noexcept { FINALIZE_SUBTREE(clone); });
    ...
    AstNode* clone = ast->shallowCopyForModify(inNode);
    auto sg = darbotdb::scopeGuard([&]() noexcept { FINALIZE_SUBTREE(clone); });
    // set sortedness bit for the IN operator
    clone->setBoolValue(true);
    // finally adjust the variable inside the IN calculation
...
void darbotdb::aql::removeRedundantSortsRule(
  ...
  ::darbotdb::containers::HashSet<ExecutionNode*> toUnlink;
...
void darbotdb::aql::removeUnnecessaryFiltersRule(
    Optimizer* opt, std::unique_ptr<ExecutionPlan> plan,
    OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
  plan->findNodesOfType(nodes, EN::FILTER, true);
...
void darbotdb::aql::removeCollectVariablesRule(
    Optimizer* opt, std::unique_ptr<ExecutionPlan> plan,
    OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
...
void darbotdb::aql::moveCalculationsUpRule(Optimizer* opt,
                                           std::unique_ptr<ExecutionPlan> plan,
                                           OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
...
void darbotdb::aql::moveCalculationsDownRule(
    Optimizer* opt, std::unique_ptr<ExecutionPlan> plan,
    OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
      ...
      if (n->getType() == EN::CALCULATION && currentType == EN::SUBQUERY &&
        ...
        if (currentType == EN::FILTER || currentType == EN::SORT ||
          ...
          if (currentType == EN::LIMIT &&
              darbotdb::ServerState::instance()->isCoordinator()) {
...
void darbotdb::aql::specializeCollectRule(Optimizer* opt,
                                          std::unique_ptr<ExecutionPlan> plan,
                                          OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
  plan->findNodesOfType(nodes, EN::COLLECT, true);
...
void darbotdb::aql::moveFiltersUpRule(Optimizer* opt,
                                      std::unique_ptr<ExecutionPlan> plan,
                                      OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
  plan->findNodesOfType(nodes, EN::FILTER, true);
...
void darbotdb::aql::simplifyConditionsRule(Optimizer* opt,
                                           std::unique_ptr<ExecutionPlan> plan,
                                           OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
  plan->findNodesOfType(nodes, EN::CALCULATION, true);
...
void darbotdb::aql::fuseFiltersRule(Optimizer* opt,
                                    std::unique_ptr<ExecutionPlan> plan,
                                    OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
  plan->findNodesOfType(nodes, EN::FILTER, true);
  ...
  ::darbotdb::containers::HashSet<ExecutionNode*> seen;
...
void darbotdb::aql::removeRedundantCalculationsRule(
    Optimizer* opt, std::unique_ptr<ExecutionPlan> plan,
    OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
  plan->findNodesOfType(nodes, EN::CALCULATION, true);
  ...
  for (auto const& n : nodes) {
    ...
    darbotdb::aql::Variable const* outvar = nn->outVariable();
...
void darbotdb::aql::removeUnnecessaryCalculationsRule(
    Optimizer* opt, std::unique_ptr<ExecutionPlan> plan,
    OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
...
void darbotdb::aql::useIndexesRule(Optimizer* opt,
                                   std::unique_ptr<ExecutionPlan> plan,
                                   OptimizerRule const& rule) {
...
struct SortToIndexNode final
 ...
 public:
  ...
  void getSpecialAttributes(
      ...
      std::vector<std::vector<darbotdb::basics::AttributeName>>&
          constAttributes,
      ::darbotdb::containers::HashSet<
          std::vector<darbotdb::basics::AttributeName>>& nonNullAttributes)
      const {
    ...
    std::pair<Variable const*, std::vector<darbotdb::basics::AttributeName>>
        result;
  ...
  bool handleEnumerateCollectionNode(
    ...
    std::vector<std::vector<darbotdb::basics::AttributeName>> constAttributes;
    ::darbotdb::containers::HashSet<
        std::vector<darbotdb::basics::AttributeName>>
        nonNullAttributes;
    ...
    if (!sortCondition.isEmpty() && sortCondition.isOnlyAttributeAccess()) {
      ...
      bool canBeUsed = darbotdb::aql::utils::getIndexForSortCondition(
  ...
  bool handleIndexNode(IndexNode* indexNode) {
    ...
    std::vector<std::vector<darbotdb::basics::AttributeName>> fields =
        index->fields();
...
void darbotdb::aql::useIndexForSortRule(Optimizer* opt,
                                        std::unique_ptr<ExecutionPlan> plan,
                                        OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
  plan->findNodesOfType(nodes, EN::SORT, true);
...
void darbotdb::aql::removeFiltersCoveredByIndexRule(
    Optimizer* opt, std::unique_ptr<ExecutionPlan> plan,
    OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
  plan->findNodesOfType(nodes, EN::FILTER, true);
...
void darbotdb::aql::interchangeAdjacentEnumerationsRule(
    Optimizer* opt, std::unique_ptr<ExecutionPlan> plan,
    OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
  ...
  ::darbotdb::containers::HashSet<ExecutionNode*> nodesSet;
  for (auto const& n : nodes) {
...
void darbotdb::aql::insertScatterGatherSnippet(
...
void moveScatterAbove(ExecutionPlan& plan, ExecutionNode* at) {
  ...
  if (!found) {
#ifdef darbotdb_ENABLE_MAINTAINER_MODE
    plan.show();
#endif
    // TODO: maybe we should *not* throw in maintainer mode, as the optimizer
    //       gives more useful error messages?
    THROW_ddb_EXCEPTION_MESSAGE(TRI_ERROR_INTERNAL, "Inconsistent plan.");
  }
}
...
void darbotdb::aql::findSubqueriesInPlan(
    ExecutionPlan& plan,
...
void darbotdb::aql::scatterInClusterRule(Optimizer* opt,
                                         std::unique_ptr<ExecutionPlan> plan,
                                         OptimizerRule const& rule) {
...
auto darbotdb::aql::createDistributeNodeFor(ExecutionPlan& plan,
                                            ExecutionNode* node)
    -> DistributeNode* {
...
auto darbotdb::aql::createGatherNodeFor(ExecutionPlan& plan,
...
//
auto darbotdb::aql::insertDistributeGatherSnippet(ExecutionPlan& plan,
                                                  ExecutionNode* at,
                                                  SubqueryNode* snode)
    -> DistributeNode* {
...
void darbotdb::aql::distributeInClusterRule(Optimizer* opt,
                                            std::unique_ptr<ExecutionPlan> plan,
                                            OptimizerRule const& rule) {
  TRI_ASSERT(darbotdb::ServerState::instance()->isCoordinator());
  bool wasModified = false;
...
void darbotdb::aql::collectInClusterRule(Optimizer* opt,
                                         std::unique_ptr<ExecutionPlan> plan,
                                         OptimizerRule const& rule) {
  TRI_ASSERT(darbotdb::ServerState::instance()->isCoordinator());
  bool wasModified = false;
...
/// filters are moved as far up in the plan as possible to make result sets
/// as small as possible as early as possible
void darbotdb::aql::distributeFilterCalcToClusterRule(
    Optimizer* opt, std::unique_ptr<ExecutionPlan> plan,
    OptimizerRule const& rule) {
  bool modified = false;
...
///
/// filters are not pushed beyond limits
void darbotdb::aql::distributeSortToClusterRule(
    Optimizer* opt, std::unique_ptr<ExecutionPlan> plan,
    OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
  VarSet usedBySort;
...
void darbotdb::aql::removeUnnecessaryRemoteScatterRule(
    Optimizer* opt, std::unique_ptr<ExecutionPlan> plan,
    OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
...
void darbotdb::aql::restrictToSingleShardRule(
    Optimizer* opt, std::unique_ptr<ExecutionPlan> plan,
    OptimizerRule const& rule) {
  TRI_ASSERT(darbotdb::ServerState::instance()->isCoordinator());
  bool wasModified = false;
      ...
      if (currentType == ExecutionNode::INSERT ||
        ...
        if (shardId.has_value()) {
          ...
          if (deps.size() && deps[0]->getType() == ExecutionNode::REMOTE) {
            ...
            ::darbotdb::containers::HashSet<ExecutionNode*> toRemove;
...
class RemoveToEnumCollFinder final
    : public WalkerWorker<ExecutionNode, WalkerUniqueness::NonUnique> {
  ExecutionPlan* _plan;
  ::darbotdb::containers::HashSet<ExecutionNode*>& _toUnlink;
      ...
      case EN::REMOVE: {
        ...
        if (_setter->getType() == EN::CALCULATION) {
          ...
          if (expr->isAttributeAccess()) {
            ...
            toFind.emplace(darbotdb::StaticStrings::KeyString);
        ...
        if (projections.isSingle(darbotdb::StaticStrings::KeyString)) {
...
void darbotdb::aql::undistributeRemoveAfterEnumCollRule(
    Optimizer* opt, std::unique_ptr<ExecutionPlan> plan,
    OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
...
void darbotdb::aql::replaceOrWithInRule(Optimizer* opt,
                                        std::unique_ptr<ExecutionPlan> plan,
                                        OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
  plan->findNodesOfType(nodes, EN::FILTER, true);
...
void darbotdb::aql::removeRedundantOrRule(Optimizer* opt,
                                          std::unique_ptr<ExecutionPlan> plan,
                                          OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
  plan->findNodesOfType(nodes, EN::FILTER, true);
...
/// if not required
void darbotdb::aql::removeDataModificationOutVariablesRule(
    Optimizer* opt, std::unique_ptr<ExecutionPlan> plan,
    OptimizerRule const& rule) {
  bool modified = false;
...
void darbotdb::aql::optimizeTraversalsRule(Optimizer* opt,
                                           std::unique_ptr<ExecutionPlan> plan,
                                           OptimizerRule const& rule) {
  ...
  std::unordered_set<darbotdb::aql::AttributeNamePath> attributes;
  bool modified = false;
  ...
  for (auto const& n : tNodes) {
    ...
    auto* options = static_cast<darbotdb::traverser::TraverserOptions*>(
        traversal->options());
      ...
      if (!ServerState::instance()->isCoordinator()) {
        if (traversal->vertexOutVariable() != nullptr &&
            ...
            options->uniqueVertices ==
                darbotdb::traverser::TraverserOptions::GLOBAL &&
...
void darbotdb::aql::optimizePathsRule(Optimizer* opt,
                                      std::unique_ptr<ExecutionPlan> plan,
                                      OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
  ...
  for (auto const& n : nodes) {
    ...
    darbotdb::ResourceMonitor& resMonitor =
        plan->getAst()->query().resourceMonitor();
...
void darbotdb::aql::removeFiltersCoveredByTraversal(
    Optimizer* opt, std::unique_ptr<ExecutionPlan> plan,
    OptimizerRule const& rule) {
...
void darbotdb::aql::removeTraversalPathVariable(
    Optimizer* opt, std::unique_ptr<ExecutionPlan> plan,
    OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> tNodes;
  plan->findNodesOfType(tNodes, EN::TRAVERSAL, true);
...
///   LET x = tmp.test
///   RETURN x.a
void darbotdb::aql::inlineSubqueriesRule(Optimizer* opt,
                                         std::unique_ptr<ExecutionPlan> plan,
                                         OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
  plan->findNodesOfType(nodes, EN::SUBQUERY, true);
...
static bool geoFuncArgCheck(ExecutionPlan* plan, AstNode const* args,
  ...
  auto indexes = collNode->collection()->indexes();
  // check for suitiable indexes
  for (std::shared_ptr<darbotdb::Index> idx : indexes) {
    // check if current index is a geo-index
    bool isGeo =
        idx->type() == darbotdb::Index::IndexType::TRI_IDX_TYPE_GEO_INDEX;
...
static bool isValidGeoArg(AstNode const* lhs, AstNode const* rhs) {
  ...
  std::pair<Variable const*, std::vector<darbotdb::basics::AttributeName>> res1,
      res2;
  ...
  Ast::traverseReadOnly(
      ...
      [&](AstNode const* node) {  // pre
        if (node->isSimpleComparisonOperator() ||
            node->type == darbotdb::aql::NODE_TYPE_FCALL ||
            node->type == darbotdb::aql::NODE_TYPE_OPERATOR_BINARY_AND ||
            node->type == darbotdb::aql::NODE_TYPE_OPERATOR_NARY_AND) {
          return true;
        }
        return false;
      },
...
static std::unique_ptr<Condition> buildGeoCondition(ExecutionPlan* plan,
  ...
  if (hasCenter && (hasDistLimit || info.sorted)) {
    ...
    if (info.minDistanceExpr == nullptr && info.maxDistanceExpr == nullptr &&
      ...
      std::string const& u = darbotdb::StaticStrings::Unlimited;
...
void darbotdb::aql::geoIndexRule(Optimizer* opt,
                                 std::unique_ptr<ExecutionPlan> plan,
                                 OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
  bool mod = false;
...
void darbotdb::aql::sortLimitRule(Optimizer* opt,
                                  std::unique_ptr<ExecutionPlan> plan,
                                  OptimizerRule const& rule) {
  bool mod = false;
...
void darbotdb::aql::optimizeSubqueriesRule(Optimizer* opt,
                                           std::unique_ptr<ExecutionPlan> plan,
                                           OptimizerRule const& rule) {
  bool modified = false;
...
void darbotdb::aql::moveFiltersIntoEnumerateRule(
    Optimizer* opt, std::unique_ptr<ExecutionPlan> plan,
    OptimizerRule const& rule) {
  bool modified = false;
...
/// operation
void darbotdb::aql::optimizeCountRule(Optimizer* opt,
                                      std::unique_ptr<ExecutionPlan> plan,
                                      OptimizerRule const& rule) {
  bool modified = false;
...
void darbotdb::aql::parallelizeGatherRule(Optimizer* opt,
                                          std::unique_ptr<ExecutionPlan> plan,
                                          OptimizerRule const& rule) {
...
void darbotdb::aql::asyncPrefetchRule(Optimizer* opt,
                                      std::unique_ptr<ExecutionPlan> plan,
                                      OptimizerRule const& rule) {
...
void darbotdb::aql::activateCallstackSplit(ExecutionPlan& plan) {
  if (willUseV8(plan)) {
...
// in between.
void darbotdb::aql::spliceSubqueriesRule(Optimizer* opt,
                                         std::unique_ptr<ExecutionPlan> plan,
                                         OptimizerRule const& rule) {
  bool modified = false;
...
void darbotdb::aql::decayUnnecessarySortedGather(
    Optimizer* opt, std::unique_ptr<ExecutionPlan> plan,
    OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
  plan->findNodesOfType(nodes, EN::GATHER, true);
...
void darbotdb::aql::insertDistributeInputCalculation(ExecutionPlan& plan) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
...
void darbotdb::aql::optimizeProjections(Optimizer* opt,
                                        std::unique_ptr<ExecutionPlan> plan,
                                        OptimizerRule const& rule) {
  containers::SmallVector<ExecutionNode*, 8> nodes;
  plan->findNodesOfType(
