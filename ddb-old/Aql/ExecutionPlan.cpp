...
struct NodeCounter final
  ...
  bool done(ExecutionNode* en) override final {
    ...
    if (!darbotdb::ServerState::instance()->isDBServer() ||
...
std::unique_ptr<graph::BaseOptions> createTraversalOptions(
    ...
    for (size_t i = 0; i < n; ++i) {
      ...
      if (member != nullptr && member->type == NODE_TYPE_OBJECT_ELEMENT) {
        ...
        if (name == "bfs") {
          options->mode =
              value->isTrue()
                  ? darbotdb::traverser::TraverserOptions::Order::BFS
                  : darbotdb::traverser::TraverserOptions::Order::DFS;
          hasBFS = true;
        ...
        } else if (name == darbotdb::StaticStrings::UseCache) {
  ...
  if (options->uniqueVertices ==
          darbotdb::traverser::TraverserOptions::UniquenessLevel::GLOBAL &&
...
std::unique_ptr<graph::BaseOptions> createPathsQueryOptions(
    Ast* ast, AstNode const* direction, AstNode const* optionsNode,
    darbotdb::graph::PathType::Type type) {
  TRI_ASSERT(direction != nullptr);
      ...
      if (member != nullptr && member->type == NODE_TYPE_OBJECT_ELEMENT) {
        ...
        if (name == "weightAttribute" && value->isStringValue()) {
          ...
          ExecutionPlan::invalidOptionAttribute(
              ast->query(), "unknown",
              darbotdb::graph::PathType::toString(type), name);
#endif
        } else if (name == StaticStrings::IndexHintOption) {
#ifdef EXTENDED_INDEX_HINTS_FOR_GRAPH_OPERATIONS
        ...
        } else if (name == darbotdb::StaticStrings::Algorithm) {
  ...
  if (node != nullptr && node->type == AstNodeType::NODE_TYPE_OBJECT) {
    for (size_t i = 0; i < node->numMembers(); i++) {
      ...
      if (child->type == AstNodeType::NODE_TYPE_OBJECT_ELEMENT) {
        ...
        if (name == darbotdb::StaticStrings::MaxProjections) {
  ...
  for (auto collection : VPackArrayIterator(collectionsSlice)) {
    colls.add(
        ...
        AccessMode::fromString(
            darbotdb::basics::VelocyPackHelper::checkAndGetStringValue(
                collection, "type")),
...
ExecutionNode* ExecutionPlan::fromNodeShortestPath(ExecutionNode* previous,
  ...
  auto options = createPathsQueryOptions(
      ...
      darbotdb::graph::PathType::Type::ShortestPath);
...
ExecutionNode* ExecutionPlan::fromNodeEnumeratePaths(ExecutionNode* previous,
  ...
  TRI_ASSERT(type == darbotdb::graph::PathType::Type::KShortestPaths ||
             type == darbotdb::graph::PathType::Type::KPaths ||
             type == darbotdb::graph::PathType::Type::AllShortestPaths);
...
void ExecutionPlan::unlinkNodes(
    ::darbotdb::containers::HashSet<ExecutionNode*> const& toRemove) {
  for (auto& node : toRemove) {
    unlinkNode(node);
  }
}
...
ExecutionNode* ExecutionPlan::fromSlice(velocypack::Slice slice,
  ...
  for (VPackSlice it : VPackArrayIterator(nodes)) {
    ...
    if (ret->getType() == darbotdb::aql::ExecutionNode::SUBQUERY) {
...
void ExecutionPlan::prepareTraversalOptions() {
  ...
  findNodesOfType(nodes,
                  {darbotdb::aql::ExecutionNode::TRAVERSAL,
                   darbotdb::aql::ExecutionNode::SHORTEST_PATH,
                   darbotdb::aql::ExecutionNode::ENUMERATE_PATHS},
                  true);
  for (auto& node : nodes) {
    switch (node->getType()) {
