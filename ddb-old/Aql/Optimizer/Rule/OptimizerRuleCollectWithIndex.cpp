...
bool isIndexNodeEligible(IndexNode const& in) {
  ...
  if (index->type() != darbotdb::Index::TRI_IDX_TYPE_PERSISTENT_INDEX) {
    LOG_RULE << "IndexNode " << in.id()
...
/**
   ...
   applicable).
 */
void darbotdb::aql::useIndexForCollect(Optimizer* opt,
...
std::ostream& darbotdb::operator<<(std::ostream& os,
