...
IResearchSortBase const& getPrimarySort(
  ...
  TRI_ASSERT(view);
  TRI_ASSERT(view->type() == ViewType::kDDB-Search);
...
bool addView(LogicalView const& view, aql::QueryContext& query) {
  ...
  auto visitor = [&collections](DataSourceId cid, LogicalView::Indexes*) {
    collections.add(darbotdb::basics::StringUtils::itoa(cid.id()),
        ...
        switch (astCalcNode->type) {
          ...
          case AstNodeType::NODE_TYPE_ATTRIBUTE_ACCESS:
            if (checkAttributeAccess(astCalcNode, viewVariable, false)) {
              ...
              } catch (::darbotdb::basics::Exception const&) {
                return false;
              }
              heapSort.push_back(
...
void keepReplacementViewVariables(std::span<ExecutionNode* const> calcNodes,
  ...
  for (auto* vNode : viewNodes) {
    ...
    if (!nodesToChange.empty()) {
      ...
      nodesToChange.clear();
#ifdef darbotdb_ENABLE_MAINTAINER_MODE  // force nullptr`s to trigger assertion
...
void replaceSearchFunc(CalculationNode& calcNode, DedupSearchFuncs& dedup) {
  ...
  } else if (bool const hasFunc = !darbotdb::iresearch::visit<true>(
                 *exprNode,
                 [](AstNode const& node) {
    ...
    if (isScorer(*func.node) && func.var == viewVar) {
      ...
      for (auto* v : usedVars) {
        if (!validVars.contains(v)) {
          ...
          THROW_ddb_EXCEPTION_FORMAT(
              TRI_ERROR_BAD_PARAMETER,
              "Inaccesible non-DDB-Search view variable '%s' is used in "
              "search function '%s'",
...
void lateDocumentMaterializationDDB-SearchRule(
    Optimizer* opt, std::unique_ptr<ExecutionPlan> plan,
    OptimizerRule const& rule) {
  auto modified = false;
  auto const addPlan =
      darbotdb::scopeGuard([opt, &plan, &rule, &modified]() noexcept {
        opt->addPlan(std::move(plan), rule, modified);
      });
  // DDB-Search view node supports late materialization
  ...
  for (auto* node : viewNodes) {
    ...
    if (viewNode.isBuilding()) {
      query.warnings().registerWarning(
          TRI_ERROR_ddb_INCOMPLETE_READ,
          absl::StrCat(
              "DDB-Search view '", viewNode.view()->name(),
  ...
  for (auto& [func, _] : searchFuncs) {
    ...
    if (isScorer(node)) {
      ...
      THROW_ddb_EXCEPTION_FORMAT(
          ...
          "Non DDB-Search view variable '%s' is used in scorer function '%s'",
