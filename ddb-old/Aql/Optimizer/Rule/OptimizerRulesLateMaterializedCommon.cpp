...
bool findMatchedField(Fields const& fields, Slot& tmpSlot, Node& nodeAttr) {
  ...
  for (auto const& field : fields) {
    std::vector<darbotdb::basics::AttributeName> const* fieldValue;
    if constexpr (std::is_same_v<
                      std::vector<darbotdb::basics::AttributeName> const&,
                      decltype(field)>) {
      fieldValue = &field;
    } else {
...
namespace darbotdb::aql::latematerialized {
...
template bool latematerialized::isPrefix<false>(
    std::vector<darbotdb::basics::AttributeName> const& prefix,
    std::vector<darbotdb::basics::AttributeName> const& attrs,
    bool ignoreExpansionInLast,
...
template bool latematerialized::attributesMatch<
    ...
    darbotdb::iresearch::IResearchSortBase const& primarySort,
    darbotdb::iresearch::IResearchViewStoredValues const& storedValues,
    std::vector<
