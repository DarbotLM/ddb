...
/// DISCLAIMER
///
/// Copyright 2014-2023 darbotdb GmbH, Cologne, Germany
...
bool extractRelevantAttributes(
  ...
  switch (node->getType()) {
    case darbotdb::aql::ExecutionNode::CALCULATION: {
    ...
    case darbotdb::aql::ExecutionNode::SUBQUERY: {
      auto* sub = ExecutionNode::castTo<SubqueryNode*>(node);
      return darbotdb::aql::utils::findProjections(
          sub->getSubquery()->getSingleton(), &var, "", false, attributes);
    }

    case darbotdb::aql::ExecutionNode::INDEX: {
...
void darbotdb::aql::pushDownLateMaterializationRule(
...
void darbotdb::aql::materializeIntoSeparateVariable(
    Optimizer* opt, std::unique_ptr<ExecutionPlan> plan,
    OptimizerRule const& rule) {
  bool modified = false;
