...
bool isGeoIndex(darbotdb::Index::IndexType type) {
  return type == darbotdb::Index::TRI_IDX_TYPE_GEO1_INDEX ||
         type == darbotdb::Index::TRI_IDX_TYPE_GEO2_INDEX ||
         type == darbotdb::Index::TRI_IDX_TYPE_GEO_INDEX;
}
...
std::pair<AstNode*, AstNode*> getAttributeAccessFromIndex(
  ...
  if (!indexFound) {
    THROW_ddb_EXCEPTION_PARAMS(TRI_ERROR_QUERY_GEO_INDEX_MISSING,
...
AstNode* replaceFullText(AstNode* funAstNode, ExecutionNode* calcNode,
  ...
  for (auto& idx : coll->indexes()) {
    if (idx->type() ==
        darbotdb::Index::IndexType::TRI_IDX_TYPE_FULLTEXT_INDEX) {
  ...
  if (!index) {  // not found or error
    THROW_ddb_EXCEPTION_PARAMS(TRI_ERROR_QUERY_FULLTEXT_INDEX_MISSING,
                                  params.collection.c_str());
  }
...
void darbotdb::aql::replaceNearWithinFulltextRule(
...
void darbotdb::aql::replaceLikeWithRangeRule(
    Optimizer* opt, std::unique_ptr<ExecutionPlan> plan,
    OptimizerRule const& rule) {
  bool modified = false;
