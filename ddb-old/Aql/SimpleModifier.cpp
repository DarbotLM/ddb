...
using namespace darbotdb::aql::ModificationExecutorHelpers;
    ...
    sqs->executeAndWakeup([&]() noexcept {
      ...
      try {
        ...
        if (std::holds_alternative<Waiting>(self->_results)) {
          ...
          THROW_ddb_EXCEPTION_MESSAGE(TRI_ERROR_INTERNAL_AQL,
                                         std::move(message));
        }
      } catch (...) {
...
template class ::darbotdb::aql::SimpleModifier<InsertModifierCompletion>;
template class ::darbotdb::aql::SimpleModifier<RemoveModifierCompletion>;
template class ::darbotdb::aql::SimpleModifier<UpdateReplaceModifierCompletion>;
