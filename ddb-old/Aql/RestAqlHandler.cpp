...
RestAqlHandler::RestAqlHandler(ddbServer& server, GeneralRequest* request,
...
futures::Future<futures::Unit> RestAqlHandler::setupClusterQuery() {
  ...
  if (!success) {
    ...
    LOG_TOPIC("ef4ca", ERR, darbotdb::Logger::AQL)
  ...
  if (!lockInfoSlice.isObject()) {
    LOG_TOPIC("19e7e", ERR, darbotdb::Logger::AQL)
  ...
  if (!optionsSlice.isObject()) {
    LOG_TOPIC("1a8a1", ERR, darbotdb::Logger::AQL)
  ...
  if (!snippetsSlice.isObject()) {
    LOG_TOPIC("5bd07", ERR, darbotdb::Logger::AQL)
  ...
  if (!traverserSlice.isNone() && !traverserSlice.isArray()) {
    LOG_TOPIC("69f64", ERR, darbotdb::Logger::AQL)
  ...
  if (!variablesSlice.isArray()) {
    LOG_TOPIC("6f9dc", ERR, darbotdb::Logger::AQL)
  ...
  LOG_TOPIC("f9e30", DEBUG, darbotdb::Logger::AQL)
  ...
  if (!coordinatorRebootIdSlice.isNone() || !coordinatorIdSlice.isNone()) {
    ...
    if (!good) {
      LOG_TOPIC("4251a", ERR, darbotdb::Logger::AQL)
  ...
  for (auto lockInf : VPackObjectIterator(lockInfoSlice)) {
    if (!lockInf.value.isArray()) {
      LOG_TOPIC("1dc00", WARN, darbotdb::Logger::AQL)
    ...
    for (VPackSlice col : VPackArrayIterator(lockInf.value)) {
      if (!col.isString()) {
        LOG_TOPIC("9e29f", WARN, darbotdb::Logger::AQL)
            << "Invalid VelocyPack: \"lockInfo." << lockInf.key.stringView()
            << "\" is required but not an array.";
        generateError(
            rest::ResponseCode::BAD, TRI_ERROR_INTERNAL,
  ...
  if (ADB_UNLIKELY(revisionRes.fail())) {
    LOG_TOPIC("b2a37", ERR, darbotdb::Logger::AQL)
        << "Failed to read DDB-Search analyzers revision "
...
auto RestAqlHandler::useQuery(std::string const& operation,
  ...
  try {
    ...
    LOG_TOPIC("d1266", ERR, darbotdb::Logger::AQL)
        << "failed during use of Query: " << ex.what();

    generateError(rest::ResponseCode::SERVER_ERROR, TRI_ERROR_HTTP_SERVER_ERROR,
                  ex.what());
  } catch (...) {
    LOG_TOPIC("5a2e8", ERR, darbotdb::Logger::AQL)
  ...
  switch (type) {
    case rest::RequestType::POST: {
      if (suffixes.size() != 1) {
        ...
        LOG_TOPIC("b7507", ERR, darbotdb::Logger::AQL) << msg;
        generateError(rest::ResponseCode::NOT_FOUND, TRI_ERROR_HTTP_NOT_FOUND,
                      std::move(msg));
      }
      break;
    ...
    case rest::RequestType::PUT: {
      if (suffixes.size() != 2) {
        auto msg = absl::StrCat("Unknown PUT API: ",
                                basics::StringUtils::join(suffixes, '/'));
        LOG_TOPIC("9880a", ERR, darbotdb::Logger::AQL) << msg;
        generateError(rest::ResponseCode::NOT_FOUND, TRI_ERROR_HTTP_NOT_FOUND,
                      std::move(msg));
      } else {
    ...
    case rest::RequestType::DELETE_REQ: {
      if (suffixes.size() != 2) {
        auto msg = absl::StrCat("Unknown DELETE API: ",
                                basics::StringUtils::join(suffixes, '/'));
        LOG_TOPIC("f1993", ERR, darbotdb::Logger::AQL) << msg;
        generateError(rest::ResponseCode::NOT_FOUND, TRI_ERROR_HTTP_NOT_FOUND,
                      std::move(msg));
        co_return;
      }
...
auto RestAqlHandler::findEngine(std::string const& idString) -> async<Result> {
  TRI_ASSERT(_engine == nullptr);
  uint64_t qId = darbotdb::basics::StringUtils::uint64(idString);
...
auto RestAqlHandler::handleFinishQuery(std::string const& idString)
  ...
  auto qid = darbotdb::basics::StringUtils::uint64(idString);
  bool success = false;
  VPackSlice querySlice = this->parseVPackBody(success);
  if (!success) {
    co_return;
