...
bool Query::tryLoadPlanFromCache() {
  if (_queryOptions.usePlanCache) {
    if (!canUsePlanCache()) {
      THROW_ddb_EXCEPTION(TRI_ERROR_QUERY_NOT_ELIGIBLE_FOR_PLAN_CACHING);
    }
...
async<void> Query::prepareQuery() {
  try {
    ...
    if (_snippets.size() > 1) {  // register coordinator snippets
      ...
      if (!registry) {
        THROW_ddb_EXCEPTION_MESSAGE(TRI_ERROR_SHUTTING_DOWN,
...
futures::Future<futures::Unit> Query::execute(
  ...
  try {
    if (killed()) {
      THROW_ddb_EXCEPTION(TRI_ERROR_QUERY_KILLED);
    }
...
QueryResult Query::explain() {
  ...
  try {
    ...
    if (commitResult.fail()) {
      THROW_ddb_EXCEPTION(commitResult);
    }
    ...
    auto f =
        std::move(future)
            .thenValue([ss, &query](network::Response&& res) mutable -> Result {
              ...
              if (res.fail()) {
                return Result{network::fuerteToddbErrorCode(res)};
...
ExecutionState Query::cleanupTrxAndEngines() {
  ...
  if (result().ok()) {
    ...
    if (commitResult.waitAndGet().fail()) {
      THROW_ddb_EXCEPTION(std::move(commitResult).waitAndGet());
    }
  ...
  try {
    TRI_IF_FAILURE("Query::finalize_error_on_finish_db_servers") {
      THROW_ddb_EXCEPTION(TRI_ERROR_INTERNAL_AQL);
    }
...
#ifndef darbotdb_ENABLE_FAILURE_TESTS
  TRI_ASSERT(false);
#else
  if (_wasDebugKilled) {
    return;
