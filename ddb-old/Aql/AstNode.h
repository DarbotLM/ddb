...
struct AstNode {
  ...
  explicit AstNode(Ast*, darbotdb::velocypack::Slice slice);
...
/// @brief dump the node (for debugging purposes)
  ...
  darbotdb::velocypack::Slice computeValue(
      darbotdb::velocypack::Builder* = nullptr) const;
  ...
  static AstNodeType getNodeTypeFromVPack(darbotdb::velocypack::Slice slice);
  ...
  void toVelocyPackValue(darbotdb::velocypack::Builder& builder) const;

  /// @brief Create a VelocyPack representation of the node
  void toVelocyPack(darbotdb::velocypack::Builder& builder, bool verbose) const;
 ...
 private:
  ...
  void computeValue(darbotdb::velocypack::Builder& builder) const;
...
std::ostream& operator<<(std::ostream&, darbotdb::aql::AstNode const*);
std::ostream& operator<<(std::ostream&, darbotdb::aql::AstNode const&);
...
#define FINALIZE_SUBTREE(n) \
  darbotdb::aql::AstNode::markFinalized(const_cast<darbotdb::aql::AstNode*>(n))
...
#define TEMPORARILY_UNLOCK_NODE(n)                                      \
  bool wasFinalizedAlready =                                            \
      (n)->hasFlag(darbotdb::aql::AstNodeFlagType::FLAG_FINALIZED);     \
  if (wasFinalizedAlready) {                                            \
    (n)->flags =                                                        \
        ((n)->flags & ~darbotdb::aql::AstNodeFlagType::FLAG_FINALIZED); \
  }                                                                     \
  auto sg = darbotdb::scopeGuard([&]() noexcept {                       \
