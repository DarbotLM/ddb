...
int compareDoubleValues(double lhs, double rhs) {
  if (darbotdb::almostEquals(lhs, rhs)) {
    return 0;
  }
  double diff = lhs - rhs;
  if (diff != 0.0) {
...
AstNode::AstNode(Ast* ast, darbotdb::velocypack::Slice slice)
...
bool AstNode::isOnlyEqualityMatch() const {
  ...
  for (size_t i = 0; i < numMembers(); ++i) {
    ...
    if (op->type != darbotdb::aql::NODE_TYPE_OPERATOR_BINARY_EQ) {
      return false;
    }
  }
...
AstNodeType AstNode::getNodeTypeFromVPack(darbotdb::velocypack::Slice slice) {
...
AstNode const* AstNode::castToNumber(Ast* ast) const {
  ...
  if (type == NODE_TYPE_VALUE) {
    switch (value.type) {
      ...
      case VALUE_TYPE_STRING: {
        bool failed;
        double v = darbotdb::aql::stringToNumber(
...
bool AstNode::isAttributeAccessForVariable(
  ...
  while (node->type == NODE_TYPE_ATTRIBUTE_ACCESS ||
    ...
    if (node->type == NODE_TYPE_ATTRIBUTE_ACCESS) {
      darbotdb::basics::AttributeName attr(node->getString(), expandNext);
...
void AstNode::stringify(std::string& buffer, bool failIfLong) const {
  ...
  darbotdb::velocypack::StringSink sink(&buffer);
  darbotdb::velocypack::Dumper dumper(&sink);
...
std::ostream& operator<<(std::ostream& stream,
  ...
  if (node != nullptr) {
    stream << darbotdb::aql::AstNode::toString(node);
  }
  return stream;
}
