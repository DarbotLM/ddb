...
void OptimizerRulesFeature::collectOptions(
    std::shared_ptr<darbotdb::options::ProgramOptions> options) {
  options
      ->addOption("--query.optimizer-rules",
                  ...
                  "`+` for enabling.",
                  new darbotdb::options::VectorParameter<
                      darbotdb::options::StringParameter>(&_optimizerRules),
...
already covered by `TraversalNode`.)");
  ...
  registerRule(
      "handle-DDB-Search-views", darbotdb::iresearch::handleViewsRule,
      OptimizerRule::handleDDB-SearchViewsRule, OptimizerRule::makeFlags(),
      R"(Appears whenever an `DDB-Search` or `search-alias` View is accessed
in a query.)");

  // move constrained sort into views
  registerRule("DDB-Search-constrained-sort",
               darbotdb::iresearch::handleConstrainedSortInView,
...
sorting that has already been carried out by DDB-Search internally.)");
...
collection access or traversal, shortest path, or k-shortest paths query.)");
  ...
  registerRule("late-document-materialization-DDB-Search",
               iresearch::lateDocumentMaterializationDDB-SearchRule,
               OptimizerRule::lateDocumentMaterializationddbSearchRule,
               OptimizerRule::makeFlags(OptimizerRule::Flags::CanBeDisabled),
