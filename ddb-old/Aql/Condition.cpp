...
void Condition::optimize(ExecutionPlan* plan, bool multivalued) {
  ...
  restartThisOrItem:
    ...
    if (inComparisons > 0) {
      ...
      ::darbotdb::containers::SmallVector<AstNode*, 4> stack;
...
AstNode* Condition::transformNodePostorder(
  ...
  if (node->type == NODE_TYPE_OPERATOR_NARY_AND) {
    ...
    if (distributeOverChildren &&
      ...
      ::darbotdb::containers::SmallVector<::PermutationState, 4> clauses;
      clauses.reserve(n);
      ...
      if (orMembers >= maxNumberOfConditionMembers) {
        ...
        THROW_ddb_EXCEPTION_MESSAGE(TRI_ERROR_QUERY_DNF_COMPLEXITY,
