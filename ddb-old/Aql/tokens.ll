...
#define YY_EXTRA_TYPE darbotdb::aql::Parser*
...
(0|[1-9][0-9]*) {
  ...
  darbotdb::aql::AstNode* node = nullptr;
  auto parser = yyextra;

  bool valid;
  int64_t value1 = darbotdb::NumberUtils::atoi<int64_t>(yytext, yytext + yyleng, valid);
...
(0[bB][01]+) {
  ...
  auto parser = yyextra;
  if (static_cast<uint64_t>(e - p) > darbotdb::aql::functions::bitFunctionsMaxSupportedBits) {
  ...
  darbotdb::aql::AstNode* node = parser->ast()->createNodeValueInt(static_cast<int64_t>(result));
  yylval->node = node;
...
(0[xX][0-9a-fA-F]+) {
  ...
  if (static_cast<uint64_t>(e - p) > darbotdb::aql::functions::bitFunctionsMaxSupportedBits / 4) {
    /* we only support up to 32 bits for now */
