...
static darbotdb::aql::QueryCache instance;

/// @brief whether or not the cache is enabled
static std::atomic<darbotdb::aql::QueryCacheMode> mode(CACHE_ON_DEMAND);
...
void QueryCacheResultEntry::toVelocyPack(VPackBuilder& builder) const {
  ...
  for (auto const& it : _dataSources) {
    builder.add(darbotdb::velocypack::Value(it.second));
  }
...
void QueryCache::store(TRI_vocbase_t* vocbase,
  ...
  auto it =
      _entries[part]
          .try_emplace(vocbase, darbotdb::lazyConstruct([&] {
