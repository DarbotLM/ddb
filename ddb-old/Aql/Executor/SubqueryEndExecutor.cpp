...
SubqueryEndExecutorInfos::SubqueryEndExecutorInfos(
    ...
    darbotdb::ResourceMonitor& resourceMonitor, RegisterId inReg,
    RegisterId outReg)
...
darbotdb::ResourceMonitor& SubqueryEndExecutorInfos::getResourceMonitor()
    const noexcept {
...
SubqueryEndExecutor::Accumulator::Accumulator(
    darbotdb::ResourceMonitor& resourceMonitor, VPackOptions const* options)
...
AqlValueGuard SubqueryEndExecutor::Accumulator::stealValue(AqlValue& result) {
  ...
  auto sg = darbotdb::scopeGuard([&]() noexcept { reset(); });
