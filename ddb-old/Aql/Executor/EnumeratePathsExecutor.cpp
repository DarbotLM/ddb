  ...
  while (input.hasDataRow()) {
    ...
    if (getVertexId(_infos.getSourceVertex(), _inputRow, _sourceBuilder,
        ...
        getVertexId(_infos.getTargetVertex(), _inputRow, _targetBuilder,
                    target)) {
      _finder.reset(darbotdb::velocypack::HashedStringRef(source),
                    darbotdb::velocypack::HashedStringRef(target));
      return true;
    }
  }
  return false;
...
template class ::darbotdb::aql::EnumeratePathsExecutorInfos<
