...
DistinctCollectExecutorInfos::DistinctCollectExecutorInfos(
    ...
    velocypack::Options const* opts, darbotdb::ResourceMonitor& resourceMonitor)
...
darbotdb::ResourceMonitor& DistinctCollectExecutorInfos::getResourceMonitor()
    const {
...
auto DistinctCollectExecutor::produceRows(AqlItemBlockInputRange& inputRange,
  ...
  while (inputRange.hasDataRow()) {
    ...
    if (!_seen.contains(groupValue)) {
      ...
      darbotdb::ResourceUsageScope guard(_infos.getResourceMonitor(),
                                         memoryUsage);
