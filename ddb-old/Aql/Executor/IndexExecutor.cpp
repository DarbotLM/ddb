...
darbotdb::aql::Projections const& IndexExecutorInfos::getProjections()
    const noexcept {
  return _projections;
}

darbotdb::aql::Projections const& IndexExecutorInfos::getFilterProjections()
    const noexcept {
...
void IndexExecutor::initIndexes(InputAqlItemRow const& input) {
  ...
  if (!_infos.getNonConstExpressions().empty()) {
    ...
    if (_infos.getV8Expression()) {
      ...
      auto cleanup = [this]() {
        if (darbotdb::ServerState::instance()->isRunningInCluster()) {
          _infos.query().exitV8Executor();
        }
      };

      _infos.query().enterV8Executor();
      auto sg = darbotdb::scopeGuard([&]() noexcept { cleanup(); });
