...
class QueryInfoLoggerThread final : public ServerThread<ddbServer> {
 public:
  explicit QueryInfoLoggerThread(ddbServer& server,
                                 size_t maxBufferedQueries,
                                 uint64_t pushInterval,
                                 uint64_t cleanupInterval, double retentionTime)
      : ServerThread<ddbServer>(server, "QueryInfoLogger"),
 ...
 private:
        ...
        "queries-gc", RequestLane::CLIENT_SLOW, offset, [this](bool canceled) {
          if (!canceled && !isStopping()) {
            if (Result res = basics::catchToResult(
                ...
                res.fail() && res.isNot(TRI_ERROR_ddb_DATABASE_NOT_FOUND) &&
                res.isNot(TRI_ERROR_ddb_DATA_SOURCE_NOT_FOUND) &&
                res.isNot(TRI_ERROR_ddb_CONFLICT) &&
                res.isNot(TRI_ERROR_ddb_READ_ONLY)) {
  ...
  Result cleanupCollection() {
    ...
    LOG_TOPIC_IF("05da1", TRACE, Logger::QUERIES,
                 queryResult.fail() &&
                     queryResult.isNot(TRI_ERROR_ddb_DATA_SOURCE_NOT_FOUND))
  ...
  Result ensureCollection() {
    ...
    if (Result res = methods::Collections::lookup(
      ...
      if (res.fail() && res.isNot(TRI_ERROR_ddb_DUPLICATE_NAME)) {
