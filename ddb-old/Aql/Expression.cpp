...
using VelocyPackHelper = darbotdb::basics::VelocyPackHelper;
...
Expression::Expression(Ast* ast, darbotdb::velocypack::Slice slice)
...
bool Expression::findInArray(AqlValue const& left, AqlValue const& right,
  ...
  if (!right.isRange() && !left.isRange()) {
    ...
    while (it.valid()) {
      int compareResult = darbotdb::basics::VelocyPackHelper::compare(
...
void Expression::prepareForExecution() {
  ...
  if (_type == ExpressionType::kJson && _data == nullptr) {
    ...
    {
      darbotdb::ResourceUsageScope guard(_resourceMonitor, bufferSize);
...
bool Expression::isReference() const {
  return (_node->type == darbotdb::aql::NODE_TYPE_REFERENCE);
}
...
AqlValue Expression::executeSimpleExpressionObject(ExpressionContext& ctx,
  ...
  darbotdb::velocypack::StringSink adapter(buffer.get());
...
AqlValue Expression::executeSimpleExpressionFCallJS(ExpressionContext& ctx,
...

  {
    ...
    TRI_v8_global_t* v8g = static_cast<TRI_v8_global_t*>(
        isolate->GetData(darbotdb::V8PlatformFeature::V8_DATA_SLOT));
    ...
    auto sg =
        darbotdb::scopeGuard([&]() noexcept { v8g->_expressionContext = old; });
  ...
  THROW_ARANGO_EXCEPTION_MESSAGE(
      TRI_ERROR_NOT_IMPLEMENTED,
      "this version of darbotdb is built without JavaScript support");
#endif
}
...
void Expression::toVelocyPack(darbotdb::velocypack::Builder& builder,
                              bool verbose) const {
