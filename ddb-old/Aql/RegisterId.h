...
namespace std {
template<>
struct hash<darbotdb::aql::RegisterId> {
  size_t operator()(darbotdb::aql::RegisterId const& x) const noexcept {
    return std::hash<unsigned>()(x.value());
  }
};

template<>
struct equal_to<darbotdb::aql::RegisterId> {
  bool operator()(darbotdb::aql::RegisterId const& lhs,
                  darbotdb::aql::RegisterId const& rhs) const noexcept {
    return lhs == rhs;
  }
};
}  // namespace std
