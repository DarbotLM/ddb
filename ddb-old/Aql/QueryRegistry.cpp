...
void QueryRegistry::insertQuery(std::shared_ptr<ClusterQuery> query, double ttl,
  ...
  LOG_TOPIC("77778", DEBUG, darbotdb::Logger::AQL)
  ...
  if (vocbase.isDropped()) {
    ...
    THROW_ddb_EXCEPTION(TRI_ERROR_ddb_DATABASE_NOT_FOUND);
  }
  ...
  try {
    ...
    if (!result.second) {
      if (result.first->second->_isTombstone) {
        ...
        // the query!
        LOG_TOPIC("580c1", DEBUG, darbotdb::Logger::AQL)
            << "unable to insert query " << qId
            << " into query registry because of existing tombstone";

        THROW_ddb_EXCEPTION_MESSAGE(result.first->second->_errorCode,
...
ResultT<void*> QueryRegistry::openEngine(EngineId id, EngineType type,
  ...
  LOG_TOPIC("8c204", DEBUG, darbotdb::Logger::AQL)
  ...
  if (it == _engines.end()) {
    LOG_TOPIC("c3ae4", DEBUG, darbotdb::Logger::AQL)
  ...
  if (ei._type != type) {
    LOG_TOPIC("c3af5", DEBUG, darbotdb::Logger::AQL)
        << "Engine with id " << id << " has other type";
    return {TRI_ERROR_QUERY_NOT_FOUND};
  }

  if (ei._isOpen) {
    LOG_TOPIC("7c2a3", DEBUG, darbotdb::Logger::AQL)
  ...
  if (ei._queryInfo) {
    ...
    LOG_TOPIC("b1cfd", TRACE, darbotdb::Logger::AQL)
        << "opening engine " << id
        << ", query id: " << ei._queryInfo->_query->id()
        << ", numOpen: " << ei._queryInfo->_numOpen;
  } else {
    LOG_TOPIC("50eff", TRACE, darbotdb::Logger::AQL)
...
void QueryRegistry::closeEngine(EngineId engineId) {
  LOG_TOPIC("3f0c9", DEBUG, darbotdb::Logger::AQL)
...

  {
    ...
    if (it == _engines.end()) {
      LOG_TOPIC("97351", DEBUG, darbotdb::Logger::AQL)
    ...
    EngineInfo& ei = it->second;
    if (!ei._isOpen) {
      LOG_TOPIC("45b8e", DEBUG, darbotdb::Logger::AQL)
    ...
    if (ei._queryInfo) {
      ...
      LOG_TOPIC("5ecdc", TRACE, darbotdb::Logger::AQL)
      ...
      LOG_TOPIC("ae981", TRACE, darbotdb::Logger::AQL)
...
auto QueryRegistry::lookupQueryForFinalization(QueryId id, ErrorCode errorCode)
  ...
  if (queryMapIt == _queries.end()) {
    if (errorCode != TRI_ERROR_NO_ERROR &&
      ...
      LOG_TOPIC("779f6", DEBUG, darbotdb::Logger::AQL)
...
std::unique_ptr<QueryRegistry::QueryInfo> QueryRegistry::deleteQuery(
...
#ifndef darbotdb_ENABLE_MAINTAINER_MODE
    _engines.erase(engine->engineId());
#else
  ...
  LOG_TOPIC("6756c", DEBUG, darbotdb::Logger::AQL)
...
bool QueryRegistry::destroyEngine(EngineId engineId, ErrorCode errorCode) {
...

  {
    ...
    auto it = _engines.find(engineId);
    if (it == _engines.end()) {
      LOG_TOPIC("2b5e6", DEBUG, darbotdb::Logger::AQL)
          << "Found no engine with id " << engineId;
      return false;
    }
    ...
    if (ei._isOpen) {
      ...
      LOG_TOPIC("b342e", DEBUG, darbotdb::Logger::AQL)
...
void QueryRegistry::expireQueries() {
  ...
  if (!queriesLeft.empty()) {
    LOG_TOPIC("4f142", DEBUG, darbotdb::Logger::AQL)
        << "queries left in QueryRegistry: " << queriesLeft;
  }
#endif
  for (auto& qId : toDelete) {
    try {  // just in case
      LOG_TOPIC("e95dc", DEBUG, darbotdb::Logger::AQL)
...
void QueryRegistry::destroyAll() try {
  ...
  for (auto& qid : allQueries) {
    try {
      LOG_TOPIC("df275", DEBUG, darbotdb::Logger::AQL)
  ...
  if (count > 0) {
    LOG_TOPIC("43bf8", INFO, darbotdb::Logger::AQL)
        << "number of remaining queries in query registry at shutdown: "
        << count;
  }
} catch (std::exception const& ex) {
  LOG_TOPIC("30bdb", INFO, darbotdb::Logger::AQL)
...
void QueryRegistry::unregisterSnippets(SnippetList const& snippets) noexcept {
  ...
  while (true) {
    ...
    if (iterations == 0) {
      LOG_TOPIC("33cfb", DEBUG, darbotdb::Logger::AQL)
          << remain << " engine snippet(s) still in use on query shutdown";
    } else if (iterations == 100) {
      LOG_TOPIC("df7c7", WARN, darbotdb::Logger::AQL)
          << remain << " engine snippet(s) still in use on query shutdown";
    }
    ++iterations;
