...
std::pair<bool, bool> findIndexHandleForAndNode(
  ...
  auto considerIndex =
    ...
    if (readOwnWrites == ReadOwnWrites::yes &&
        idx->type() == darbotdb::Index::TRI_IDX_TYPE_INVERTED_INDEX) {
...
void extractNonConstPartsOfLeafNode(
  ...
  if (lhs->isAttributeAccessForVariable(indexVariable, false)) {
    ...
    auto isInvertedIndexFunc = [&] {
      if (index == nullptr ||
          index->type() != darbotdb::Index::TRI_IDX_TYPE_INVERTED_INDEX) {
        return false;
      }
...
darbotdb::aql::Collection const* getCollection(
  ...
  switch (node->getType()) {
    case EN::ENUMERATE_COLLECTION:
      return ExecutionNode::castTo<
                 darbotdb::aql::EnumerateCollectionNode const*>(node)
          ->collection();
    case EN::INDEX:
      return ExecutionNode::castTo<darbotdb::aql::IndexNode const*>(node)
          ->collection();
    case EN::TRAVERSAL:
    case EN::ENUMERATE_PATHS:
    case EN::SHORTEST_PATH:
      return ExecutionNode::castTo<darbotdb::aql::GraphNode const*>(node)
          ->collection();
