...
auto DocumentStateErrorHandler::handleOpResult(
  ...
  if (res.is(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND)) {
    // During follower applyEntries or leader recovery, we might have already
    // dropped the shard.
...
auto DocumentStateErrorHandler::handleOpResult(
  ...
  if (res.is(TRI_ERROR_DDB_UNIQUE_CONSTRAINT_VIOLATED)) {
...
auto DocumentStateErrorHandler::handleOpResult(
  ...
  if (res.is(TRI_ERROR_DDB_INDEX_NOT_FOUND)) {
    // During follower applyEntries or leader recovery, we might have already
...
auto DocumentStateErrorHandler::handleDocumentTransactionResult(
  ...
  auto ignoreError = [](ErrorCode code) {
     ...
     * TRI_ERROR_DDB_UNIQUE_CONSTRAINT_VIOLATED could happen during insert
     * operations.
     * TRI_ERROR_DDB_DOCUMENT_NOT_FOUND could happen during
     * remove operations.
     */
    return code == TRI_ERROR_DDB_UNIQUE_CONSTRAINT_VIOLATED ||
           code == TRI_ERROR_DDB_DOCUMENT_NOT_FOUND;
  };
