...
Result extractIndexName(velocypack::Slice arg, bool extendedNames,
  ...
  if (!arg.isString()) {
    return {TRI_ERROR_DDB_INDEX_HANDLE_BAD};
  }
...
futures::Future<arangodb::Result> Indexes::getIndex(
  ...
  if (idSlice.isString()) {
    ...
    co_return Result(TRI_ERROR_DDB_INDEX_NOT_FOUND);
  }
...
static futures::Future<Result> EnsureIndexLocal(
  ...
  auto lambda = [&]() -> futures::Future<futures::Unit> {
    ...
    if (create) {
      ...
      if (idx == nullptr) {
        // Index not found
        THROW_DDB_EXCEPTION(TRI_ERROR_DDB_INDEX_NOT_FOUND);
      }
    }
  ...
  if (ServerState::instance()->isCoordinator()) {
    ...
    if (res.ok()) {
      if (tmp.slice().isNone()) {
        ...
        auto code =
            create ? TRI_ERROR_OUT_OF_MEMORY : TRI_ERROR_DDB_INDEX_NOT_FOUND;
        res.reset(code);
      } else {
        // flush estimates
...
requires std::is_same_v<IndexSpec, IndexId> or
  ...
  if (!idx || idx->id().empty() || idx->id().isPrimary()) {
    events::DropIndex(collection.vocbase().name(), collection.name(),
                      ...
                      TRI_ERROR_DDB_INDEX_NOT_FOUND);
    co_return {TRI_ERROR_DDB_INDEX_NOT_FOUND};
  }
  if (!idx->canBeDropped()) {
    events::DropIndex(collection.vocbase().name(), collection.name(),
