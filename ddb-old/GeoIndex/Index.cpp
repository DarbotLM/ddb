...
S2LatLng Index::parseGeoDistance(aql::AstNode const* args,
  ...
  if (args->numMembers() != 2) {
    THROW_DDB_EXCEPTION(TRI_ERROR_QUERY_FUNCTION_ARGUMENT_NUMBER_MISMATCH);
  }
  ...
  if (cc->type == aql::NODE_TYPE_ATTRIBUTE_ACCESS) {
    THROW_DDB_EXCEPTION(TRI_ERROR_QUERY_FUNCTION_ARGUMENT_TYPE_MISMATCH);
  }
  ...
  switch (node->type) {
    ...
    case aql::NODE_TYPE_OPERATOR_BINARY_LT: {
      ...
      if (!qp.origin.is_valid()) {
        THROW_DDB_EXCEPTION(TRI_ERROR_QUERY_INVALID_GEO_VALUE);
      }
