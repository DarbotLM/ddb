...
using namespace ddb;
using namespace ddb::basics;
using namespace ddb::rest;
 ...
 public:
  ...
  TRI_action_result_t execute(TRI_vocbase_t* vocbase, GeneralRequest* request,
...

    {
      ...
      if (it == _callbacks.end()) {
        LOG_TOPIC("94556", WARN, ddb::Logger::FIXME)
...
v8::Handle<v8::Object> TRI_RequestCppToV8(v8::Isolate* isolate,
  ...
  //
  //        "cookies" : {
  //          "ddb_SESSION_ID" : "0cwuzusd23nw3qiwui84uwqwqw23e"
  //        },
  //
  //        "requestType" : "GET",
  ...
  if (!acceptPlain.empty()) {
    ...
    switch (request->contentTypeResponse()) {
      ...
      case ContentType::HTML:  // text/html
        ...
        break;
      case ContentType::DUMP:  // application/x-ddb-dump
  ...
  if (httpRequest == nullptr) {
    ...
    THROW_ddb_EXCEPTION_MESSAGE(TRI_ERROR_INTERNAL, "invalid request type");
  } else {
...
static TRI_action_result_t ExecuteActionVocbase(
    ...
    GeneralResponse* response) {
  if (response == nullptr) {
    THROW_ddb_EXCEPTION_MESSAGE(TRI_ERROR_INTERNAL, "invalid response");
  }
  ...
  } catch (ddb::basics::Exception const& ex) {
    errorCode = ex.code();
  ...
  if (errorCode != TRI_ERROR_NO_ERROR) {
    ...
    if (tryCatch.CanContinue()) {
      ...
      LOG_TOPIC("b8286", WARN, ddb::Logger::V8)
          << "Caught an error while executing an action: " << jsError;
#ifdef ddb_ENABLE_MAINTAINER_MODE
      VPackBuilder b;
...
static void JS_DefineAction(v8::FunctionCallbackInfo<v8::Value> const& args) {
  ...
  v8::HandleScope scope(isolate);
  TRI_GET_SERVER_GLOBALS(ddbdServer);
  ...
  if (v8ActionForName != nullptr) {
    ...
    LOG_TOPIC("43be9", WARN, ddb::Logger::FIXME)
...
static void JS_RawRequestBody(v8::FunctionCallbackInfo<v8::Value> const& args) {
  ...
  if (current->IsObject()) {
    ...
    if (property->IsExternal()) {
      ...
      auto httpRequest = static_cast<ddb::HttpRequest*>(e->Value());
      if (httpRequest != nullptr) {
        V8Buffer* buffer;
...
static void JS_RequestParts(v8::FunctionCallbackInfo<v8::Value> const& args) {
  ...
  if (current->IsObject()) {
    ...
    if (property->IsExternal()) {
      v8::Handle<v8::External> e = v8::Handle<v8::External>::Cast(property);
      auto request = static_cast<ddb::HttpRequest*>(e->Value());
...
#ifdef ddb_ENABLE_FAILURE_TESTS
static ErrorCode clusterSendToAllServers(
    ...
    ddb::rest::RequestType const& method, std::string const& body) {
  TRI_GET_SERVER_GLOBALS(ddbdServer);
  ...
  fuerte::RestVerb verb = network::ddbRestVerbToFuerte(method);
  ...
  for (auto& f : futures) {
    ...
    auto commError = network::fuerteToddbErrorCode(res);
...
static void JS_DebugRemoveFailAt(
  ...
  if (ServerState::instance()->isCoordinator()) {
    auto res = clusterSendToAllServers(
        isolate, dbname, "_admin/debug/failat/" + StringUtils::urlEncode(point),
        ddb::rest::RequestType::DELETE_REQ, "");
    if (res != TRI_ERROR_NO_ERROR) {
      TRI_V8_THROW_EXCEPTION(res);
    }
  }
