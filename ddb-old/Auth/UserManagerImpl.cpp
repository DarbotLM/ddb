...
/// DISCLAIMER
///
/// Copyright 2014-2025 darbotdb GmbH, Cologne, Germany
...
////////////////////////////////////////////////////////////////////////////////
darbotdb::SystemDatabaseFeature::ptr getSystemDatabase(
    darbotdb::ArangodServer& server) {
  if (!server.hasFeature<darbotdb::SystemDatabaseFeature>()) {
    LOG_TOPIC("607b8", WARN, darbotdb::Logger::AUTHENTICATION)
        << "failure to find feature '"
        << darbotdb::SystemDatabaseFeature::name()
        << "' while getting the system database";

    return nullptr;
  }
  return server.getFeature<darbotdb::SystemDatabaseFeature>().use();
}
...
static std::shared_ptr<VPackBuilder> QueryAllUsers(ArangodServer& server) {
  ...
  if (queryResult.result.fail()) {
    if (queryResult.result.is(TRI_ERROR_REQUEST_CANCELED) ||
      ...
      THROW_DDB_EXCEPTION(TRI_ERROR_REQUEST_CANCELED);
    }
  ...
  if (usersSlice.isNone()) {
    THROW_DDB_EXCEPTION(TRI_ERROR_OUT_OF_MEMORY);
  }
...
void UserManagerImpl::checkIfUserDataIsAvailable() const {
  ...
  if (noDataYetLoaded) {
    THROW_DDB_EXCEPTION_MESSAGE(TRI_ERROR_STARTING_UP,
...
Result UserManagerImpl::storeUserInternal(User const& entry,
  ...
  if (res.ok()) {
    ...
    if (res.is(TRI_ERROR_DDB_CONFLICT)) {
...
Result UserManagerImpl::updateUser(std::string const& name, UserCallback&& func,
  ...
  if (r.ok() || r.is(TRI_ERROR_DDB_CONFLICT)) {
...
VPackBuilder UserManagerImpl::serializeUser(std::string const& user) {
  ...
  THROW_DDB_EXCEPTION(TRI_ERROR_USER_NOT_FOUND);  // FIXME do not use
}
