...
Result GraphManager::findOrCreateCollectionsByEdgeDefinition(
  ...
  } else if (!res.is(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND)) {
    return res;
  } else {
...
ResultT<std::unique_ptr<Graph>> GraphManager::lookupGraphByName(
  ...
  if (result.fail()) {
    if (result.is(TRI_ERROR_DDB_DOCUMENT_NOT_FOUND)) {
...
OperationResult GraphManager::removeGraph(Graph const& graph, bool waitForSync,
  ...
  {  // Remove from _graphs
    ...
    Result res = trx.begin();
    if (res.fail()) {
      return OperationResult(TRI_ERROR_DDB_DOCUMENT_NOT_FOUND, options);
    }
