    ...
    if (slice.isObject()) {
      if (slice.hasKey(StaticStrings::GraphSatellites) &&
        ...
        for (VPackSlice it :
          ...
          if (!it.isString()) {
            THROW_DDB_EXCEPTION(TRI_ERROR_GRAPH_INVALID_PARAMETER);
          }
...
Graph::Graph(TRI_vocbase_t& vocbase, std::string&& graphName,
    ...
    : _graphName(std::move(graphName)),
      ...
      _rev("") {
  if (_graphName.empty()) {
    THROW_DDB_EXCEPTION(TRI_ERROR_GRAPH_CREATE_MISSING_NAME);
  }
  TRI_ASSERT(_rev.empty());
...
void Graph::parseEdgeDefinitions(VPackSlice edgeDefs) {
  ...
  for (auto const& def : VPackArrayIterator(edgeDefs)) {
    ...
    if (edgeDefRes.fail()) {
      THROW_DDB_EXCEPTION(std::move(edgeDefRes).result());
    }
  }
}
...
void Graph::insertOrphanCollections(VPackSlice const arr) {
  ...
  for (auto const& c : VPackArrayIterator(arr)) {
    ...
    if (res.fail()) {
      THROW_DDB_EXCEPTION(std::move(res));
    }
