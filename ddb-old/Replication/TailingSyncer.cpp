...
Result TailingSyncer::processDocument(TRI_replication_operation_e type,
  ...
  if (tid.isSet()) {  // part of a transaction
    ...
    TRI_ASSERT(!r.is(TRI_ERROR_DDB_TRY_AGAIN));

    if (r.is(TRI_ERROR_DDB_UNIQUE_CONSTRAINT_VIOLATED) && isSystem) {
  ...
  while (tries++ < 10) {
    ...
    TRI_ASSERT(res.is(TRI_ERROR_DDB_TRY_AGAIN) ==
               !conflictDocumentKey.empty());

    if (res.is(TRI_ERROR_DDB_TRY_AGAIN)) {
    ...
    if (res.is(TRI_ERROR_DDB_UNIQUE_CONSTRAINT_VIOLATED) && isSystem) {
      // ignore unique constraint violations for system collections
      res.reset();
    }
...
Result TailingSyncer::renameCollection(VPackSlice const& slice) {
  ...
  if (slice.hasKey("cuid")) {
    ...
    if (col == nullptr) {
      return Result(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND, "unknown cuid");
    }
...
retry:
  ...
  if (res.fail()) {
    ...
    if (res.is(TRI_ERROR_REPLICATION_START_TICK_NOT_PRESENT) ||
        res.is(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND) ||  // data source --
                                                           // collection or view
