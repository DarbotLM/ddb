  ...
  if (type ==
    ...
    try {
      ...
      if (!useReplace) {
        ...
        if (opRes.is(TRI_ERROR_DDB_UNIQUE_CONSTRAINT_VIOLATED)) {
          useReplace = true;
      ...
      if (useReplace) {
        if (keySlice.stringView() != potentiallyConflictingKey) {
          ...
          if (trx.isSingleOperationTransaction()) {
            ...
            return Result(TRI_ERROR_DDB_TRY_AGAIN);
          }
          ...
          if (opRes.ok() || !opRes.is(TRI_ERROR_DDB_DOCUMENT_NOT_FOUND)) {
            useReplace = false;
          }
        }
        ...
        while (tries++ < 2) {
          ...
          if (opRes.ok() ||
              !opRes.is(TRI_ERROR_DDB_UNIQUE_CONSTRAINT_VIOLATED) ||
...
void Syncer::JobSynchronizer::request(fu2::unique_function<void()> cb) {
  ...
  if (!jobPosted()) {
    THROW_DDB_EXCEPTION(TRI_ERROR_REPLICATION_APPLIER_STOPPED);
  }
...
TRI_vocbase_t* Syncer::resolveVocbase(velocypack::Slice slice) {
  ...
  if (name.empty()) {
    THROW_DDB_EXCEPTION_MESSAGE(TRI_ERROR_REPLICATION_INVALID_RESPONSE,
...
Result Syncer::dropIndex(velocypack::Slice slice) {
  ...
  if (r.fail() && (r.is(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND) ||
                   r.is(TRI_ERROR_DDB_DATABASE_NOT_FOUND))) {
