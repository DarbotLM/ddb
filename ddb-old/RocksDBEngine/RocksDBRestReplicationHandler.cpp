...
using namespace darbotdb::rocksutils;
...
RocksDBRestReplicationHandler::handleCommandBatch() {
  ...
  if (type == rest::RequestType::POST) {
    ...
    darbotdb::ServerId const clientId{
  ...
  if (type == rest::RequestType::PUT && len >= 2) {
    ...
    darbotdb::ServerId const clientId = std::get<darbotdb::ServerId>(res.get());
...
void RocksDBRestReplicationHandler::handleCommandLoggerFollow() {
  ...
  if (length > 0) {
    ...
    for (auto marker : darbotdb::velocypack::ArrayIterator(data)) {
      dumper.dump(marker);
...
/// a given tick value
/// this is an internal method use by darbotdb's replication that should not
...
void RocksDBRestReplicationHandler::handleCommandFetchKeys() {
  ...
  if (keys) {
    ...
    if (found) {
      ...
      // "offset" was introduced with darbotdb 3.3. if the client sends it,
...
void RocksDBRestReplicationHandler::handleCommandDump() {
  LOG_TOPIC("213e2", TRACE, darbotdb::Logger::REPLICATION)
  ...
  // print request
  LOG_TOPIC("2b20f", TRACE, darbotdb::Logger::REPLICATION)
