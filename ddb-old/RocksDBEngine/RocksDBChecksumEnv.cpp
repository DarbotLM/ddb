...
bool ChecksumHelper::writeShaFile(std::string const& fileName,
  ...
  LOG_TOPIC("80257", DEBUG, darbotdb::Logger::ENGINES)
  ...
  LOG_TOPIC("8f7ef", WARN, darbotdb::Logger::ENGINES)
...
void ChecksumHelper::checkMissingShaFiles() {
  ...
  for (auto it = fileList.begin(); it != fileList.end(); ++it) {
    ...
    if (shaIndex != std::string::npos) {
      ...
      if (nextIt != fileList.end() &&
        ...
        LOG_TOPIC("4eac9", DEBUG, darbotdb::Logger::ENGINES)
  ...
  if (!toProduce.empty()) {
    LOG_TOPIC("ff71d", INFO, darbotdb::Logger::ENGINES)
        << "calculating SHA256 checksums for " << toProduce.size()
        << " RocksDB .sst file(s)";
    size_t produced = 0;
    for (auto const& tempPath : toProduce) {
      LOG_TOPIC("d6c86", DEBUG, darbotdb::Logger::ENGINES)
      ...
      if (produced != toProduce.size() && (produced % 100 == 0)) {
        ...
        LOG_TOPIC("cf86b", INFO, darbotdb::Logger::ENGINES)
    ...
    if (toProduce.size() >= 10) {
      ...
      LOG_TOPIC("96bbd", INFO, darbotdb::Logger::ENGINES)
...
rocksdb::Status ChecksumWritableFile::Close() {
  ...
  if (!_helper->writeShaFile(_fileName, _checksumCalc.getChecksum())) {
    LOG_TOPIC("0b00e", WARN, darbotdb::Logger::ENGINES)
...
rocksdb::Status ChecksumEnv::NewWritableFile(
  ...
  try {
    ...
    LOG_TOPIC("8b19e", ERR, darbotdb::Logger::ENGINES)
...
rocksdb::Status ChecksumEnv::DeleteFile(std::string const& fileName) {
  if (_helper->isSstFile(fileName) || _helper->isBlobFile(fileName)) {
    ...
    if (!shaFileName.empty()) {
      ...
      if (res == TRI_ERROR_NO_ERROR) {
        LOG_TOPIC("e0a0d", DEBUG, darbotdb::Logger::ENGINES)
            << "deleteCalcFile: delete file succeeded for " << shaFileName;
      } else {
        LOG_TOPIC("acb34", WARN, darbotdb::Logger::ENGINES)
  ...
  if (s.ok()) {
    LOG_TOPIC("77a2a", DEBUG, darbotdb::Logger::ENGINES)
    ...
    LOG_TOPIC("ce937", WARN, darbotdb::Logger::ENGINES)
