...
void darbotdb::rocksdbStartupVersionCheck(ArangodServer& server,
  ...
  if (dbExisted) {
    ...
    if (s.IsNotFound() || oldVersion.size() != 1) {
      ...
      LOG_TOPIC("c9009", FATAL, Logger::ENGINES)
          << "You are using an old version of darbotdb, please update "
      ...
      if (s.ok() && endianSlice.size() == 1) {
        ...
        if (endianess == RocksDBEndianness::Little) {
          LOG_TOPIC("31103", FATAL, Logger::ENGINES)
              ...
              << " endianness from darbotdb 3.2 or 3.3. Using this on-disk "
                 "format "
                 "has a severe negative impact on write performance and is not "
                 "compatible with several newer darbotdb features. Please move "
                 "to the "
      ...
      if (s.ok() && storedValue.size() == 1) {
        if (storedValue[0] == '1') {
          if (!localValue && server.options()->processingResult().touched(
            ...
            // still continue, so it is possible to downgrade the darbotdb
            // version later.
          }
        }
