...
void checkIteratorStatus(rocksdb::Iterator const& iterator) {
  ...
  if (!s.ok()) {
    THROW_DDB_EXCEPTION(darbotdb::rocksutils::convertStatus(s));
  }
}
  ...
  try {
    ...
    rocksdb::Slice upper(bounds.end());
    {
      ...
      if (!s.ok()) {
        ...
        darbotdb::Result r = rocksutils::convertStatus(s);
        LOG_TOPIC("60468", WARN, darbotdb::Logger::ENGINES)
    ...
    if (useRangeDelete) {
      ...
      if (!s.ok()) {
        LOG_TOPIC("e7e1b", WARN, darbotdb::Logger::ENGINES)
    ...
    for (it->Seek(lower); it->Valid(); it->Next()) {
      ...
      if (counter >= 1000) {
        ...
        if (!status.ok()) {
          LOG_TOPIC("18fe8", WARN, darbotdb::Logger::ENGINES)
    ...
    if (counter > 0) {
      ...
      if (!status.ok()) {
        LOG_TOPIC("ba426", WARN, darbotdb::Logger::ENGINES)
            << "RocksDB key deletion failed: " << status.ToString();
        return rocksutils::convertStatus(status);
      }
    }
    ...
    LOG_TOPIC("c26f2", ERR, darbotdb::Logger::ENGINES)
        << "caught exception during RocksDB key prefix deletion: " << ex.what();
    return Result(ex.code(), ex.what());
  } catch (std::exception const& ex) {
    LOG_TOPIC("dfd4b", ERR, darbotdb::Logger::ENGINES)
        << "caught exception during RocksDB key prefix deletion: " << ex.what();
    return Result(TRI_ERROR_INTERNAL, ex.what());
  } catch (...) {
    LOG_TOPIC("16927", ERR, darbotdb::Logger::ENGINES)
...
Result compactAll(rocksdb::DB* db, bool changeLevel,
  ...
  LOG_TOPIC("d8a5d", INFO, darbotdb::Logger::ENGINES)
  ...
  for (auto* cf : cfs) {
    ...
    if (!s.ok()) {
      ...
      LOG_TOPIC("e46a3", WARN, darbotdb::Logger::ENGINES)
          << "compaction of entire RocksDB database key range failed: "
          << res.errorMessage();
      return res;
    }
  }
  LOG_TOPIC("65594", INFO, darbotdb::Logger::ENGINES)
