...
DECLARE_GAUGE(darbotdb_dump_ongoing, std::uint64_t,
              "Number of dumps currently ongoing");
DECLARE_GAUGE(darbotdb_dump_memory_usage, std::uint64_t,
              "Memory usage of currently ongoing dumps");
DECLARE_COUNTER(
    darbotdb_dump_threads_blocked_total,
...
RocksDBDumpManager::RocksDBDumpManager(RocksDBEngine& engine,
    ...
    : _engine(engine),
      _limits(limits),
      _dumpsOngoing(metricsFeature.add(darbotdb_dump_ongoing{})),
      _dumpsMemoryUsage(metricsFeature.add(darbotdb_dump_memory_usage{})),
      _dumpsThreadsBlocked(
          metricsFeature.add(darbotdb_dump_threads_blocked_total{})) {}
...
std::shared_ptr<RocksDBDumpContext> RocksDBDumpManager::createContext(
  ...
  if (rocksutils::rocksDBEndianness == RocksDBEndianness::Little) {
    THROW_DDB_EXCEPTION(TRI_ERROR_DDB_OLD_ROCKSDB_FORMAT);
  }
...
RocksDBDumpManager::MapType::iterator RocksDBDumpManager::lookupContext(
  ...
  if (it == _contexts.end()) {
    ...
    THROW_DDB_EXCEPTION_MESSAGE(TRI_ERROR_CURSOR_NOT_FOUND,
  ...
  if (!context->canAccess(database, user)) {
    THROW_DDB_EXCEPTION_MESSAGE(TRI_ERROR_FORBIDDEN,
