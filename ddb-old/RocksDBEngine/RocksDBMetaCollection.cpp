...
RocksDBMetaCollection::computeRevisionTree(uint64_t batchId) {
  ...
  if (!iter) {
    LOG_TOPIC("52617", WARN, darbotdb::Logger::ENGINES)
...
RocksDBMetaCollection::revisionTreeFromCollection(bool checkForBlockers) {
  ...
  if (res.fail()) {
    LOG_TOPIC("d1e53", WARN, darbotdb::Logger::ENGINES)
  ...
  if (!iter) {
    LOG_TOPIC("d1e54", WARN, darbotdb::Logger::ENGINES)
    ...
    while (true) {
      ...
      TRI_IF_FAILURE("rebuildRevisionTree::sleep") {
        if (RandomGenerator::interval(uint32_t(2000)) > 1750) {
          THROW_DDB_EXCEPTION_MESSAGE(TRI_ERROR_DEBUG,
      ...
      if (_meta.hasBlockerUpTo(beginSeq - 1)) {
        ...
        if (std::chrono::steady_clock::now() > endTime) {
          ...
          THROW_DDB_EXCEPTION_MESSAGE(TRI_ERROR_INTERNAL, std::move(msg));
        }
...
futures::Future<ErrorCode> RocksDBMetaCollection::doLock(
  ...
  LOG_TOPIC("d1e52", TRACE, darbotdb::Logger::ENGINES)
...
std::uint64_t RocksDBMetaCollection::RevisionTreeAccessor::compressedSize()
  ...
  _tree->serializeBinary(
      output, darbotdb::containers::MerkleTreeBase::BinaryFormat::Optimal);
  return output.size();
}
...
void RocksDBMetaCollection::RevisionTreeAccessor::hibernate(bool force) {
  ...
  _compressed.clear();
  _tree->serializeBinary(
      _compressed, darbotdb::containers::MerkleTreeBase::BinaryFormat::Optimal);
