...
using namespace darbotdb::application_features;
...
DECLARE_GAUGE(darbotdb_revision_tree_memory_usage, uint64_t,
              "Total memory consumed by all revision trees");
DECLARE_GAUGE(
    darbotdb_revision_tree_buffered_memory_usage, uint64_t,
    "Total memory consumed by buffered updates for all revision trees");
DECLARE_GAUGE(darbotdb_index_estimates_memory_usage, uint64_t,
              "Total memory consumed by all index selectivity estimates");
DECLARE_COUNTER(darbotdb_revision_tree_rebuilds_success_total,
                "Number of successful revision tree rebuilds");
DECLARE_COUNTER(darbotdb_revision_tree_rebuilds_failure_total,
                "Number of failed revision tree rebuilds");
DECLARE_COUNTER(darbotdb_revision_tree_hibernations_total,
                "Number of revision tree hibernations");
DECLARE_COUNTER(darbotdb_revision_tree_resurrections_total,
...
RocksDBEngine::RocksDBEngine(Server& server,
    ...
    : StorageEngine(server, kEngineName, name(), Server::id<RocksDBEngine>(),
      ...
      _metricsIndexEstimatorMemoryUsage(
          metrics.add(darbotdb_index_estimates_memory_usage{})),
      ...
      _metricsTreeMemoryUsage(
          metrics.add(darbotdb_revision_tree_memory_usage{})),
      _metricsTreeBufferedMemoryUsage(
          metrics.add(darbotdb_revision_tree_buffered_memory_usage{})),
      _metricsTreeRebuildsSuccess(
          metrics.add(darbotdb_revision_tree_rebuilds_success_total{})),
      _metricsTreeRebuildsFailure(
          metrics.add(darbotdb_revision_tree_rebuilds_failure_total{})),
      _metricsTreeHibernations(
          metrics.add(darbotdb_revision_tree_hibernations_total{})),
      _metricsTreeResurrections(
          metrics.add(darbotdb_revision_tree_resurrections_total{})),
      ...
      _sortingMethod(
          darbotdb::basics::VelocyPackHelper::SortingMethod::Correct) {
  ...
  options
      ->addOption(
          ...
          darbotdb::options::makeFlags(
              ...
              darbotdb::options::Flags::OnDBServer,
              darbotdb::options::Flags::OnSingle))
...
("resource limit exceeded").)");
  ...
  options
      ->addOption(
          ...
          darbotdb::options::makeFlags(darbotdb::options::Flags::OnDBServer,
                                       darbotdb::options::Flags::OnSingle))
...
attribute.)");
  ...
  options->addOption(
      ...
      darbotdb::options::makeFlags(
          ...
          darbotdb::options::Flags::OnDBServer,
          darbotdb::options::Flags::OnSingle,
          darbotdb::options::Flags::Uncommon));
...
on the write rate.)");
  ...
  options->addOption("--rocksdb.create-sha-files",
                     ...
                     darbotdb::options::makeFlags(
                         ...
                         darbotdb::options::Flags::OnDBServer,
                         darbotdb::options::Flags::OnSingle,
                         darbotdb::options::Flags::Enterprise));
#endif
...
RocksDB's actions into the logfile written by darbotdb (if the
...
RocksDB internals and performance.)");
  ...
  options
      ->addOption("--rocksdb.verify-sst",
                  ...
                  darbotdb::options::makeFlags(
                      darbotdb::options::Flags::Command,
                      darbotdb::options::Flags::DefaultNoComponents,
                      darbotdb::options::Flags::OnAgent,
                      darbotdb::options::Flags::OnDBServer,
                      darbotdb::options::Flags::OnSingle,
                      darbotdb::options::Flags::Uncommon))
  ...
  options
      ->addOption(
          ...
          darbotdb::options::makeFlags(
              darbotdb::options::Flags::DefaultNoComponents,
              darbotdb::options::Flags::Uncommon,
              darbotdb::options::Flags::Experimental,
              darbotdb::options::Flags::OnAgent,
              darbotdb::options::Flags::OnDBServer,
              darbotdb::options::Flags::OnSingle))
...
void RocksDBEngine::validateOptions(
  ...
  if (_syncInterval > 0) {
    if (_syncInterval < minSyncInterval) {
      ...
      LOG_TOPIC("bbd68", FATAL, darbotdb::Logger::CONFIG)
    ...
    if (_syncDelayThreshold > 0 && _syncDelayThreshold <= _syncInterval) {
      if (!options->processingResult().touched("rocksdb.sync-interval") &&
        ...
        // --rocksdb.sync-delay-threshold
        LOG_TOPIC("c3f45", WARN, darbotdb::Logger::CONFIG)
      ...
      LOG_TOPIC("c0fa3", WARN, darbotdb::Logger::CONFIG)
  ...
  if (_pruneWaitTimeInitial < 10) {
    LOG_TOPIC("a9667", WARN, darbotdb::Logger::ENGINES)
...
void RocksDBEngine::verifySstFiles(rocksdb::Options const& options) const {
  ...
  LOG_TOPIC("e210d", INFO, darbotdb::Logger::STARTUP)
  ...
  for (auto const& fileName : TRI_FullTreeDirectory(_path.c_str())) {
    ...
    if (!res.ok()) {
      ...
      LOG_TOPIC("2943c", FATAL, darbotdb::Logger::STARTUP)
  ...
  LOG_TOPIC("02224", INFO, darbotdb::Logger::STARTUP)
...
struct RocksDBAsyncLogWriteBatcherMetricsImpl
  ...
  explicit RocksDBAsyncLogWriteBatcherMetricsImpl(
    ...
    using namespace darbotdb::replication2::storage::rocksdb;
    numWorkerThreadsWaitForSync = &metricsFeature->add(
        darbotdb_replication2_rocksdb_num_persistor_worker{}.withLabel("ws",
                                                                       "true"));
    numWorkerThreadsNoWaitForSync = &metricsFeature->add(
        darbotdb_replication2_rocksdb_num_persistor_worker{}.withLabel(
            "ws", "false"));

    queueLength =
        &metricsFeature->add(darbotdb_replication2_rocksdb_queue_length{});
    writeBatchSize =
        &metricsFeature->add(darbotdb_replication2_rocksdb_write_batch_size{});
    rocksdbWriteTimeInUs =
        &metricsFeature->add(darbotdb_replication2_rocksdb_write_time{});
    rocksdbSyncTimeInUs =
        &metricsFeature->add(darbotdb_replication2_rocksdb_sync_time{});

    operationLatencyInsert = &metricsFeature->add(
        darbotdb_replication2_storage_operation_latency{}.withLabel("op",
                                                                    "insert"));
    operationLatencyRemoveFront = &metricsFeature->add(
        darbotdb_replication2_storage_operation_latency{}.withLabel(
            "op", "remove-front"));
...
void RocksDBEngine::start() {
  ...
  LOG_TOPIC("107fd", TRACE, darbotdb::Logger::ENGINES)
  ...
  if (!basics::FileUtils::isDirectory(_path)) {
    ...
    if (res == TRI_ERROR_NO_ERROR) {
      LOG_TOPIC("b2958", TRACE, darbotdb::Logger::ENGINES)
          << "created RocksDB data directory '" << _path << "'";
      createdEngineDir = true;
    } else {
      LOG_TOPIC("a5ae3", FATAL, darbotdb::Logger::ENGINES)
    ...
    _sortingMethod =
        _forceLegacySortingMethod
            ? darbotdb::basics::VelocyPackHelper::SortingMethod::Legacy
            : darbotdb::basics::VelocyPackHelper::SortingMethod::Correct;
  ...
  if (TRI_GetDiskSpaceInfo(_path, totalSpace, freeSpace).ok() &&
      totalSpace != 0) {
    LOG_TOPIC("b71b9", DEBUG, darbotdb::Logger::ENGINES)
  ...
  LOG_TOPIC("bc82a", TRACE, darbotdb::Logger::ENGINES)
  ...
  if (!_optionsProvider.useFileLogging()) {
    ...
    // our own logger that logs to darbotdb's logfile
  ...
  darbotdb::rocksdbStartupVersionCheck(server(), _db, dbExisted,
  ...
  TRI_ASSERT((darbotdb::ServerState::instance()->isCoordinator() ||
              darbotdb::ServerState::instance()->isAgent()) ||
             _useReleasedTick);
...
void RocksDBEngine::getDatabases(darbotdb::velocypack::Builder& result) {
  ...
  for (iter->Seek(rSlice); iter->Valid() && iter->key().starts_with(rSlice);
    ...
    if (!idSlice.isString()) {
      LOG_TOPIC("099d7", ERR, darbotdb::Logger::STARTUP)
          << "found invalid database declaration with non-string id: "
          << slice.toJson();
      THROW_DDB_EXCEPTION(TRI_ERROR_DDB_ILLEGAL_PARAMETER_FILE);
    }

    // deleted
    if (darbotdb::basics::VelocyPackHelper::getBooleanValue(slice, "deleted",
                                                            false)) {
      TRI_voc_tick_t id = static_cast<TRI_voc_tick_t>(
          basics::StringUtils::uint64(idSlice.copyString()));

      // database is deleted, skip it!
      LOG_TOPIC("43cbc", DEBUG, darbotdb::Logger::STARTUP)
    ...
    if (!nameSlice.isString()) {
      LOG_TOPIC("96ffc", ERR, darbotdb::Logger::STARTUP)
...
ErrorCode RocksDBEngine::getCollectionsAndIndexes(
  ...
  for (iter->Seek(rSlice); iter->Valid() && iter->key().starts_with(rSlice);
    ...
    if (darbotdb::basics::VelocyPackHelper::getBooleanValue(
...
ErrorCode RocksDBEngine::getViews(TRI_vocbase_t& vocbase,
  ...
  for (iter->Seek(bounds.start()); iter->Valid(); iter->Next()) {
    ...
    if (ServerState::instance()->isDBServer() &&
        darbotdb::basics::VelocyPackHelper::getStringView(
...
void RocksDBEngine::processTreeRebuilds() {
  Scheduler* scheduler = darbotdb::SchedulerFeature::SCHEDULER;
  ...
  while (++iterations <= maxParallelRebuilds) {
    ...
    scheduler->queue(darbotdb::RequestLane::CLIENT_SLOW, [this, candidate]() {
      if (!server().isStopping()) {
        VocbasePtr vocbase;
        try {
...
void RocksDBEngine::processCompactions() {
  ...
  while (++iterations <= maxIterations) {
    ...
    scheduler->queue(darbotdb::RequestLane::CLIENT_SLOW, [this, bounds]() {
      if (server().isStopping()) {
...
Result RocksDBEngine::dropCollection(TRI_vocbase_t& vocbase,
  ...
  if (numDocs > 0) {
    ...
    THROW_DDB_EXCEPTION_MESSAGE(TRI_ERROR_INTERNAL, errorMsg);
  }
#endif
...
void RocksDBEngine::addSystemDatabase() {
  ...
  if (res.fail()) {
    LOG_TOPIC("8c26a", FATAL, darbotdb::Logger::STARTUP)
...
std::unique_ptr<TRI_vocbase_t> RocksDBEngine::openExistingDatabase(
  ...
  LOG_TOPIC("26c21", TRACE, darbotdb::Logger::ENGINES)
  ...
  auto scanViews = [&](std::string_view type) {
    try {
      ...
      LOG_TOPIC("48f11", TRACE, darbotdb::Logger::ENGINES)
      ...
      for (VPackSlice it : VPackArrayIterator(slice)) {
        ...
        LOG_TOPIC("4dfdd", TRACE, darbotdb::Logger::ENGINES)
      ...
      LOG_TOPIC("584b1", ERR, darbotdb::Logger::ENGINES)
          << "error while opening database '" << vocbase->name()
          << "': " << ex.what();
      throw;
    } catch (...) {
      LOG_TOPIC("593fd", ERR, darbotdb::Logger::ENGINES)
          << "error while opening database '" << vocbase->name()
          << "': unknown exception";
      throw;
    }
  ...
  if (vocbase->replicationVersion() == replication::Version::TWO) {
    ...
    try {
      ...
      LOG_TOPIC("554c1", ERR, darbotdb::Logger::ENGINES)
          << "error while opening database: " << ex.what();
      throw;
    } catch (...) {
      LOG_TOPIC("5f33d", ERR, darbotdb::Logger::ENGINES)
  ...
  try {
    ...
    LOG_TOPIC("f1275", TRACE, darbotdb::Logger::ENGINES)
        << "processing collections metadata in database '" << vocbase->name()
        << "': " << slice.toJson();

    for (VPackSlice it : VPackArrayIterator(slice)) {
      // we found a collection that is still active
      LOG_TOPIC("b2ef2", TRACE, darbotdb::Logger::ENGINES)
      ...
      if (r.fail()) {
        LOG_TOPIC("4a404", ERR, darbotdb::Logger::ENGINES)
            << "error while "
      ...
      LOG_TOPIC("39404", DEBUG, darbotdb::Logger::ENGINES)
          << "added collection '" << vocbase->name() << "/"
          << collection->name() << "'";
    }
  } catch (std::exception const& ex) {
    LOG_TOPIC("8d427", ERR, darbotdb::Logger::ENGINES)
        << "error while opening database '" << vocbase->name()
        << "': " << ex.what();
    throw;
  } catch (...) {
    LOG_TOPIC("0268e", ERR, darbotdb::Logger::ENGINES)
        << "error while opening database '" << vocbase->name()
        << "': unknown exception";
    throw;
  }
...
Result RocksDBEngine::createLoggerState(TRI_vocbase_t* vocbase,
  ...
  builder.add("version", VPackValue(darbotdb_VERSION));
...
bool RocksDBEngine::checkExistingDB(
  ...
  if (!status.ok()) {
    ...
    Result res = rocksutils::convertStatus(status);
    if (res.isNot(TRI_ERROR_DDB_IO_ERROR)) {
      // not an I/O error. so we better report the error and abort here
      LOG_TOPIC("74b7f", FATAL, darbotdb::Logger::STARTUP)
  ...
  if (status.ok()) {
    ...
    LOG_TOPIC("528b8", DEBUG, darbotdb::Logger::STARTUP)
    ...
    for (auto const& it : cfFamilies) {
      ...
      if (it2 == existingColumnFamilies.end()) {
        ...
        LOG_TOPIC("d9df8", FATAL, darbotdb::Logger::STARTUP)
            << "column family '" << it.name << "' is missing in database"
            << ". if you are upgrading from an earlier alpha or beta version "
               "of darbotdb, it is required to restart with a new database "
               "directory and "
               "re-import data";
        FATAL_ERROR_EXIT();
      }
    ...
    if (existingColumnFamilies.size() <
        RocksDBColumnFamilyManager::minNumberOfColumnFamilies) {
      LOG_TOPIC("e99ec", FATAL, darbotdb::Logger::STARTUP)
          << "unexpected number of column families found in database ("
...
using SortingMethod = darbotdb::basics::VelocyPackHelper::SortingMethod;
...
SortingMethod RocksDBEngine::readSortingFile() {
  ...
  try {
    ...
    value = darbotdb::basics::StringUtils::trim(value);
    sortingMethod =
...
std::string RocksDBEngine::getSortingMethodFile() const {
  return darbotdb::basics::FileUtils::buildFilename(_basePath,
                                                    kSortingMethodFile);
}

std::string RocksDBEngine::getLanguageFile() const {
  return darbotdb::basics::FileUtils::buildFilename(_basePath, kLanguageFile);
}
