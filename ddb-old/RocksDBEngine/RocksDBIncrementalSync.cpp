  ...
  iterator.next(
      [&](rocksdb::Slice const& rocksKey, rocksdb::Slice const& rocksValue) {
        ...
        if (docKey.compare(lowRef) < 0) {
          ...
          if (r.fail() && r.isNot(TRI_ERROR_DDB_DOCUMENT_NOT_FOUND)) {
  ...
  iterator.next(
      [&](rocksdb::Slice const& rocksKey, rocksdb::Slice const& rocksValue) {
        ...
        if (docKey.compare(highRef) > 0) {
          ...
          if (r.ok()) {
            ++stats.numDocsRemoved;
          } else if (r.isNot(TRI_ERROR_DDB_DOCUMENT_NOT_FOUND)) {
            // ignore not found, we remove conflicting docs ahead of time
            THROW_DDB_EXCEPTION(r);
          }
        }
  ...
  while (true) {
    ...
    for (VPackSlice it : VPackArrayIterator(slice)) {
      ...
      bool mustInsert = physical
                            ...
                            .is(TRI_ERROR_DDB_DOCUMENT_NOT_FOUND);
      ...
      while (tries-- > 0) {
        if (tries == 0) {
          options.indexOperationMode = darbotdb::IndexOperationMode::normal;
        }
        ...
        options.indexOperationMode = darbotdb::IndexOperationMode::internal;
