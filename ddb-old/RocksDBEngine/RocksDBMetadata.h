...
struct RocksDBMetadata final {
 ...
 public:
  ...
  darbotdb::Result serializeMeta(rocksdb::WriteBatch&, LogicalCollection&,
                                 bool force, darbotdb::velocypack::Builder&,
                                 rocksdb::SequenceNumber& appliedSeq,
                                 std::string& output);

  /// @brief deserialize collection metadata, only called on startup
  darbotdb::Result deserializeMeta(rocksdb::DB*, LogicalCollection&);
 ...
 private:
  ...
  mutable darbotdb::basics::ReadWriteLock _blockerLock;
