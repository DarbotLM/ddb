...
rocksdb::CompressionType compressionTypeFromString(std::string_view type) {
  ...
  TRI_ASSERT(false);
  LOG_TOPIC("edc91", FATAL, darbotdb::Logger::STARTUP)
...
rocksdb::CompactionStyle compactionStyleFromString(std::string_view type) {
  ...
  TRI_ASSERT(false);
  LOG_TOPIC("edc92", FATAL, darbotdb::Logger::STARTUP)
...
void RocksDBOptionFeature::collectOptions(
  ...
  options
      ->addOption("--rocksdb.transaction-lock-stripes",
                  ...
                  darbotdb::options::makeFlags(
                      darbotdb::options::Flags::Dynamic,
                      darbotdb::options::Flags::DefaultNoComponents,
                      darbotdb::options::Flags::OnAgent,
                      darbotdb::options::Flags::OnDBServer,
                      darbotdb::options::Flags::OnSingle))
...
number of cross-page I/O operations.)");
  ...
  options->addOption(
      ...
      darbotdb::options::makeFlags(darbotdb::options::Flags::Uncommon));
...
incompatible with darbotdb versions with a lower RocksDB version in case of
downgrading.)");
  ...
  options
      ->addOption(
          ...
          darbotdb::options::makeFlags(darbotdb::options::Flags::Experimental,
                                       darbotdb::options::Flags::Uncommon,
                                       darbotdb::options::Flags::OnAgent,
                                       darbotdb::options::Flags::OnDBServer,
                                       darbotdb::options::Flags::OnSingle))
...
of outgrowing the maximum number of file descriptors the darbotdb process 
...
void RocksDBOptionFeature::validateOptions(
  ...
  if (_blockCacheType == ::kBlockCacheTypeLRU &&
    ...
    LOG_TOPIC("a527b", WARN, darbotdb::Logger::ENGINES)
...
rocksdb::Options RocksDBOptionFeature::doGetOptions() const {
  ...
  if (!_useFileLogging) {
    // if we don't use file logging but log into darbotdb's logfile,
...
rocksdb::BlockBasedTableOptions RocksDBOptionFeature::doGetTableOptions()
  ...
  if (_checksumType == ::kChecksumTypeCRC32C) {
    ...
    TRI_ASSERT(false);
    LOG_TOPIC("8d602", WARN, darbotdb::Logger::STARTUP)
