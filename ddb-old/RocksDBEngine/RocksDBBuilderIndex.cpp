  ...
  for (it->Seek(bounds.start()); it->Valid(); it->Next()) {
    ...
    if (numDocsWritten > 0 &&
      ...
      if (ridx.collection().vocbase().isDropped()) {
        // database dropped
        res.reset(TRI_ERROR_DDB_DATABASE_NOT_FOUND);
        break;
      }
      if (ridx.collection().deleted()) {
        // collection dropped
        res.reset(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND);
        break;
      }
...
static Result fillIndex(
  ...
  if (ridx.type() == darbotdb::Index::TRI_IDX_TYPE_VECTOR_INDEX) {
...
#ifdef USE_ENTERPRISE
  if (darbotdb::rocksutils::rocksDBEndianness == RocksDBEndianness::Little &&
      numThreads > 1) {
  ...
  bool Continue() override {
    ...
    if (++_iterations % 128 == 0) {
      ...
      if (_index.collection().vocbase().isDropped()) {
        // database dropped
        tmpRes.reset(TRI_ERROR_DDB_DATABASE_NOT_FOUND);
      } else if (_index.collection().deleted()) {
        // collection dropped
        tmpRes.reset(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND);
      }
    }
    return tmpRes.ok();
  }
...
futures::Future<Result> RocksDBBuilderIndex::fillIndexBackground(
  ...
  do {
    ...
    if (res.fail() && !res.is(TRI_ERROR_DDB_TRY_AGAIN)) {
      co_return res;
    }
