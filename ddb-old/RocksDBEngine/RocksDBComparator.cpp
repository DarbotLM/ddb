...
template<darbotdb::basics::VelocyPackHelper::SortingMethod sortingMethod>
int compareIndexedValues(darbotdb::velocypack::Slice const& lhs,
                         darbotdb::velocypack::Slice const& rhs) {
  TRI_ASSERT(lhs.isArray());
  TRI_ASSERT(rhs.isArray());

  darbotdb::velocypack::ArrayIterator lhsIter(lhs);
  darbotdb::velocypack::ArrayIterator rhsIter(rhs);
  ...
  do {
    ...
    if constexpr (sortingMethod ==
      ...
      res = darbotdb::basics::VelocyPackHelper::compareLegacy(
          (lhsValid ? *lhsIter : VPackSlice::noneSlice()),
          (rhsValid ? *rhsIter : VPackSlice::noneSlice()), true);
    } else {
      res = darbotdb::basics::VelocyPackHelper::compare(
          (lhsValid ? *lhsIter : VPackSlice::noneSlice()),
          (rhsValid ? *rhsIter : VPackSlice::noneSlice()), true);
    }
    if (res != 0) {
