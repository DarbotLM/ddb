...
namespace {
void buildSettings(darbotdb::StorageEngine& engine, VPackBuilder& b,
                   uint64_t seqNumber) {
  b.clear();
  b.openObject();
...
darbotdb::Result writeSettings(VPackSlice slice, rocksdb::WriteBatch& batch) {
  using darbotdb::Logger;
  using darbotdb::RocksDBColumnFamilyManager;
  using darbotdb::RocksDBKey;
  using darbotdb::RocksDBSettingsType;
  ...
  if (!s.ok()) {
    ...
    return darbotdb::rocksutils::convertStatus(s);
  }
...
ResultT<bool> RocksDBSettingsManager::sync(bool force) {
  ...
  try {
    ...
    auto& dbfeature = _engine.server().getFeature<darbotdb::DatabaseFeature>();
