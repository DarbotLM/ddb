...
void RocksDBThrottle::setThrottleWriteRate(std::chrono::microseconds micros,
  ...
  LOG_TOPIC("7afe9", DEBUG, darbotdb::Logger::ENGINES)
...
void RocksDBThrottle::threadLoop() {
  ...
  LOG_TOPIC("a4a57", DEBUG, darbotdb::Logger::ENGINES)
      << "ThreadLoop() started";
  ...
  while (_throttleState.load(std::memory_order_relaxed) ==
    ...
    try {
      ...
      LOG_TOPIC("b0a2e", WARN, darbotdb::Logger::ENGINES)
  ...
  LOG_TOPIC("eebbe", DEBUG, darbotdb::Logger::ENGINES) << "ThreadLoop() ended";
}
...
void RocksDBThrottle::recalculateThrottle() {
  ...
  // lock _threadMutex while we update _throttleData
  {
    ...
    if (!_firstThrottle) {
      ...
      LOG_TOPIC("46d4a", DEBUG, darbotdb::Logger::ENGINES)
      ...
      LOG_TOPIC("e0bbb", DEBUG, darbotdb::Logger::ENGINES)
