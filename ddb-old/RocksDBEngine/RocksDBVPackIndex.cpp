...
void RocksDBVPackIndex::buildIndexValues(
  ...
  auto moveOn = [&](VPackSlice something) -> void {
    ...
    if (it == seen.end()) {
      ...
      THROW_DDB_EXCEPTION(TRI_ERROR_DDB_UNIQUE_CONSTRAINT_VIOLATED);
    }
  };
  ...
  if (_unique) {
    ...
    for (RocksDBKey const& key : elements) {
      ...
      if (s.ok()) {  // detected conflicting index entry
        ...
        res.reset(TRI_ERROR_DDB_UNIQUE_CONSTRAINT_VIOLATED);
        ...
        if (readResult.fail()) {
          ...
          THROW_DDB_EXCEPTION(readResult);
        }
        TRI_ASSERT(res.is(TRI_ERROR_DDB_UNIQUE_CONSTRAINT_VIOLATED));
        break;
