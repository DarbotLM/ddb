...
RocksDBFulltextIndex::RocksDBFulltextIndex(
    IndexId iid, darbotdb::LogicalCollection& collection,
  ...
  if (value.isNumber()) {
    ...
    THROW_DDB_EXCEPTION_MESSAGE(TRI_ERROR_BAD_PARAMETER,
...
bool RocksDBFulltextIndex::matchesDefinition(VPackSlice const& info) const {
  ...
  auto typeSlice = info.get(darbotdb::StaticStrings::IndexType);
  TRI_ASSERT(typeSlice.isString());
  TRI_ASSERT(typeSlice.stringView() == oldtypeName());
#endif
  auto value = info.get(darbotdb::StaticStrings::IndexId);
  ...
  value = info.get(darbotdb::StaticStrings::IndexFields);
  ...
  if (_unique != darbotdb::basics::VelocyPackHelper::getBooleanValue(
                     info, darbotdb::StaticStrings::IndexUnique, false)) {
    return false;
  }

  if (_sparse != darbotdb::basics::VelocyPackHelper::getBooleanValue(
                     info, darbotdb::StaticStrings::IndexSparse, true)) {
    return false;
  }

  // This check takes ordering of attributes into account.
  std::vector<darbotdb::basics::AttributeName> translate;
  for (size_t i = 0; i < n; ++i) {
    ...
    if (!darbotdb::basics::AttributeName::isIdentical(_fields[i], translate,
                                                      false)) {
      return false;
    }
  }
...
static void ExtractWords(std::set<std::string>& words, VPackSlice const value,
  ...
  if (value.isString()) {
    ...
    darbotdb::basics::Utf8Helper::DefaultUtf8Helper.tokenize(
...
Result RocksDBFulltextIndex::parseQueryString(std::string const& qstr,
  ...
  while (*ptr) {
    ...
    auto sg = darbotdb::scopeGuard([&]() noexcept { TRI_Free(lowered); });
...
std::unique_ptr<IndexIterator> RocksDBFulltextIndex::iteratorForCondition(
  ...
  TRI_ASSERT(fcall->type == darbotdb::aql::NODE_TYPE_FCALL);
