...
futures::Future<std::shared_ptr<Index>> RocksDBCollection::createIndex(
  ...
  if (!co_await locker.lock()) {
    THROW_DDB_EXCEPTION(TRI_ERROR_LOCK_TIMEOUT);
  }

  {
    ...
    if (auto existingIdx = findIndex(info, _indexes); existingIdx != nullptr) {
      ...
      if (existingIdx->type() == darbotdb::Index::TRI_IDX_TYPE_TTL_INDEX) {
  ...
  try {
    ...
    THROW_DDB_EXCEPTION_MESSAGE(TRI_ERROR_DDB_INDEX_CREATION_FAILED,
                                   ex.what());
  }
  ...
  auto lambda = [&]() -> futures::Future<Result> {
    ...
    if (vocbase.replicationVersion() == replication::Version::TWO &&
      ...
      if (!replicationResult.ok()) {
        THROW_DDB_EXCEPTION(std::move(replicationResult).result());
      }
    }
...
Result RocksDBCollection::lookup(transaction::Methods* trx,
  ...
  do {
    ...
    documentId = primaryIndex()->lookupKey(
        ...
        foundInCache);
    if (!documentId.isSet()) {
      res.reset(TRI_ERROR_DDB_DOCUMENT_NOT_FOUND);
      return res;
    }  // else found
  ...
  } while (res.is(TRI_ERROR_DDB_DOCUMENT_NOT_FOUND) &&
           RocksDBTransactionState::toState(trx)->ensureSnapshot());
  if (res.is(TRI_ERROR_DDB_DOCUMENT_NOT_FOUND)) {
...
void RocksDBCollection::figuresSpecific(
    bool details, darbotdb::velocypack::Builder& builder) {
  ...
  if (std::shared_ptr<cache::Cache> cache = useCache()) {
    do {
      ...
      if (status == TRI_ERROR_NO_ERROR ||
          status == TRI_ERROR_DDB_DOCUMENT_NOT_FOUND) {
        break;
