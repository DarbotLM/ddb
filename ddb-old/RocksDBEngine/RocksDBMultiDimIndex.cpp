...
ResultT<zkd::byte_string> readDocumentKey(
  ...
  for (auto const& path : fields) {
    ...
    if (std::isnan(dv)) {
      THROW_DDB_EXCEPTION_MESSAGE(TRI_ERROR_QUERY_INVALID_ARITHMETIC_VALUE,
                                     "NaN is not allowed");
    }
...
void useAsBound(size_t idx, aql::AstNode* bound_value, bool isLower,
                bool isStrict,
                std::unordered_map<size_t, darbotdb::mdi::ExpressionBounds>&
                    extractedBounds) {
...
ResultT<transaction::BuilderLeaser> extractAttributeValues(
  ...
  for (auto const& it : storedValues) {
    ...
    if (s.isNull() && !nullAllowed) {
      return {TRI_ERROR_DDB_DOCUMENT_KEY_MISSING};
    }
...
Result RocksDBMdiIndex::insert(transaction::Methods& trx,
  ...
  if (!isPrefixed()) {
    ...
    if (result.fail()) {
      TRI_ASSERT(_sparse);
      TRI_ASSERT(result.is(TRI_ERROR_DDB_DOCUMENT_KEY_MISSING));
...
bool RocksDBMdiIndexBase::matchesDefinition(VPackSlice const& info) const {
  ...
  auto value = info.get(darbotdb::StaticStrings::IndexPrefixFields);
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    if (!darbotdb::basics::AttributeName::isIdentical(_prefixFields[i],
                                                      translate, false)) {
      return false;
    }
  }
