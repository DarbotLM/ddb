...
uint64_t RocksDBKeyBounds::objectId() const {
  ...
  switch (_type) {
    ...
    default:
      THROW_DDB_EXCEPTION(TRI_ERROR_TYPE_ERROR);
  }
#else
  return uint64FromPersistent(_internals.buffer().data());
#endif
...
RocksDBKeyBounds::RocksDBKeyBounds(RocksDBEntryType type, uint64_t id,
  ...
  switch (_type) {
    ...
    default:
      THROW_DDB_EXCEPTION(TRI_ERROR_BAD_PARAMETER);
  }
}
...
RocksDBKeyBounds::RocksDBKeyBounds(RocksDBEntryType type, uint64_t first)
  ...
  switch (_type) {
    ...
    case RocksDBEntryType::ReplicatedState: {
      ...
      // Key: 1 + 8-byte darbotdb database ID + 8-byte darbotdb collection ID
...
RocksDBKeyBounds::RocksDBKeyBounds(RocksDBEntryType type, uint64_t first,
  ...
  switch (_type) {
    case RocksDBEntryType::Document: {
      ...
      // (used in databases created with darbotdb 3.2 and 3.3), since
...
std::ostream& operator<<(std::ostream& stream, RocksDBKeyBounds const& bounds) {
  stream << "[bounds cf: "
         ...
         << " type: " << darbotdb::rocksDBEntryTypeName(bounds.type()) << " ";
