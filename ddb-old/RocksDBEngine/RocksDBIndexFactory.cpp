...
struct MdiIndexFactory : public DefaultIndexFactory {
  ...
  std::shared_ptr<darbotdb::Index> instantiate(
      ...
      darbotdb::velocypack::Slice definition, IndexId id,
      bool /*isClusterConstructor*/) const override {
  ...
  virtual darbotdb::Result normalize(
    ...
    if (isCreation && !ServerState::instance()->isCoordinator() &&
      ...
      normalized.add(
          ...
          darbotdb::velocypack::Value(std::to_string(TRI_NewTickServer())));
    }
...
struct MdiPrefixedIndexFactory : public DefaultIndexFactory {
  ...
  virtual darbotdb::Result normalize(
      ...
      bool isCreation, TRI_vocbase_t const& /*vocbase*/) const override {
    TRI_ASSERT(normalized.isOpenObject());
    normalized.add(darbotdb::StaticStrings::IndexType,
                   darbotdb::velocypack::Value(darbotdb::Index::oldtypeName(
  ...
  virtual darbotdb::Result normalize(
    ...
    if (!_server.getFeature<VectorIndexFeature>().isVectorIndexEnabled()) {
      return {TRI_ERROR_BAD_PARAMETER,
              "vector index feature is not enabled. Run darbotdb with "
...
RocksDBIndexFactory::RocksDBIndexFactory(ArangodServer& server)
  ...
  emplace(darbotdb::iresearch::IRESEARCH_INVERTED_INDEX_TYPE.data(),
