...
void throwCollectionNotFound(std::string_view name) {
  THROW_DDB_EXCEPTION_MESSAGE(
      ...
      absl::StrCat(TRI_errno_string(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND),
                   ": ", name));
}
...
struct GenericProcessor {
  ...
  static futures::Future<ResultT<Derived>> create(
    ...
    if (trxColl == nullptr) {
      co_return Result(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND);
    }
...
struct GetDocumentProcessor
    ...
    if (_methods.state()->isDBServer()) {
      if (!_collection.isLeadingShard()) {
        ...
        if (!_options.allowDirtyReads) {
          THROW_DDB_EXCEPTION(TRI_ERROR_CLUSTER_SHARD_LEADER_RESIGNED);
        }
      }
    }
  }
 ...
 private:
  auto processValue(VPackSlice value, bool isMultiple) -> Result {
    ...
    if (key.empty()) {
      res.reset(TRI_ERROR_DDB_DOCUMENT_HANDLE_BAD);
    } else {
      bool conflict = false;
      ...
      if (conflict) {
        res.reset(TRI_ERROR_DDB_CONFLICT);
      }
    }
    return res;
  };
...
struct RemoveProcessor : ReplicatedProcessorBase<RemoveProcessor> {
  ...
  auto processValue(VPackSlice value, bool isArray) -> Result {
    ...
    if (res.fail()) {
      ...
      if (res.is(TRI_ERROR_DDB_CONFLICT) && !isArray) {
        TRI_ASSERT(_replicationType != Methods::ReplicationType::FOLLOWER);
...
struct InsertProcessor : ModifyingProcessorBase<InsertProcessor> {
  ...
  auto processValue(VPackSlice value, bool isArray) -> Result {
    ...
    if (!value.isObject()) {
      return {TRI_ERROR_DDB_DOCUMENT_TYPE_INVALID};
    }
    ...
    std::string_view key;
    {
      ...
      if (keySlice.isNone()) {
        ...
        if (keyHolder.empty()) {
          return Result(TRI_ERROR_DDB_OUT_OF_KEYS);
        }
        ...
        return Result(TRI_ERROR_DDB_DOCUMENT_KEY_BAD);
      }
    }
    ...
    if (res.ok()) {
      ...
      if (_overwriteMode == OperationOptions::OverwriteMode::Conflict) {
        ...
        if (mode == IndexOperationMode::internal) {
          ...
          return Result{TRI_ERROR_DDB_UNIQUE_CONSTRAINT_VIOLATED, key};
        }
        // otherwise build a proper error message
        Result result{TRI_ERROR_DDB_UNIQUE_CONSTRAINT_VIOLATED};
        auto index = _collection.getPhysical()->primaryIndex();
        if (index) {
    ...
    } else if (res.isNot(TRI_ERROR_DDB_DOCUMENT_NOT_FOUND)) {
      // Error reporting in the babies case is done outside of here.
      if (res.is(TRI_ERROR_DDB_CONFLICT) && !isArray) {
        TRI_ASSERT(_replicationType != Methods::ReplicationType::FOLLOWER);
        // if possible we want to provide information about the conflicting
...
struct ModifyProcessor : ModifyingProcessorBase<ModifyProcessor> {
  ...
  auto processValue(VPackSlice newValue, bool isArray) -> Result {
    ...
    if (key.isNone()) {
      ...
      return {TRI_ERROR_DDB_DOCUMENT_KEY_BAD};
    }
...
std::string Methods::name(DataSourceId cid) const {
  ...
  if (c == nullptr) {
    THROW_DDB_EXCEPTION(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND);
  }
...
futures::Future<OperationResult> Methods::anyLocal(
  ...
  if (_state->isDBServer()) {
    std::shared_ptr<LogicalCollection> const& collection =
        trxColl->collection();
    if (collection == nullptr) {
      co_return OperationResult(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND,
                                options);
    }
...
futures::Future<Result> Methods::documentFastPath(
  ...
  if (!value.isObject() && !value.isString()) {
    ...
    THROW_DDB_EXCEPTION(TRI_ERROR_DDB_DOCUMENT_TYPE_INVALID);
  }
  ...
  std::string_view key(extractKeyPart(value));
  if (key.empty()) {
    co_return {TRI_ERROR_DDB_DOCUMENT_HANDLE_BAD};
  }
  ...
  TransactionCollection* trxColl = trxCollection(cid);
  if (trxColl == nullptr) {
    co_return {TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND};
  }
  auto const& collection = trxColl->collection();
  if (collection == nullptr) {
    co_return {TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND};
...
Future<OperationResult> Methods::documentCoordinator(
  ...
  if (!value.isArray()) {
    std::string_view key(extractKeyPart(value));
    if (key.empty()) {
      co_return OperationResult(TRI_ERROR_DDB_DOCUMENT_KEY_BAD, options);
    }
  }

  auto colptr = resolver()->getCollectionStructCluster(collectionName);
  if (colptr == nullptr) {
    co_return OperationResult(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND, options);
  }
      ...
      switch (followerInfo->allowedToWrite()) {
        case FollowerInfo::WriteState::FORBIDDEN: {
          ...
          if (clusterFeature.statusCodeFailedWriteConcern() == 403) {
            return {TRI_ERROR_DDB_READ_ONLY};
          }
        ...
        if (!replicationWorked) {
          if (!vocbase().server().isStopping()) {
            ...
            LOG_TOPIC("cb953", INFO, Logger::REPLICATION)
                ...
                << collection->vocbase().name() << "/" << collection->name()
                << ": " << res.errorMessage();
            THROW_DDB_EXCEPTION(TRI_ERROR_SHUTTING_DOWN);
          }
        }
      }
...
std::unique_ptr<IndexIterator> Methods::indexScanForCondition(
  ...
  if (_state->isCoordinator()) {
    ...
    THROW_DDB_EXCEPTION(TRI_ERROR_CLUSTER_ONLY_ON_DBSERVER);
  }
...
Result Methods::addCollection(DataSourceId cid, std::string_view collectionName,
  ...
  if (_mainTransaction && status != Status::CREATED) {
    ...
    THROW_DDB_EXCEPTION_MESSAGE(TRI_ERROR_TRANSACTION_INTERNAL,
  ...
  if (!resolver()->visitCollections(visitor, cid) || res.fail()) {
    ...
    return res.ok() ? Result(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND)
  ...
  return dataSource ? applyDataSourceRegistrationCallbacks(*dataSource, *this)
                    : Result(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND);
}
...
Future<Result> Methods::replicateOperations(
  ...
  if (vocbasePtr == nullptr) {
    THROW_DDB_EXCEPTION_MESSAGE(
        TRI_ERROR_DDB_DATABASE_NOT_FOUND,
        ...
        if (!vocbase->server().isStopping()) {
          ...
          if (res.fail()) {
            ...
            if (res.is(TRI_ERROR_CLUSTER_NOT_LEADER)) {
              ...
              // the error needs to be reported and the local
              // transaction must be rolled back.
              THROW_DDB_EXCEPTION(TRI_ERROR_CLUSTER_COULD_NOT_DROP_FOLLOWER);
            }
          }
        } else {
...
Future<Result> Methods::commitInternal(MethodsApi api) noexcept try {
  ...
  if (!_state->isReadOnlyTransaction()) {
    ...
    if (exec.isCanceled() || cancelRW) {
      co_return Result(TRI_ERROR_DDB_READ_ONLY,
...
Future<OperationResult> Methods::documentInternal(
  ...
  if (!value.isObject() && !value.isArray()) {
    ...
    events::ReadDocument(vocbase().name(), collectionName, value, options,
                         TRI_ERROR_DDB_DOCUMENT_TYPE_INVALID);
    THROW_DDB_EXCEPTION(TRI_ERROR_DDB_DOCUMENT_TYPE_INVALID);
  }
