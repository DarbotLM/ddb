...
futures::Future<Result> Manager::addCollections(
    ...
    for (auto const& cname : cols) {
      ...
      if (cid.empty()) {
        ...
        res.reset(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND,
                  absl::StrCat(
                      TRI_errno_string(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND),
                      ": ", cname));
      } else {
#ifdef USE_ENTERPRISE
        if (state->isCoordinator()) {
  ...
  if (!co_await addCols(exclusiveCollections, AccessMode::Type::EXCLUSIVE) ||
    ...
    co_return res.reset(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND);
  }
  ...
  if (fanout) {
    ...
    if (!futures.empty()) {
      ...
      for (auto const& it : responses) {
        if (!it.hasValue()) {
          THROW_DDB_EXCEPTION(TRI_ERROR_CLUSTER_BACKEND_UNAVAILABLE);
        }
        auto& res = it.get();
