...
darbotdb::Result existsCollection(darbotdb::ddbServer& server,
                                  std::string const& database,
                                  std::string const& collection) {
  if (!server.hasFeature<darbotdb::DatabaseFeature>()) {
  ...
  if (wildcard == database) {
    return darbotdb::Result();  // wildcard always matches
  }
  ...
  if (!vocbase) {
    return darbotdb::Result(TRI_ERROR_DDB_DATABASE_NOT_FOUND);
  }
  ...
  return !darbotdb::CollectionNameResolver(*vocbase).getCollection(collection)
             ? darbotdb::Result(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND)
             : darbotdb::Result();
}
...
RestUsersHandler::RestUsersHandler(ddbServer& server,
