...
RestDatabaseHandler::RestDatabaseHandler(ddbServer& server,
  ...
  if (suffixes.empty() || suffixes[0] == "user") {
    ...
    if (suffixes.empty()) {
      if (!_vocbase.isSystem()) {
        res.reset(TRI_ERROR_ddb_USE_SYSTEM_DATABASE);
      } else {
...
RestStatus RestDatabaseHandler::createDatabase() {
  if (!_vocbase.isSystem()) {
    generateError(
        GeneralResponse::responseCode(TRI_ERROR_ddb_USE_SYSTEM_DATABASE),
        TRI_ERROR_ddb_USE_SYSTEM_DATABASE);
    events::CreateDatabase("", Result(TRI_ERROR_ddb_USE_SYSTEM_DATABASE),
                           _context);
    return RestStatus::DONE;
  }
  ...
  if (res.ok()) {
    ...
    if (res.is(TRI_ERROR_FORBIDDEN) ||
        res.is(TRI_ERROR_ddb_DUPLICATE_NAME)) {
      generateError(res);
...
RestStatus RestDatabaseHandler::deleteDatabase() {
  if (!_vocbase.isSystem()) {
    generateError(
        GeneralResponse::responseCode(TRI_ERROR_ddb_USE_SYSTEM_DATABASE),
        TRI_ERROR_ddb_USE_SYSTEM_DATABASE);
    events::DropDatabase("", Result(TRI_ERROR_ddb_USE_SYSTEM_DATABASE),
                         _context);
    return RestStatus::DONE;
  }
