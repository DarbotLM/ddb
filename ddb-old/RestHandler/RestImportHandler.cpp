...
RestImportHandler::RestImportHandler(ddbServer& server,
...
auto RestImportHandler::executeAsync() -> futures::Future<futures::Unit> {
  ...
  switch (type) {
    case rest::RequestType::POST: {
      ...
      if (_request->contentType() == darbotdb::ContentType::VPACK) {
...
ErrorCode RestImportHandler::handleSingleDocument(
  ...
  try {
    darbotdb::basics::VelocyPackHelper::ensureStringValue(
    ...
    registerError(result, errorMsg);
    return TRI_ERROR_DDB_INVALID_EDGE_ATTRIBUTE;
  }
...
futures::Future<futures::Unit> RestImportHandler::createFromJson(
  ...
  if (!found || collectionName.empty()) {
    generateError(rest::ResponseCode::BAD,
                  TRI_ERROR_DDB_COLLECTION_PARAMETER_MISSING,
    ...
    if (resultSlice.isArray()) {
      ...
      for (VPackSlice it : VPackArrayIterator(resultSlice)) {
        ...
        if (!s.isBool() || !s.getBool()) {
          ...
          if (errorCode == TRI_ERROR_DDB_UNIQUE_CONSTRAINT_VIOLATED &&
              _onDuplicateAction == DUPLICATE_IGNORE) {
...
void RestImportHandler::createVelocyPackObject(VPackBuilder& result,
  ...
  if (!values.isArray()) {
    ...
    THROW_DDB_EXCEPTION_MESSAGE(TRI_ERROR_BAD_PARAMETER, errorMsg);
  }
