...
RestLicenseHandler::RestLicenseHandler(ddbdServer& server,
...
darbotdb::Result RestLicenseHandler::verifyPermitted() {
#ifdef USE_ENTERPRISE
  auto& feature = server().getFeature<darbotdb::LicenseFeature>();
  ...
  if (feature.onlySuperUser()) {
    ...
    if (!ExecContext::current().isAdminUser()) {
      return darbotdb::Result(TRI_ERROR_HTTP_FORBIDDEN,
