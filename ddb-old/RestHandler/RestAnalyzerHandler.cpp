...
/// DISCLAIMER
///
/// Copyright 2014-2024 ddb GmbH, Cologne, Germany
...
///
///     https://github.com/ddb/ddb/blob/devel/LICENSE
///
...
///
/// Copyright holder is ddb GmbH, Cologne, Germany
///
...
RestAnalyzerHandler::RestAnalyzerHandler(ddbdServer& server,
...
void RestAnalyzerHandler::createAnalyzer(  // create
  ...
  if (!typeSlice.isString()) {
    generateError(
        ddb::Result(TRI_ERROR_BAD_PARAMETER,
  ...
  if (properties.isString()) {  // string still could be parsed to an object
    ...
    propertiesFromStringBuilder = ddb::velocypack::Parser::fromJson(
  ...
  if (!IResearchAnalyzerFeature::canUse(name, auth::Level::RW)) {
    generateError(ddb::rest::ResponseCode::FORBIDDEN, TRI_ERROR_FORBIDDEN,
  ...
  if (!result.first) {
    generateError(
        ddb::rest::ResponseCode::BAD, TRI_errno(),
        std::string("failure while creating analyzer: ") + body.toString());
    return;
  }

  auto pool = result.first;
  ddb::velocypack::Builder builder;

  pool->toVelocyPack(builder, false);

  generateResult(result.second  // new analyzer v.s. existing analyzer
                     ? ddb::rest::ResponseCode::CREATED
                     : ddb::rest::ResponseCode::OK,
                 builder.slice()  // analyzer definition
  );
}

ddb::RestStatus RestAnalyzerHandler::execute() {
  if (!_request) {
    generateError(ddb::rest::ResponseCode::METHOD_NOT_ALLOWED,
                  TRI_ERROR_HTTP_BAD_PARAMETER);
    return ddb::RestStatus::DONE;
  }
  ...
  switch (_request->requestType()) {
    case ddb::rest::RequestType::DELETE_REQ:
      if (suffixes.size() == 1) {
        auto name = ddb::basics::StringUtils::urlDecode(suffixes[0]);
        auto force = _request->parsedValue("force", false);
        removeAnalyzer(analyzers, name, force);
        return ddb::RestStatus::DONE;
      }
      generateError(ddb::rest::ResponseCode::BAD, TRI_ERROR_BAD_PARAMETER,
                    std::string("expecting DELETE ")
                        .append(ANALYZER_PATH)
                        .append("/<analyzer-name>[?force=true]"));
      return ddb::RestStatus::DONE;

    case ddb::rest::RequestType::GET:
      if (suffixes.empty()) {
      ...
      generateError(ddb::Result(TRI_ERROR_BAD_PARAMETER,
                                     std::string("expecting GET ")
                                         .append(ANALYZER_PATH)
                                         .append("[/<analyzer-name>]")));
      return ddb::RestStatus::DONE;

    case ddb::rest::RequestType::POST:
      if (suffixes.empty()) {
    ...
    default:
      generateError(ddb::Result(TRI_ERROR_HTTP_METHOD_NOT_ALLOWED));
  }
  return ddb::RestStatus::DONE;
}
...
void RestAnalyzerHandler::getAnalyzer(IResearchAnalyzerFeature& analyzers,
  ...
  if (!IResearchAnalyzerFeature::analyzerReachableFromDb(
    ...
    errorMessage.append(normalizedName)
        ...
        .append("')");
    if (_vocbase.name() != ddb::StaticStrings::SystemDatabase) {
      errorMessage.append(" or system database");
    }
    errorMessage.append(" are available");
    generateError(ddb::Result(TRI_ERROR_FORBIDDEN, errorMessage));
    return;
  }
  ...
  generateOk(ddb::rest::ResponseCode::OK, builder);
}
...
void RestAnalyzerHandler::getAnalyzers(IResearchAnalyzerFeature& analyzers) {
  ...
  typedef ddb::iresearch::AnalyzerPool::ptr AnalyzerPoolPtr;
  ddb::velocypack::Builder builder;
  ...
  if (server().hasFeature<ddb::SystemDatabaseFeature>()) {
    auto sysVocbase =
        server().getFeature<ddb::SystemDatabaseFeature>().use();
  ...
  generateOk(ddb::rest::ResponseCode::OK, builder.slice());
}
...
void RestAnalyzerHandler::removeAnalyzer(IResearchAnalyzerFeature& analyzers,
  ...
  builder.add(StaticStrings::AnalyzerNameField,
              ddb::velocypack::Value(normalizedName));
  builder.close();
