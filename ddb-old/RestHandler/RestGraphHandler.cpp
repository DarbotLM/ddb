...
using namespace darbotdb::graph;
...
futures::Future<Result> RestGraphHandler::executeGharial() {
  ...
  // ERROR_DDB_DATA_SOURCE_NOT_FOUND.
...
futures::Future<darbotdb::Result> RestGraphHandler::vertexSetsAction(
    Graph& graph) {
  switch (request()->requestType()) {
    case RequestType::GET:
...
futures::Future<darbotdb::Result> RestGraphHandler::edgeSetsAction(
    Graph& graph) {
  switch (request()->requestType()) {
    case RequestType::GET:
...
futures::Future<darbotdb::Result> RestGraphHandler::edgeSetAction(
...
futures::Future<darbotdb::Result> RestGraphHandler::vertexSetAction(
...
std::unique_ptr<Graph> RestGraphHandler::getGraph(
  ...
  if (graphResult.fail()) {
    THROW_DDB_EXCEPTION(std::move(graphResult).result());
  }
...
// HTTP/Gharial!
futures::Future<darbotdb::Result> RestGraphHandler::edgeActionRemove(
    Graph& graph, std::string const& definitionName, std::string const& key) {
  bool waitForSync =
...
futures::Future<darbotdb::Result> RestGraphHandler::vertexActionUpdate(
    graph::Graph& graph, std::string const& collectionName,
    std::string const& key) {
  return vertexModify(graph, collectionName, key, true);
}

futures::Future<darbotdb::Result> RestGraphHandler::vertexActionReplace(
    graph::Graph& graph, std::string const& collectionName,
    std::string const& key) {
  return vertexModify(graph, collectionName, key, false);
}

futures::Future<darbotdb::Result> RestGraphHandler::vertexActionCreate(
    graph::Graph& graph, std::string const& collectionName) {
  return vertexCreate(graph, collectionName);
}

futures::Future<darbotdb::Result> RestGraphHandler::edgeActionUpdate(
    graph::Graph& graph, std::string const& collectionName,
    std::string const& key) {
  return edgeModify(graph, collectionName, key, true);
}

futures::Future<darbotdb::Result> RestGraphHandler::edgeActionReplace(
    graph::Graph& graph, std::string const& collectionName,
    std::string const& key) {
  return edgeModify(graph, collectionName, key, false);
}

futures::Future<darbotdb::Result> RestGraphHandler::edgeModify(
    graph::Graph& graph, std::string const& collectionName,
    std::string const& key, bool isPatch) {
  return documentModify(graph, collectionName, key, isPatch, TRI_COL_TYPE_EDGE);
}

futures::Future<darbotdb::Result> RestGraphHandler::edgeCreate(
    graph::Graph& graph, std::string const& collectionName) {
  return documentCreate(graph, collectionName, TRI_COL_TYPE_EDGE);
}

futures::Future<darbotdb::Result> RestGraphHandler::edgeActionCreate(
    graph::Graph& graph, std::string const& collectionName) {
  return edgeCreate(graph, collectionName);
}

futures::Future<darbotdb::Result> RestGraphHandler::vertexModify(
    graph::Graph& graph, std::string const& collectionName,
    std::string const& key, bool isPatch) {
...
futures::Future<darbotdb::Result> RestGraphHandler::vertexCreate(
    graph::Graph& graph, std::string const& collectionName) {
  return documentCreate(graph, collectionName, TRI_COL_TYPE_DOCUMENT);
}

// /_api/gharial/{graph-name}/edge/{definition-name}
futures::Future<darbotdb::Result> RestGraphHandler::editEdgeDefinition(
    graph::Graph& graph, std::string const& edgeDefinitionName) {
  return modifyEdgeDefinition(graph, EdgeDefinitionAction::EDIT,
                              edgeDefinitionName);
}

futures::Future<darbotdb::Result> RestGraphHandler::createEdgeDefinition(
    graph::Graph& graph) {
  return modifyEdgeDefinition(graph, EdgeDefinitionAction::CREATE);
}

// /_api/gharial/{graph-name}/edge
futures::Future<darbotdb::Result> RestGraphHandler::modifyEdgeDefinition(
...
futures::Future<darbotdb::Result> RestGraphHandler::modifyVertexDefinition(
...
futures::Future<darbotdb::Result> RestGraphHandler::removeEdgeDefinition(
    graph::Graph& graph, std::string const& edgeDefinitionName) {
...
// should gharial, too?
futures::Future<darbotdb::Result> RestGraphHandler::documentModify(
    graph::Graph& graph, std::string const& collectionName,
...
futures::Future<darbotdb::Result> RestGraphHandler::documentCreate(
    graph::Graph& graph, std::string const& collectionName,
  ...
  if (!body.isObject()) {
    co_return returnError(TRI_ERROR_DDB_DOCUMENT_TYPE_INVALID);
  }
...
futures::Future<darbotdb::Result> RestGraphHandler::vertexActionRemove(
    graph::Graph& graph, std::string const& collectionName,
    std::string const& key) {
  bool waitForSync =
      _request->parsedValue(StaticStrings::WaitForSyncString, false);
