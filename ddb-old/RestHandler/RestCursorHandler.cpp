...
RestCursorHandler::RestCursorHandler(
    ddbServer& server, GeneralRequest* request, GeneralResponse* response,
    darbotdb::aql::QueryRegistry* queryRegistry)
...
async<void> RestCursorHandler::handleQueryResult() {
  ...
  if (_queryResult.result.fail()) {
    ...
    THROW_DDB_EXCEPTION(_queryResult.result);
  }
  ...
  if (n <= batchSize) {
    ...
    try {
      ...
      THROW_DDB_EXCEPTION_MESSAGE(TRI_ERROR_INTERNAL, ex.what());
    } catch (...) {
      THROW_DDB_EXCEPTION(TRI_ERROR_OUT_OF_MEMORY);
    }
...
ResultT<std::pair<std::string, bool>> RestCursorHandler::forwardingTarget() {
  ...
  uint64_t tick = darbotdb::basics::StringUtils::uint64(suffixes[0]);
...
void RestCursorHandler::registerQuery(
    std::shared_ptr<darbotdb::aql::Query> query) {
...
RestStatus RestCursorHandler::deleteQueryCursor() {
  ...
  auto cursorId =
      static_cast<darbotdb::CursorId>(basics::StringUtils::uint64(id));
