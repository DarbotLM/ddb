...
class RestGraphHandler : public darbotdb::RestVocbaseBaseHandler {
 private:
 ...
 public:
  RestGraphHandler(ddbServer& server, GeneralRequest* request,
 ...
 private:
  ...
  futures::Future<darbotdb::Result> executeGharial();

  // /_api/gharial
  darbotdb::Result graphsAction();

  // /_api/gharial/{graph-name}
  darbotdb::Result graphAction(graph::Graph& graph);

  // /_api/gharial/{graph-name}/vertex
  futures::Future<darbotdb::Result> vertexSetsAction(graph::Graph& graph);

  // /_api/gharial/{graph-name}/edge
  futures::Future<darbotdb::Result> edgeSetsAction(graph::Graph& graph);

  // /_api/gharial/{graph-name}/vertex/{collection-name}
  futures::Future<darbotdb::Result> vertexSetAction(
      graph::Graph& graph, std::string const& vertexCollectionName);

  // /_api/gharial/{graph-name}/edge/{definition-name}
  futures::Future<darbotdb::Result> edgeSetAction(
      graph::Graph& graph, std::string const& edgeDefinitionName);

  // /_api/gharial/{graph-name}/vertex/{collection-name}/{vertex-key}
  futures::Future<darbotdb::Result> vertexAction(
      graph::Graph& graph, std::string const& vertexCollectionName,
      std::string const& vertexKey);

  // /_api/gharial/{graph-name}/edge/{definition-name}/{edge-key}
  futures::Future<darbotdb::Result> edgeAction(
      graph::Graph& graph, std::string const& edgeDefinitionName,
  ...
  futures::Future<darbotdb::Result> vertexActionRemove(
      graph::Graph& graph, std::string const& collectionName,
      std::string const& key);

  // PATCH /_api/gharial/{graph-name}/vertex/{collection-name}/{vertex-key}
  futures::Future<darbotdb::Result> vertexActionUpdate(
      graph::Graph& graph, std::string const& collectionName,
      std::string const& key);

  // PUT /_api/gharial/{graph-name}/vertex/{collection-name}/{vertex-key}
  futures::Future<darbotdb::Result> vertexActionReplace(
      graph::Graph& graph, std::string const& collectionName,
      std::string const& key);

  // POST /_api/gharial/{graph-name}/vertex/{collection-name}/{vertex-key}
  futures::Future<darbotdb::Result> vertexActionCreate(
      graph::Graph& graph, std::string const& collectionName);
  ...
  futures::Future<darbotdb::Result> edgeActionRemove(
      graph::Graph& graph, std::string const& definitionName,
      std::string const& key);

  // POST /_api/gharial/{graph-name}/edge/{definition-name}/{edge-key}
  futures::Future<darbotdb::Result> edgeActionCreate(
      graph::Graph& graph, std::string const& definitionName);

  // PATCH /_api/gharial/{graph-name}/edge/{definition-name}/{edge-key}
  futures::Future<darbotdb::Result> edgeActionUpdate(
      graph::Graph& graph, std::string const& collectionName,
      std::string const& key);

  // PUT /_api/gharial/{graph-name}/edge/{definition-name}/{edge-key}
  futures::Future<darbotdb::Result> edgeActionReplace(
      graph::Graph& graph, std::string const& collectionName,
      std::string const& key);
  ...
  futures::Future<darbotdb::Result> vertexModify(
      graph::Graph& graph, std::string const& collectionName,
      std::string const& key, bool isPatch);

  futures::Future<darbotdb::Result> vertexCreate(
      graph::Graph& graph, std::string const& collectionName);
  ...
  futures::Future<darbotdb::Result> edgeModify(
      graph::Graph& graph, std::string const& collectionName,
      std::string const& key, bool isPatch);

  futures::Future<darbotdb::Result> edgeCreate(
      graph::Graph& graph, std::string const& collectionName);

  futures::Future<darbotdb::Result> documentModify(
      graph::Graph& graph, std::string const& collectionName,
      std::string const& key, bool isPatch, TRI_col_type_e colType);

  futures::Future<darbotdb::Result> documentCreate(
      graph::Graph& graph, std::string const& collectionName,
  ...
  futures::Future<darbotdb::Result> editEdgeDefinition(
      graph::Graph& graph, std::string const& edgeDefinitionName);

  // DELETE /_api/gharial/{graph-name}/edge/{definition-name}
  futures::Future<darbotdb::Result> removeEdgeDefinition(
      graph::Graph& graph, std::string const& edgeDefinitionName);

  // POST /_api/gharial/{graph-name}/edge/
  futures::Future<darbotdb::Result> createEdgeDefinition(graph::Graph& graph);

  // edgeDefinitionName may be omitted when action == CREATE
  futures::Future<darbotdb::Result> modifyEdgeDefinition(
      graph::Graph& graph, EdgeDefinitionAction action,
      std::string edgeDefinitionName = {});

  futures::Future<darbotdb::Result> modifyVertexDefinition(
