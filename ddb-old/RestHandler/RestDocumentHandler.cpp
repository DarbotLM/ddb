...
RestDocumentHandler::RestDocumentHandler(ddbServer& server,
...
async<void> RestDocumentHandler::insertDocument() {
  ...
  darbotdb::OperationOptions opOptions(_context);
...
async<void> RestDocumentHandler::modifyDocument(bool isPatch) {
  ...
  if ((!isArrayCase && !body.isObject()) || (isArrayCase && !body.isArray())) {
    generateTransactionError(
        cname,
        OperationResult(TRI_ERROR_ddb_DOCUMENT_TYPE_INVALID, opOptions), "");
    co_return;
  }
