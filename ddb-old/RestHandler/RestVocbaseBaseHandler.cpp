...
RestVocbaseBaseHandler::RestVocbaseBaseHandler(ddbServer& server,
...
void RestVocbaseBaseHandler::generateConflictError(OperationResult const& opres,
                                                   bool precFailed) {
  TRI_ASSERT(opres.is(TRI_ERROR_DDB_CONFLICT));
  auto code =
  ...
  VPackBuilder builder;
  {
    ...
    builder.add(StaticStrings::ErrorNum, VPackValue(TRI_ERROR_DDB_CONFLICT));
...
RestVocbaseBaseHandler::createTransaction(
  ...
  if (pos > 0 && pos < value.size()) {
    if (value.compare(pos, std::string::npos, " aql") == 0) {
      THROW_DDB_EXCEPTION_MESSAGE(TRI_ERROR_TRANSACTION_DISALLOWED_OPERATION,
