...
using namespace darbotdb::basics;
using namespace darbotdb::rest;
using namespace darbotdb::cluster;
using Helper = darbotdb::basics::VelocyPackHelper;
...
bool ignoreHiddenEnterpriseCollection(std::string const& name, bool force) {
  ...
  if (!force && name[0] == '_') {
    if (name.starts_with(StaticStrings::FullLocalPrefix) ||
      ...
      LOG_TOPIC("944c4", WARN, darbotdb::Logger::REPLICATION)
...
auto handlingOfExistingCollection(TRI_vocbase_t& vocbase,
      ...
      try {
        ...
        if (dropResult.fail()) {
          ...
          co_return Result(dropResult.errorNumber(),
                           darbotdb::basics::StringUtils::concatT(
  ...
  } else if (lookupResult.isNot(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND)) {
...
RestReplicationHandler::RestReplicationHandler(ddbServer& server,
      ...
      if (command == Dump) {
        ...
        if (ServerState::instance()->isCoordinator()) {
          ...
          if (maybeShardID.fail()) {
            ...
            return Result{TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND};
          }
        ...
        if (parameters.isObject()) {
          if (parameters.get("name").isString()) {
            ...
            if (!collectionName.empty()) {
              ...
              if (vocbase == nullptr) {
                return Result(TRI_ERROR_DDB_DATABASE_NOT_FOUND);
              }
...
void RestReplicationHandler::handleUnforwardedTrampolineCoordinator() {
  if (_request == nullptr) {
    THROW_DDB_EXCEPTION_MESSAGE(TRI_ERROR_INTERNAL, "invalid request");
  }
...
void RestReplicationHandler::handleCommandClusterInventory() {
  ...
  auto vocbase = databaseFeature.useDatabase(dbName);
  if (!vocbase) {
    generateError(ResponseCode::NOT_FOUND, TRI_ERROR_DDB_DATABASE_NOT_FOUND);
    return;
  }
...
RestReplicationHandler::handleCommandRestoreCollection() {
  ...
  if (res.is(TRI_ERROR_DDB_DUPLICATE_NAME)) {
...
futures::Future<Result> RestReplicationHandler::processRestoreCollection(
  ...
  if (darbotdb::basics::VelocyPackHelper::getBooleanValue(parameters, "deleted",
                                                          false)) {
...
futures::Future<Result> RestReplicationHandler::processRestoreUsersBatch() {
  ...
  auto query = darbotdb::aql::Query::create(
      ...
      darbotdb::aql::QueryString(aql), std::move(bindVars));
...
Result RestReplicationHandler::processRestoreIndexes(
  ...
  std::string const name = darbotdb::basics::VelocyPackHelper::getStringValue(
      parameters, "name", "");
  ...
  try {
    ...
    for (VPackSlice idxDef : VPackArrayIterator(indexes)) {
      ...
      std::shared_ptr<darbotdb::Index> idx;
      try {
        bool created = false;
...
Result RestReplicationHandler::processRestoreIndexesCoordinator(
  ...
  std::string name = darbotdb::basics::VelocyPackHelper::getStringValue(
      parameters, "name", "");
  ...
  if (col == nullptr) {
    return {TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND,
  ...
  for (VPackSlice idxDef : VPackArrayIterator(indexes)) {
    ...
    if (type.isEqualString(StaticStrings::IndexNameVector) &&
        !server().getFeature<VectorIndexFeature>().isVectorIndexEnabled()) {
      auto const indexName = darbotdb::basics::VelocyPackHelper::getStringValue(
          parameters, "name", "");
...
void RestReplicationHandler::handleCommandRestoreView() {
  ...
  try {
    ...
    if (view) {
      if (!overwrite) {
        generateError(
            GeneralResponse::responseCode(TRI_ERROR_DDB_DUPLICATE_NAME),
            TRI_ERROR_DDB_DUPLICATE_NAME,
            StringUtils::concatT(
                "unable to restore view '", nameSlice.copyString(), ": ",
                TRI_errno_string(TRI_ERROR_DDB_DUPLICATE_NAME)));
        return;
      }
...
RestReplicationHandler::handleCommandAddFollower() {
  ...
  if (col == nullptr || col->deleted()) {
    generateError(Result(TRI_ERROR_DDB_DATA_SOURCE_NOT_FOUND));
    co_return;
  }
  ...
  auto cleanup = scopeGuard([&body, this]() noexcept {
    try {
      ...
      darbotdb::ServerId const serverId{StringUtils::uint64(
...
/// @caller js/client/modules/@darbotdb/replication.js
...
bool RestReplicationHandler::prepareRevisionOperation(
  ...
  LOG_TOPIC("253e2", TRACE, darbotdb::Logger::REPLICATION)
  ...
  // print request
  LOG_TOPIC("2b70f", TRACE, darbotdb::Logger::REPLICATION)
...
bool RestReplicationHandler::prepareCollectionForRevisionOperation(
  ...
  // print request
  LOG_TOPIC("6e075", TRACE, darbotdb::Logger::REPLICATION)
      << "requested revision tree for collection '" << ctx.cname << "'";
...
void RestReplicationHandler::handleCommandRevisionRanges() {
  ...
  VPackBuilder response;
  {
    ...
    char ridBuffer[darbotdb::basics::maxUInt64StringSize];
    {
...
struct RebootCookie final : public darbotdb::TransactionState::Cookie {
...
Result RestReplicationHandler::isLockHeld(TransactionId id) const {
  ...
  if (_vocbase.isDropped()) {
    return {TRI_ERROR_DDB_DATABASE_NOT_FOUND};
  }
