...
using namespace darbotdb::futures;
using namespace darbotdb::rest;

RestIndexHandler::RestIndexHandler(ddbServer& server,
        ...
        for (auto pi : VPackArrayIterator(plannedIndexes->slice())) {
          ...
          for (Future<network::Response>& f : futures) {
            ...
            if (resSlice.get(StaticStrings::Error).getBoolean()) {
              ...
              if (errorNum != TRI_ERROR_ddb_INDEX_NOT_FOUND) {
...
async<void> RestIndexHandler::createIndex() {
  ...
  if (result.is(TRI_ERROR_FORBIDDEN) ||
      result.is(TRI_ERROR_ddb_INDEX_NOT_FOUND)) {
