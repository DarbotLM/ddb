...
RestWalAccessHandler::RestWalAccessHandler(ddbServer& server,
...
bool RestWalAccessHandler::parseFilter(WalAccess::Filter& filter) {
  ...
  if (_request->requestType() == darbotdb::rest::RequestType::PUT) {
...
void RestWalAccessHandler::handleCommandTickRange(WalAccess const* wal) {
  ...
  if (r.ok()) {
    ...
    {  // "server" part
      ...
      server->add("version", VPackValue(darbotdb_VERSION));
      server->add("serverId",
...
void RestWalAccessHandler::handleCommandTail(WalAccess const* wal) {
  ...
  if (_request->requestType() == darbotdb::rest::RequestType::DELETE_REQ) {
  ...
  auto prepOpts = [&handlers, &opts](TRI_vocbase_t& vocbase) -> void {
    auto it =
        handlers
            .try_emplace(vocbase.id(), darbotdb::lazyConstruct([&] {
  ...
  if (httpResponse == nullptr) {
    THROW_DDB_EXCEPTION_MESSAGE(TRI_ERROR_INTERNAL, "invalid response type");
  }
