...
RestCollectionHandler::RestCollectionHandler(ddbServer& server,
    ...
    if (checkSyncStatus) {
      ...
      if (maybeShard.ok()) {
        isSyncing = server().getFeature<MaintenanceFeature>().hasAction(
            ...
            darbotdb::maintenance::SYNCHRONIZE_SHARD);
      }
...
async<void> RestCollectionHandler::handleCommandPut() {
  ...
  if (sub == "rename") {
    ...
    if (!newNameSlice.isString()) {
      generateError(Result(TRI_ERROR_ddb_ILLEGAL_NAME, "name is empty"));
      co_return;
    }
...
futures::Future<futures::Unit> RestCollectionHandler::collectionRepresentation(
  ...
  if (figuresRes.fail()) {
    THROW_ddb_EXCEPTION(figuresRes.result);
  }
  ...
  if (opRes.fail()) {
    ...
    THROW_ddb_EXCEPTION(opRes.result);
  }
