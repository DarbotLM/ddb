...
////////////////////////////////////////////////////////////////////////////////
bool canUse(darbotdb::auth::Level level, TRI_vocbase_t const& vocbase) {
  return darbotdb::ExecContext::current().canUseDatabase(vocbase.name(), level);
}
...
RestViewHandler::RestViewHandler(ddbServer& server, GeneralRequest* request,
...
void RestViewHandler::getView(std::string const& nameOrId, bool detailed) {
  ...
  try {
    darbotdb::velocypack::Builder viewBuilder;
    ...
    generateError(darbotdb::Result(TRI_ERROR_INTERNAL));
...
void RestViewHandler::createView() {
  ...
  try {
    ...
    if (!view) {
      generateError(
          darbotdb::Result(TRI_ERROR_INTERNAL, "problem creating view"));
      events::CreateView(_vocbase.name(), nameSlice.copyString(),
                         TRI_ERROR_INTERNAL);
      return;
    }
    ...
    generateError(darbotdb::Result(ex.code(), ex.message()));
  } catch (...) {
    events::CreateView(_vocbase.name(), nameSlice.copyString(), TRI_errno());
    generateError(darbotdb::Result(TRI_errno(), "problem creating view"));
  }
}
...
void RestViewHandler::modifyView(bool partialUpdate) {
  ...
  // per https://github.com/darbotdb/backlog/issues/459
  builder.openObject();
...
void RestViewHandler::deleteView() {
  ...
  auto name = darbotdb::basics::StringUtils::urlDecode(suffixes[0]);
