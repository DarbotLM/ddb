...
class RestVocbaseBaseHandler : public RestBaseHandler {
 ...
 public:
  ...
  RestVocbaseBaseHandler(ddbServer&, GeneralRequest*, GeneralResponse*);
 ...
 protected:
  ...
  void generate20x(darbotdb::OperationResult const& result,
                   std::string_view collectionName, TRI_col_type_e type,
                   darbotdb::velocypack::Options const* options,
                   bool isMultiple, bool silent,
                   rest::ResponseCode waitForSyncResponseCode);

  // generates conflict error
  void generateConflictError(darbotdb::OperationResult const&,
